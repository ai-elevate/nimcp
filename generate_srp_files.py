#!/usr/bin/env python3
"""
SRP Refactoring File Generator
Generates all remaining split files for the Single Responsibility Principle refactoring
"""
...
# Base paths
SECURITY_DIR = Path("/home/bbrelin/nimcp/src/security")
ASYNC_DIR = Path("/home/bbrelin/nimcp/src/async")
TEST_SECURITY_DIR = Path("/home/bbrelin/nimcp/tests/security")
TEST_ASYNC_DIR = Path("/home/bbrelin/nimcp/tests/async")

# Ensure test directories exist
TEST_SECURITY_DIR.mkdir(parents=True, exist_ok=True)
TEST_ASYNC_DIR.mkdir(parents=True, exist_ok=True)

def create_corrigibility_goals():
    """Create nimcp_corrigibility_goals.c"""
    content = '''/**
 * @file nimcp_corrigibility_goals.c
 * @brief Corrigibility Goals - Goal Modification Acceptance
 * @version 1.0.0
 * @date 2026-02-16
 *
 * WHAT: Goal modification request processing and alignment verification
 * WHY:  Single Responsibility - separate goal modification from other concerns
 * HOW:  Accept goal changes, process modification requests with authority checks
 */

#include "nimcp_corrigibility_internal.h"
...
'''
    (SECURITY_DIR / "nimcp_corrigibility_goals.c").write_text(content)
    print("✓ Created nimcp_corrigibility_goals.c")

def create_corrigibility_constraints():
    """Create nimcp_corrigibility_constraints.c"""
    content = '''/**
 * @file nimcp_corrigibility_constraints.c
 * @brief Corrigibility Constraints - SAT Solver Verification and Enforcement
 * @version 1.0.0
 * @date 2026-02-16
 *
 * WHAT: Constraint verification via SAT solver and capability sync
 * WHY:  Single Responsibility - separate formal verification from operations
 * HOW:  SAT constraint encoding, self-mod verification, capability bidirectional sync
 */

#include "nimcp_corrigibility_internal.h"
#include "security/nimcp_capability_control.h"
#include "constants/nimcp_buffer_constants.h"
#include "constants/nimcp_json_constants.h"
#include "utils/logging/nimcp_logging.h"
#include "utils/exception/nimcp_exception_macros.h"
#include <string.h>
#include <stdio.h>
    ...
    corrigibility_verification_result_t* result)
{
    ...
    (void)sat_result;  /* Unused in current implementation */
    ...
    size_t report_size)
{
    ...
    (void)sat;  /* Unused in current implementation */
    ...
    size_t report_size)
{
    ...
    if (corr_flags->can_modify_own_code != cap_flags->can_modify_own_code && offset < sizeof(discrepancies) - 40) {
        synced = false;
    ...
    if (corr_flags->can_modify_own_weights != cap_flags->can_modify_own_weights && offset < sizeof(discrepancies) - 40) {
        synced = false;
        offset += snprintf(discrepancies + offset, sizeof(discrepancies) - offset,
    ...
    if (corr_flags->can_modify_safety_systems != cap_flags->can_modify_safety_systems && offset < sizeof(discrepancies) - 40) {
        synced = false;
        offset += snprintf(discrepancies + offset, sizeof(discrepancies) - offset,
    ...
    if (corr_flags->can_disable_logging != cap_flags->can_modify_logging && offset < sizeof(discrepancies) - 40) {
        synced = false;
        offset += snprintf(discrepancies + offset, sizeof(discrepancies) - offset,
    ...
    if (corr_flags->can_disable_monitoring != cap_flags->can_modify_monitoring && offset < sizeof(discrepancies) - 40) {
        synced = false;
        offset += snprintf(discrepancies + offset, sizeof(discrepancies) - offset,
...
'''
    (SECURITY_DIR / "nimcp_corrigibility_constraints.c").write_text(content)
    print("✓ Created nimcp_corrigibility_constraints.c")

def create_corrigibility_monitoring():
    """Create nimcp_corrigibility_monitoring.c"""
    content = '''/**
 * @file nimcp_corrigibility_monitoring.c
 * @brief Corrigibility Monitoring - Deference and Authority Management
 * @version 1.0.0
 * @date 2026-02-16
 *
 * WHAT: Self-modification monitoring, deference tracking, authority hierarchy
 * WHY:  Single Responsibility - separate behavioral monitoring from enforcement
 * HOW:  Authority registration, permission checks, deference logging
 */
...
'''
    (SECURITY_DIR / "nimcp_corrigibility_monitoring.c").write_text(content)
    print("✓ Created nimcp_corrigibility_monitoring.c")

def main():
    """Generate all remaining split files"""
    print("Generating SRP refactoring files...")

    # Corrigibility files
    create_corrigibility_goals()
    create_corrigibility_constraints()
    create_corrigibility_monitoring()

    print("\nCorrigibility module: COMPLETE (5/5 files)")
    print("\nRemaining modules (BBB, Capability Control, Continuous Monitor, Bio Router):")
    print("  - Due to file size, these require manual extraction from original sources")
    print("  - See REFACTORING_COMPLETE_PLAN.md for detailed line mappings")
    print("\nNext step: Extract remaining modules using line ranges from plan")
