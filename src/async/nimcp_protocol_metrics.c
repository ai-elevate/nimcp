...
/** Global health agent for protocol_metrics module */
static nimcp_health_agent_t* g_protocol_metrics_health_agent = NULL;

/**
 * @brief Set health agent for protocol_metrics heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void protocol_metrics_set_health_agent(nimcp_health_agent_t* agent) {
    g_protocol_metrics_health_agent = agent;
}

/** @brief Send heartbeat from protocol_metrics module */
static inline void protocol_metrics_heartbeat(const char* operation, float progress) {
    if (g_protocol_metrics_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_protocol_metrics_health_agent, operation, progress);
    }
}
...
bool protocol_metrics_export_file(protocol_metrics_t metrics,
    ...
    if (!metrics || !filename) return false;
