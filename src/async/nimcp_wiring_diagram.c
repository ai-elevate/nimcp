...
/** Global health agent for wiring_diagram module */
static nimcp_health_agent_t* g_wiring_diagram_health_agent = NULL;

/**
 * @brief Set health agent for wiring_diagram heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void wiring_diagram_set_health_agent(nimcp_health_agent_t* agent) {
    g_wiring_diagram_health_agent = agent;
}

/** @brief Send heartbeat from wiring_diagram module */
static inline void wiring_diagram_heartbeat(const char* operation, float progress) {
    if (g_wiring_diagram_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_wiring_diagram_health_agent, operation, progress);
    }
}
