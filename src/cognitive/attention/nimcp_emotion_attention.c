...
NIMCP_DECLARE_HEALTH_AGENT_ATOMIC(emotion_attention)
//=============================================================================
// Mesh Participant Registration
//=============================================================================

static mesh_participant_id_t g_emotion_attention_mesh_id = 0;
static mesh_participant_registry_t* g_emotion_attention_mesh_registry = NULL;

nimcp_error_t emotion_attention_mesh_register(mesh_participant_registry_t* registry) {
    if (!registry) return NIMCP_ERROR_NULL_POINTER;
    if (g_emotion_attention_mesh_id != 0) return NIMCP_SUCCESS;
    mesh_participant_interface_t iface;
    mesh_participant_interface_init(&iface);
    strncpy(iface.module_name, "emotion_attention", MESH_MAX_NAME_LEN - 1);
    iface.type = MESH_PARTICIPANT_MODULE;
    iface.home_channel = mesh_adapter_get_default_channel(MESH_ADAPTER_CATEGORY_COGNITIVE);
    mesh_participant_config_t config;
    mesh_participant_config_init(&config);
    config.module_name = "emotion_attention";
    config.type = MESH_PARTICIPANT_MODULE;
    config.home_channel = iface.home_channel;
    nimcp_error_t err = mesh_participant_register(registry, &iface, &config, &g_emotion_attention_mesh_id);
    if (err == NIMCP_SUCCESS) g_emotion_attention_mesh_registry = registry;
    return err;
}

void emotion_attention_mesh_unregister(void) {
    if (g_emotion_attention_mesh_registry && g_emotion_attention_mesh_id != 0) {
        mesh_participant_unregister(g_emotion_attention_mesh_registry, g_emotion_attention_mesh_id);
        g_emotion_attention_mesh_id = 0;
        g_emotion_attention_mesh_registry = NULL;
    }
}


/** @brief Send heartbeat from emotion_attention module (instance-level) */
static inline void emotion_attention_heartbeat_instance(
    nimcp_health_agent_t* instance_agent, const char* operation, float progress)
{
    if (g_emotion_attention_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_emotion_attention_health_agent, operation, progress);
    }
    if (instance_agent && instance_agent != g_emotion_attention_health_agent) {
...
emotion_attention_system_t* emotion_attention_create(
    ...
    emotion_attention_system_t* system = nimcp_calloc(1, sizeof(emotion_attention_system_t));
    if (!system) {
...
/**
 * WHAT: Query knowledge graph for Emotion Attention self-knowledge
...
int emotion_attention_query_self_knowledge(kg_reader_t* kg) {
    ...
    kg_relation_list_t* connections = kg_reader_get_relations_from(kg, "Emotion_Attention");
    if (connections) { kg_relation_list_destroy(connections); }
    kg_relation_list_t* incoming = kg_reader_get_relations_to(kg, "Emotion_Attention");
...
int emotion_attention_training_begin(void* instance) {
    if (!instance) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "emotion_attention_training_begin: NULL argument");
        return -1;
    }
    emotion_attention_heartbeat_instance(NULL, "emotion_attention_training_begin", 0.0f);
    (void)(struct emotion_attention_system*)instance; /* Module state available for reset */
    return 0;
}

int emotion_attention_training_end(void* instance) {
    if (!instance) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "emotion_attention_training_end: NULL argument");
        return -1;
    }
    emotion_attention_heartbeat_instance(NULL, "emotion_attention_training_end", 1.0f);
    (void)(struct emotion_attention_system*)instance; /* Module state available for finalization */
    return 0;
}

int emotion_attention_training_step(void* instance, float progress) {
    if (!instance) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "emotion_attention_training_step: NULL argument");
        return -1;
    }
    if (progress < 0.0f) progress = 0.0f;
    if (progress > 1.0f) progress = 1.0f;
    emotion_attention_heartbeat_instance(NULL, "emotion_attention_training_step", progress);
    (void)(struct emotion_attention_system*)instance; /* Module state available for step adaptation */
    return 0;
}
