...
/** Global health agent for creative_bridge module */
static nimcp_health_agent_t* g_creative_bridge_health_agent = NULL;

/**
 * @brief Set health agent for creative_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
void creative_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_creative_bridge_health_agent = agent;
}

/** @brief Send heartbeat from creative_bridge module */
static inline void creative_bridge_heartbeat(const char* operation, float progress) {
    if (g_creative_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_creative_bridge_health_agent, operation, progress);
    }
}
