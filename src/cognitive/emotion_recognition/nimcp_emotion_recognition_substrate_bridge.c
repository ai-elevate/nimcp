...
static mesh_participant_id_t g_emotion_recognition_substrate_bridge_mesh_id = 0;
static mesh_participant_registry_t* g_emotion_recognition_substrate_bridge_mesh_registry = NULL;

nimcp_error_t emotion_recognition_substrate_bridge_mesh_register(mesh_participant_registry_t* registry) {
    if (!registry) return NIMCP_ERROR_NULL_POINTER;
    if (g_emotion_recognition_substrate_bridge_mesh_id != 0) return NIMCP_SUCCESS;
    mesh_participant_interface_t iface;
    mesh_participant_interface_init(&iface);
    strncpy(iface.module_name, "emotion_recognition_substrate_bridge", MESH_MAX_NAME_LEN - 1);
    iface.type = MESH_PARTICIPANT_MODULE;
    iface.home_channel = mesh_adapter_get_default_channel(MESH_ADAPTER_CATEGORY_COGNITIVE);
    mesh_participant_config_t config;
    mesh_participant_config_init(&config);
    config.module_name = "emotion_recognition_substrate_bridge";
    config.type = MESH_PARTICIPANT_MODULE;
    config.home_channel = iface.home_channel;
    nimcp_error_t err = mesh_participant_register(registry, &iface, &config, &g_emotion_recognition_substrate_bridge_mesh_id);
    if (err == NIMCP_SUCCESS) g_emotion_recognition_substrate_bridge_mesh_registry = registry;
    return err;
}

void emotion_recognition_substrate_bridge_mesh_unregister(void) {
    if (g_emotion_recognition_substrate_bridge_mesh_registry && g_emotion_recognition_substrate_bridge_mesh_id != 0) {
        mesh_participant_unregister(g_emotion_recognition_substrate_bridge_mesh_registry, g_emotion_recognition_substrate_bridge_mesh_id);
        g_emotion_recognition_substrate_bridge_mesh_id = 0;
        g_emotion_recognition_substrate_bridge_mesh_registry = NULL;
    }
}
...
int emotion_recognition_substrate_bridge_update(emotion_recognition_substrate_bridge_t* bridge) {
    if (!bridge || !bridge->substrate) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "emotion_recognition_substrate_bridge_update: required parameter is NULL (bridge, bridge->substrate)");
        return -1;
    }
    /* Phase 8: Heartbeat at operation start */
    ...
    if (substrate_get_metabolic_state(bridge->substrate, &metabolic) != 0) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_INVALID_PARAM, "emotion_recognition_substrate_bridge_update: validation failed");
        return -1;
    }
    ...
    if (bridge->config.enable_atp_modulation) {
        ...
        bridge->effects.subtle_sensitivity = nimcp_clampf(atp * 1.1f * bridge->config.atp_sensitivity, min_cap, 1.0f);
    }
    /* Low fatigue enables detection speed and context integration */
    if (bridge->config.enable_fatigue_modulation) {
        bridge->effects.detection_speed = nimcp_clampf(metabolic_cap * bridge->config.fatigue_sensitivity, min_cap, 1.0f);
        bridge->effects.context_integration = nimcp_clampf(metabolic_cap * 0.9f * bridge->config.fatigue_sensitivity, min_cap, 1.0f);
    }
...
int emotion_recognition_substrate_bridge_apply_effects(emotion_recognition_substrate_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "emotion_recognition_substrate_bridge_apply_effects: bridge is NULL");
        return -1;
    }
...
int emotion_recognition_substrate_bridge_register_bio_async(emotion_recognition_substrate_bridge_t* bridge, bio_router_t* router) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "emotion_recognition_substrate_bridge_register_bio_async: bridge is NULL");
        return -1;
    }
    /* Phase 8: Heartbeat at operation start */
