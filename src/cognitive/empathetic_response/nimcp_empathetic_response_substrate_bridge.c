...
static mesh_participant_id_t g_empathetic_response_substrate_bridge_mesh_id = 0;
static mesh_participant_registry_t* g_empathetic_response_substrate_bridge_mesh_registry = NULL;

nimcp_error_t empathetic_response_substrate_bridge_mesh_register(mesh_participant_registry_t* registry) {
    if (!registry) return NIMCP_ERROR_NULL_POINTER;
    if (g_empathetic_response_substrate_bridge_mesh_id != 0) return NIMCP_SUCCESS;
    mesh_participant_interface_t iface;
    mesh_participant_interface_init(&iface);
    strncpy(iface.module_name, "empathetic_response_substrate_bridge", MESH_MAX_NAME_LEN - 1);
    iface.type = MESH_PARTICIPANT_MODULE;
    iface.home_channel = mesh_adapter_get_default_channel(MESH_ADAPTER_CATEGORY_COGNITIVE);
    mesh_participant_config_t config;
    mesh_participant_config_init(&config);
    config.module_name = "empathetic_response_substrate_bridge";
    config.type = MESH_PARTICIPANT_MODULE;
    config.home_channel = iface.home_channel;
    nimcp_error_t err = mesh_participant_register(registry, &iface, &config, &g_empathetic_response_substrate_bridge_mesh_id);
    if (err == NIMCP_SUCCESS) g_empathetic_response_substrate_bridge_mesh_registry = registry;
    return err;
}

void empathetic_response_substrate_bridge_mesh_unregister(void) {
    if (g_empathetic_response_substrate_bridge_mesh_registry && g_empathetic_response_substrate_bridge_mesh_id != 0) {
        mesh_participant_unregister(g_empathetic_response_substrate_bridge_mesh_registry, g_empathetic_response_substrate_bridge_mesh_id);
        g_empathetic_response_substrate_bridge_mesh_id = 0;
        g_empathetic_response_substrate_bridge_mesh_registry = NULL;
    }
}


/** @brief Send heartbeat from empathetic_response_substrate_bridge module (instance-level) */
static inline void empathetic_response_substrate_bridge_heartbeat_instance(
    nimcp_health_agent_t* instance_agent, const char* operation, float progress)
{
    if (g_empathetic_response_substrate_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_empathetic_response_substrate_bridge_health_agent, operation, progress);
    }
    if (instance_agent && instance_agent != g_empathetic_response_substrate_bridge_health_agent) {
...
int empathetic_response_substrate_bridge_update(empathetic_response_substrate_bridge_t* bridge) {
    if (!bridge || !bridge->substrate) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "empathetic_response_substrate_bridge_update: required parameter is NULL (bridge, bridge->substrate)");
        return -1;
    }
    ...
    if (substrate_get_metabolic_state(bridge->substrate, &metabolic) != 0) {
        ...
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_INVALID_PARAM, "empathetic_response_substrate_bridge_update: validation failed");
        return -1;
    }
...
int empathetic_response_substrate_bridge_get_effects(const empathetic_response_substrate_bridge_t* bridge, empathetic_response_substrate_effects_t* effects) {
    if (!bridge || !effects) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "empathetic_response_substrate_bridge_get_effects: required parameter is NULL (bridge, effects)");
        return -1;
    }
...
int empathetic_response_substrate_bridge_apply_effects(empathetic_response_substrate_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "empathetic_response_substrate_bridge_apply_effects: bridge is NULL");
        return -1;
    }
    ...
    bio_msg_init_header(&msg.header, BIO_MSG_SUBSTRATE_MODULATION,
                        BIO_MODULE_SUBSTRATE_EMPATHETIC_RESPONSE, 0, sizeof(msg));
    msg.header.channel = BIO_CHANNEL_SEROTONIN;  /* Empathy uses serotonin */

    msg.bridge_module_id = BIO_MODULE_SUBSTRATE_EMPATHETIC_RESPONSE;
    msg.processing_capacity = bridge->effects.empathic_accuracy;
    msg.overall_capacity = bridge->effects.overall_capacity;
    msg.effect_values[0] = bridge->effects.empathic_accuracy;
    msg.effect_values[1] = bridge->effects.response_depth;
    msg.effect_values[2] = bridge->effects.perspective_taking;
    msg.effect_values[3] = bridge->effects.compassion_endurance;
    ...
    if (delta < -0.1f || delta > 0.1f) {
        ...
        bio_msg_init_header(&update_msg.header, BIO_MSG_SUBSTRATE_CAPACITY_UPDATE,
                            BIO_MODULE_SUBSTRATE_EMPATHETIC_RESPONSE, 0, sizeof(update_msg));
        update_msg.bridge_module_id = BIO_MODULE_SUBSTRATE_EMPATHETIC_RESPONSE;
    ...
    if (msg.critical_low) {
        ...
        bio_msg_init_header(&alert.header, BIO_MSG_SUBSTRATE_ATP_CRITICAL,
                            BIO_MODULE_SUBSTRATE_EMPATHETIC_RESPONSE, 0, sizeof(alert));
        alert.header.channel = BIO_CHANNEL_NOREPINEPHRINE;
        alert.bridge_module_id = BIO_MODULE_SUBSTRATE_EMPATHETIC_RESPONSE;
...
int empathetic_response_substrate_bridge_register_bio_async(empathetic_response_substrate_bridge_t* bridge, bio_router_t* router) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "empathetic_response_substrate_bridge_register_bio_async: bridge is NULL");
        return -1;
    }
    ...
    bio_module_info_t info = {
        .module_id = BIO_MODULE_SUBSTRATE_EMPATHETIC_RESPONSE,
        .module_name = "empathetic_response_substrate_bridge",
        .inbox_capacity = 16,
        .user_data = bridge
    };
...
int empathetic_response_substrate_bridge_query_self_knowledge(kg_reader_t* kg) {
    ...
    kg_relation_list_t* connections = kg_reader_get_relations_from(kg, "Empathetic_Response_Substrate_Bridge");
    if (connections) {
        kg_relation_list_destroy(connections);
    }

    kg_relation_list_t* incoming = kg_reader_get_relations_to(kg, "Empathetic_Response_Substrate_Bridge");
    if (incoming) {
