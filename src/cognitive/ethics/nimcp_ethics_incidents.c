...
NIMCP_DECLARE_HEALTH_AGENT_ATOMIC(ethics_incidents)
//=============================================================================
// Mesh Participant Registration
//=============================================================================

static mesh_participant_id_t g_ethics_incidents_mesh_id = 0;
static mesh_participant_registry_t* g_ethics_incidents_mesh_registry = NULL;

nimcp_error_t ethics_incidents_mesh_register(mesh_participant_registry_t* registry) {
    if (!registry) return NIMCP_ERROR_NULL_POINTER;
    if (g_ethics_incidents_mesh_id != 0) return NIMCP_SUCCESS;
    mesh_participant_interface_t iface;
    mesh_participant_interface_init(&iface);
    strncpy(iface.module_name, "ethics_incidents", MESH_MAX_NAME_LEN - 1);
    iface.type = MESH_PARTICIPANT_MODULE;
    iface.home_channel = mesh_adapter_get_default_channel(MESH_ADAPTER_CATEGORY_COGNITIVE);
    mesh_participant_config_t config;
    mesh_participant_config_init(&config);
    config.module_name = "ethics_incidents";
    config.type = MESH_PARTICIPANT_MODULE;
    config.home_channel = iface.home_channel;
    nimcp_error_t err = mesh_participant_register(registry, &iface, &config, &g_ethics_incidents_mesh_id);
    if (err == NIMCP_SUCCESS) g_ethics_incidents_mesh_registry = registry;
    return err;
}

void ethics_incidents_mesh_unregister(void) {
    if (g_ethics_incidents_mesh_registry && g_ethics_incidents_mesh_id != 0) {
        mesh_participant_unregister(g_ethics_incidents_mesh_registry, g_ethics_incidents_mesh_id);
        g_ethics_incidents_mesh_id = 0;
        g_ethics_incidents_mesh_registry = NULL;
    }
}


/** @brief Send heartbeat (instance-level) */
static inline void ethics_incidents_heartbeat_instance(
    nimcp_health_agent_t* instance_agent, const char* operation, float progress)
{
    if (g_ethics_incidents_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_ethics_incidents_health_agent, operation, progress);
    }
    if (instance_agent && instance_agent != g_ethics_incidents_health_agent) {
