...
static mesh_participant_id_t g_fractal_cognitive_substrate_bridge_mesh_id = 0;
static mesh_participant_registry_t* g_fractal_cognitive_substrate_bridge_mesh_registry = NULL;

nimcp_error_t fractal_cognitive_substrate_bridge_mesh_register(mesh_participant_registry_t* registry) {
    if (!registry) return NIMCP_ERROR_NULL_POINTER;
    if (g_fractal_cognitive_substrate_bridge_mesh_id != 0) return NIMCP_SUCCESS;
    mesh_participant_interface_t iface;
    mesh_participant_interface_init(&iface);
    strncpy(iface.module_name, "fractal_cognitive_substrate_bridge", MESH_MAX_NAME_LEN - 1);
    iface.type = MESH_PARTICIPANT_MODULE;
    iface.home_channel = mesh_adapter_get_default_channel(MESH_ADAPTER_CATEGORY_COGNITIVE);
    mesh_participant_config_t config;
    mesh_participant_config_init(&config);
    config.module_name = "fractal_cognitive_substrate_bridge";
    config.type = MESH_PARTICIPANT_MODULE;
    config.home_channel = iface.home_channel;
    nimcp_error_t err = mesh_participant_register(registry, &iface, &config, &g_fractal_cognitive_substrate_bridge_mesh_id);
    if (err == NIMCP_SUCCESS) g_fractal_cognitive_substrate_bridge_mesh_registry = registry;
    return err;
}

void fractal_cognitive_substrate_bridge_mesh_unregister(void) {
    if (g_fractal_cognitive_substrate_bridge_mesh_registry && g_fractal_cognitive_substrate_bridge_mesh_id != 0) {
        mesh_participant_unregister(g_fractal_cognitive_substrate_bridge_mesh_registry, g_fractal_cognitive_substrate_bridge_mesh_id);
        g_fractal_cognitive_substrate_bridge_mesh_id = 0;
        g_fractal_cognitive_substrate_bridge_mesh_registry = NULL;
    }
}
...
fractal_cognitive_substrate_config_t fractal_cognitive_substrate_default_config(void) {
    ...
    fractal_cognitive_substrate_bridge_heartbeat("fractal_cogn_fractal_cognitive_su", 0.0f);
...
fractal_cognitive_substrate_bridge_t* fractal_cognitive_substrate_bridge_create(void* fractal_cognitive, neural_substrate_t* substrate, const fractal_cognitive_substrate_config_t* config) {
    ...
    /* Phase 8: Heartbeat at operation start */
    fractal_cognitive_substrate_bridge_heartbeat("fractal_cogn_create", 0.0f);
...
int fractal_cognitive_substrate_bridge_apply_effects(fractal_cognitive_substrate_bridge_t* bridge) {
    ...
    /* Phase 8: Heartbeat at operation start */
    fractal_cognitive_substrate_bridge_heartbeat("fractal_cogn_apply_effects", 0.0f);
...
int fractal_cognitive_substrate_bridge_query_self_knowledge(kg_reader_t* kg) {
    ...
    /* Phase 8: Heartbeat at operation start */
    fractal_cognitive_substrate_bridge_heartbeat("fractal_cogn_query_self_knowledge", 0.0f);
    ...
    if (self) {
        for (uint32_t i = 0; i < self->num_observations; i++) {
            ...
            if ((i & 0xFF) == 0 && self->num_observations > 256) {
                fractal_cognitive_substrate_bridge_heartbeat("fractal_cogn_loop",
