/**
 * @file nimcp_game_theory_plasticity_bridge.c
 * @brief Game Theory - Plasticity Bidirectional Integration Bridge Implementation
 * @version 1.0.0
 * @date 2026-01-06
 */

#include "cognitive/game_theory/nimcp_game_theory_plasticity_bridge.h"
...
struct game_theory_plasticity_bridge {
    bridge_base_t base;
    nimcp_health_agent_t* health_agent;  /**< Phase 8: instance-level health agent */
    game_theory_plasticity_config_t config;

    /* State */
    game_theory_plasticity_state_t state;
    ...
    /* Strategy state */
    game_theory_strategy_state_t strategy_state;
    ...
    /* Callbacks */
    game_theory_plasticity_learn_callback_t learn_callback;
    void* learn_callback_data;
    game_theory_plasticity_strategy_callback_t strategy_callback;
...
static synapse_entry_t* find_synapse(game_theory_plasticity_bridge_t* bridge, uint32_t synapse_id) {
    for (uint32_t i = 0; i < bridge->max_synapses; i++) {
        /* Phase 8: Loop progress heartbeat */
        if ((i & 0xFF) == 0 && bridge->max_synapses > 256) {
            game_theory_plasticity_bridge_heartbeat("game_theory__loop",
...
static bool is_protected_type(game_theory_synapse_type_t type) {
    return type == GT_SYNAPSE_STRATEGY ||
           type == GT_SYNAPSE_EQUILIBRIUM;
}
...
game_theory_plasticity_config_t game_theory_plasticity_config_default(void) {
    ...
    game_theory_plasticity_config_t config = {
        ...
        .target_cooperation = 0.5f,

        .payoff_learning_boost = 1.5f,
        .equilibrium_learning_boost = 1.3f,
        .opponent_prediction_modulation = 1.2f,

        .weight_min = 0.0f,
        .weight_max = 1.0f,

        .protect_strategy_weights = true,
        .protect_equilibrium_weights = true,
        .protection_strength = 1.0f,

        .max_synapses = GAME_THEORY_PLASTICITY_MAX_SYNAPSES,
...
game_theory_plasticity_bridge_t* game_theory_plasticity_create(
    ...
    game_theory_plasticity_bridge_t* bridge = nimcp_calloc(1, sizeof(game_theory_plasticity_bridge_t));
    if (!bridge) {
    ...
    if (config) {
        ...
        bridge->config = game_theory_plasticity_config_default();
    }
...
int game_theory_plasticity_reset(game_theory_plasticity_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "game_theory_plasticity_reset: bridge is NULL");
        return -1;
    }
...
int game_theory_plasticity_register_synapse(
    ...
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "game_theory_plasticity_register_synapse: bridge is NULL");
        return -1;
    }
    ...
    if (find_synapse(bridge, synapse_id)) {
        nimcp_mutex_unlock(bridge->base.mutex);
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_INVALID_PARAM, "game_theory_plasticity_register_synapse: validation failed");
        return -1;
    }
    ...
    /* Auto-protect strategy and equilibrium synapses */
    slot->synapse.is_protected = is_protected_type(type) &&
        (bridge->config.protect_strategy_weights || bridge->config.protect_equilibrium_weights);
...
int game_theory_plasticity_unregister_synapse(
    ...
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "game_theory_plasticity_unregister_synapse: bridge is NULL");
        return -1;
    }
...
int game_theory_plasticity_get_synapse(
    ...
    if (!bridge || !synapse) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "game_theory_plasticity_get_synapse: required parameter is NULL (bridge, synapse)");
        return -1;
    }
...
int game_theory_plasticity_protect_synapse(
    ...
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "game_theory_plasticity_protect_synapse: bridge is NULL");
        return -1;
    }
...
int game_theory_plasticity_learn(
    ...
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "game_theory_plasticity_learn: bridge is NULL");
        return -1;
    }
    ...
    nimcp_mutex_lock(bridge->base.mutex);
    bridge->state = GT_PLASTICITY_STATE_LEARNING;
    ...
    switch (event) {
        case GT_LEARN_COOPERATION_REWARDED:
            weight_change = lr * bridge->config.payoff_learning_boost;
            bridge->stats.cooperation_rewarded_events++;
            break;

        case GT_LEARN_COOPERATION_PUNISHED:
            weight_change = -lr * 0.5f;
            break;

        case GT_LEARN_DEFECTION_REWARDED:
            weight_change = lr * 0.8f;
            bridge->stats.defection_rewarded_events++;
            break;

        case GT_LEARN_DEFECTION_PUNISHED:
            weight_change = -lr * 0.6f;
            break;

        case GT_LEARN_EQUILIBRIUM_REACHED:
            weight_change = lr * bridge->config.equilibrium_learning_boost;
            bridge->stats.equilibrium_events++;
            break;

        case GT_LEARN_EQUILIBRIUM_DEVIATED:
            weight_change = -lr * 0.3f;
            break;

        case GT_LEARN_OPPONENT_PREDICTED:
            weight_change = lr * bridge->config.opponent_prediction_modulation;
            bridge->stats.opponent_prediction_events++;
            break;

        case GT_LEARN_OPPONENT_SURPRISED:
            weight_change = -lr * 0.2f;
            break;

        case GT_LEARN_PAYOFF_EXCEEDED:
            weight_change = lr * 1.0f;
            break;

        case GT_LEARN_PAYOFF_DISAPPOINTED:
...
int game_theory_plasticity_apply_reward(
    ...
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "game_theory_plasticity_apply_reward: bridge is NULL");
        return -1;
    }
...
int game_theory_plasticity_update_bcm(
    ...
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "game_theory_plasticity_update_bcm: bridge is NULL");
        return -1;
    }
    if (dt_ms <= 0.0f) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_INVALID_PARAM, "game_theory_plasticity_update_bcm: validation failed");
        return -1;
    }
    ...
    nimcp_mutex_lock(bridge->base.mutex);
    bridge->state = GT_PLASTICITY_STATE_UPDATING;
...
int game_theory_plasticity_homeostatic_update(
    ...
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "game_theory_plasticity_homeostatic_update: bridge is NULL");
        return -1;
    }
    ...
    /* Calculate mean cooperation */
    float mean_cooperation = 0.0f;
    uint32_t cooperation_count = 0;
    for (uint32_t i = 0; i < bridge->max_synapses; i++) {
        /* Phase 8: Loop progress heartbeat */
        if ((i & 0xFF) == 0 && bridge->max_synapses > 256) {
            game_theory_plasticity_bridge_heartbeat("game_theory__loop",
        ...
        if (bridge->synapses[i].in_use &&
            bridge->synapses[i].synapse.type == GT_SYNAPSE_COOPERATION) {
            mean_cooperation += bridge->synapses[i].synapse.weight;
            cooperation_count++;
        }
    }
    if (cooperation_count > 0) {
        mean_cooperation /= cooperation_count;
    }

    /* Scale non-protected synapses toward target */
    float target = bridge->config.target_cooperation;
    float scale_factor = 1.0f;
    if (mean_cooperation > 0.0f) {
        scale_factor = target / mean_cooperation;
...
int game_theory_plasticity_update_traces(
    ...
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "game_theory_plasticity_update_traces: bridge is NULL");
        return -1;
    }
...
int game_theory_plasticity_consolidate(game_theory_plasticity_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "game_theory_plasticity_consolidate: bridge is NULL");
        return -1;
    }
    ...
    nimcp_mutex_lock(bridge->base.mutex);
    bridge->state = GT_PLASTICITY_STATE_CONSOLIDATING;
    ...
    float defection_sum = 0.0f, defection_count = 0;
    float payoff_sum = 0.0f, payoff_count = 0;
    float opponent_sum = 0.0f, opponent_count = 0;
    float equilibrium_sum = 0.0f, equilibrium_count = 0;
    ...
    for (uint32_t i = 0; i < bridge->max_synapses; i++) {
        ...
        if (bridge->synapses[i].in_use) {
            switch (bridge->synapses[i].synapse.type) {
                case GT_SYNAPSE_COOPERATION:
                    cooperation_sum += bridge->synapses[i].synapse.weight;
                    cooperation_count++;
                    break;
                case GT_SYNAPSE_DEFECTION:
                    defection_sum += bridge->synapses[i].synapse.weight;
                    defection_count++;
                    break;
                case GT_SYNAPSE_PAYOFF:
                    payoff_sum += bridge->synapses[i].synapse.weight;
                    payoff_count++;
                    break;
                case GT_SYNAPSE_OPPONENT:
                    opponent_sum += bridge->synapses[i].synapse.weight;
                    opponent_count++;
                    break;
                case GT_SYNAPSE_EQUILIBRIUM:
                    equilibrium_sum += bridge->synapses[i].synapse.weight;
                    equilibrium_count++;
                    break;
                default:
                    break;
    ...
    if (cooperation_count > 0) {
        bridge->strategy_state.cooperation_tendency = cooperation_sum / cooperation_count;
    }
    if (defection_count > 0) {
        bridge->strategy_state.defection_tendency = defection_sum / defection_count;
    }
    if (payoff_count > 0) {
        bridge->strategy_state.payoff_sensitivity = payoff_sum / payoff_count * 2.0f;
    }
    if (opponent_count > 0) {
        bridge->strategy_state.opponent_model_accuracy = opponent_sum / opponent_count;
    }
    if (equilibrium_count > 0) {
        bridge->strategy_state.equilibrium_tracking = equilibrium_sum / equilibrium_count;
    }
...
int game_theory_plasticity_get_strategy_state(
    ...
    if (!bridge || !state) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "game_theory_plasticity_get_strategy_state: required parameter is NULL (bridge, state)");
        return -1;
    }
...
int game_theory_plasticity_get_state(
    ...
    if (!bridge || !state) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "game_theory_plasticity_get_state: required parameter is NULL (bridge, state)");
        return -1;
    }
...
int game_theory_plasticity_get_stats(
    ...
    if (!bridge || !stats) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "game_theory_plasticity_get_stats: required parameter is NULL (bridge, stats)");
        return -1;
    }
...
int game_theory_plasticity_reset_stats(game_theory_plasticity_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "game_theory_plasticity_reset_stats: bridge is NULL");
        return -1;
    }
    ...
    nimcp_mutex_lock(bridge->base.mutex);
    memset(&bridge->stats, 0, sizeof(game_theory_plasticity_stats_t));
    nimcp_mutex_unlock(bridge->base.mutex);
...
int game_theory_plasticity_register_learn_callback(
    ...
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "game_theory_plasticity_register_learn_callback: bridge is NULL");
        return -1;
    }
...
int game_theory_plasticity_register_strategy_callback(
    ...
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "game_theory_plasticity_register_strategy_callback: bridge is NULL");
        return -1;
    }
...
int game_theory_plasticity_bio_async_connect(game_theory_plasticity_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "game_theory_plasticity_bio_async_connect: bridge is NULL");
        return -1;
    }
...
int game_theory_plasticity_bio_async_disconnect(game_theory_plasticity_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "game_theory_plasticity_bio_async_disconnect: bridge is NULL");
        return -1;
    }
...
void game_theory_plasticity_bridge_set_instance_health_agent(game_theory_plasticity_bridge_t* bridge, nimcp_health_agent_t* agent) {
    if (!bridge) {
        NIMCP_THROW(NIMCP_ERROR_NULL_POINTER,
                    "game_theory_plasticity_bridge_set_instance_health_agent: NULL bridge");
        return;
    }
...
int game_theory_plasticity_bridge_training_begin(game_theory_plasticity_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "game_theory_plasticity_bridge_training_begin: NULL argument");
        return -1;
    }
    game_theory_plasticity_bridge_heartbeat_instance(bridge->health_agent, "game_theory_plasticity_bridge_training_begin", 0.0f);
    return 0;
}

int game_theory_plasticity_bridge_training_end(game_theory_plasticity_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "game_theory_plasticity_bridge_training_end: NULL argument");
        return -1;
    }
    game_theory_plasticity_bridge_heartbeat_instance(bridge->health_agent, "game_theory_plasticity_bridge_training_end", 1.0f);
    return 0;
}

int game_theory_plasticity_bridge_training_step(game_theory_plasticity_bridge_t* bridge, float progress) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "game_theory_plasticity_bridge_training_step: NULL argument");
        return -1;
    }
    if (progress < 0.0f) progress = 0.0f;
    if (progress > 1.0f) progress = 1.0f;
    game_theory_plasticity_bridge_heartbeat_instance(bridge->health_agent, "game_theory_plasticity_bridge_training_step", progress);
    return 0;
}
