...
int attention_immune_apply_cytokine_effects(attention_immune_bridge_t* bridge) {
    ...
    /* Compute cytokine-induced deficits (simplified - would query actual cytokine levels) */
    float inflammation_proxy = stats.inflammation_sites > 0 ? 0.5f : 0.0f;

    bridge->cytokine_effects.il1_attention_deficit =
        inflammation_proxy * CYTOKINE_IL1_ATTENTION_IMPACT;
    bridge->cytokine_effects.il6_attention_deficit =
        inflammation_proxy * CYTOKINE_IL6_ATTENTION_IMPACT;
    bridge->cytokine_effects.tnf_attention_deficit =
        inflammation_proxy * CYTOKINE_TNF_ATTENTION_IMPACT;
    bridge->cytokine_effects.ifn_gamma_attention_deficit =
        inflammation_proxy * CYTOKINE_IFN_GAMMA_ATTENTION_IMPACT;
