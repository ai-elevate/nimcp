...
NIMCP_DECLARE_HEALTH_AGENT_ATOMIC(brain_immune_tick)
//=============================================================================
// Mesh Participant Registration
//=============================================================================

static mesh_participant_id_t g_brain_immune_tick_mesh_id = 0;
static mesh_participant_registry_t* g_brain_immune_tick_mesh_registry = NULL;

nimcp_error_t brain_immune_tick_mesh_register(mesh_participant_registry_t* registry) {
    if (!registry) return NIMCP_ERROR_NULL_POINTER;
    if (g_brain_immune_tick_mesh_id != 0) return NIMCP_SUCCESS;
    mesh_participant_interface_t iface;
    mesh_participant_interface_init(&iface);
    strncpy(iface.module_name, "brain_immune_tick", MESH_MAX_NAME_LEN - 1);
    iface.type = MESH_PARTICIPANT_MODULE;
    iface.home_channel = mesh_adapter_get_default_channel(MESH_ADAPTER_CATEGORY_SECURITY);
    mesh_participant_config_t config;
    mesh_participant_config_init(&config);
    config.module_name = "brain_immune_tick";
    config.type = MESH_PARTICIPANT_MODULE;
    config.home_channel = iface.home_channel;
    nimcp_error_t err = mesh_participant_register(registry, &iface, &config, &g_brain_immune_tick_mesh_id);
    if (err == NIMCP_SUCCESS) g_brain_immune_tick_mesh_registry = registry;
    return err;
}

void brain_immune_tick_mesh_unregister(void) {
    if (g_brain_immune_tick_mesh_registry && g_brain_immune_tick_mesh_id != 0) {
        mesh_participant_unregister(g_brain_immune_tick_mesh_registry, g_brain_immune_tick_mesh_id);
        g_brain_immune_tick_mesh_id = 0;
        g_brain_immune_tick_mesh_registry = NULL;
    }
}


/** @brief Send heartbeat from brain_immune_tick module (instance-level) */
static inline void brain_immune_tick_heartbeat_instance(
    nimcp_health_agent_t* instance_agent, const char* operation, float progress)
{
    ...
    if (instance_agent && instance_agent != g_brain_immune_tick_health_agent) {
...
int brain_immune_tick_training_begin(void* instance) {
    if (!instance) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "brain_immune_tick_training_begin: NULL argument");
        return -1;
    }
    brain_immune_tick_heartbeat_instance(NULL, "brain_immune_tick_training_begin", 0.0f);
    (void)(epitope_pattern_t*)instance; /* Module state available for reset */
    return 0;
}

int brain_immune_tick_training_end(void* instance) {
    if (!instance) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "brain_immune_tick_training_end: NULL argument");
        return -1;
    }
    brain_immune_tick_heartbeat_instance(NULL, "brain_immune_tick_training_end", 1.0f);
    (void)(epitope_pattern_t*)instance; /* Module state available for finalization */
    return 0;
}

int brain_immune_tick_training_step(void* instance, float progress) {
    if (!instance) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "brain_immune_tick_training_step: NULL argument");
        return -1;
    }
    if (progress < 0.0f) progress = 0.0f;
    if (progress > 1.0f) progress = 1.0f;
    brain_immune_tick_heartbeat_instance(NULL, "brain_immune_tick_training_step", progress);
    (void)(epitope_pattern_t*)instance; /* Module state available for step adaptation */
    return 0;
}
