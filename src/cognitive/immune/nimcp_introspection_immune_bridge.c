...
introspection_immune_bridge_t* introspection_immune_bridge_create(
    ...
    if (!immune_system || !introspection_context) {
        nimcp_log(NIMCP_LOG_ERROR, "introspection_immune_bridge",
    ...
    if (!bridge) {
        nimcp_log(NIMCP_LOG_ERROR, "introspection_immune_bridge", "Allocation failed");
        return NULL;
    }
    ...
    nimcp_log(NIMCP_LOG_INFO, "introspection_immune_bridge", "Bridge created successfully");
    return bridge;
}
...
void introspection_immune_bridge_destroy(introspection_immune_bridge_t* bridge) {
    ...
    nimcp_free(bridge);
    nimcp_log(NIMCP_LOG_INFO, "introspection_immune_bridge", "Bridge destroyed");
}
