...
knowledge_immune_bridge_t* knowledge_immune_bridge_create(
    ...
    if (!immune_system || !knowledge_system) {
        nimcp_log(NIMCP_LOG_ERROR, "knowledge_immune_bridge",
    ...
    if (!bridge) {
        nimcp_log(NIMCP_LOG_ERROR, "knowledge_immune_bridge", "Allocation failed");
        return NULL;
    }
    ...
    nimcp_log(NIMCP_LOG_INFO, "knowledge_immune_bridge", "Bridge created successfully");
    return bridge;
}
...
void knowledge_immune_bridge_destroy(knowledge_immune_bridge_t* bridge) {
    ...
    nimcp_free(bridge);
    nimcp_log(NIMCP_LOG_INFO, "knowledge_immune_bridge", "Bridge destroyed");
}
