...
static nimcp_health_agent_t* g_convergence_health_agent = NULL;

void inner_dialogue_convergence_set_health_agent_global(nimcp_health_agent_t* agent) {
    g_convergence_health_agent = agent;
}

static inline void convergence_heartbeat(const char* op, float progress) {
    if (g_convergence_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_convergence_health_agent, op, progress);
    }
}
