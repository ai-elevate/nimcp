...
NIMCP_DECLARE_HEALTH_AGENT_ATOMIC(joy_substrate_bridge)
//=============================================================================
// Mesh Participant Registration
//=============================================================================

static mesh_participant_id_t g_joy_substrate_bridge_mesh_id = 0;
static mesh_participant_registry_t* g_joy_substrate_bridge_mesh_registry = NULL;

nimcp_error_t joy_substrate_bridge_mesh_register(mesh_participant_registry_t* registry) {
    if (!registry) return NIMCP_ERROR_NULL_POINTER;
    if (g_joy_substrate_bridge_mesh_id != 0) return NIMCP_SUCCESS;
    mesh_participant_interface_t iface;
    mesh_participant_interface_init(&iface);
    strncpy(iface.module_name, "joy_substrate_bridge", MESH_MAX_NAME_LEN - 1);
    iface.type = MESH_PARTICIPANT_MODULE;
    iface.home_channel = mesh_adapter_get_default_channel(MESH_ADAPTER_CATEGORY_COGNITIVE);
    mesh_participant_config_t config;
    mesh_participant_config_init(&config);
    config.module_name = "joy_substrate_bridge";
    config.type = MESH_PARTICIPANT_MODULE;
    config.home_channel = iface.home_channel;
    nimcp_error_t err = mesh_participant_register(registry, &iface, &config, &g_joy_substrate_bridge_mesh_id);
    if (err == NIMCP_SUCCESS) g_joy_substrate_bridge_mesh_registry = registry;
    return err;
}

void joy_substrate_bridge_mesh_unregister(void) {
    if (g_joy_substrate_bridge_mesh_registry && g_joy_substrate_bridge_mesh_id != 0) {
        mesh_participant_unregister(g_joy_substrate_bridge_mesh_registry, g_joy_substrate_bridge_mesh_id);
        g_joy_substrate_bridge_mesh_id = 0;
        g_joy_substrate_bridge_mesh_registry = NULL;
    }
}


/** @brief Send heartbeat from joy_substrate_bridge module (instance-level) */
static inline void joy_substrate_bridge_heartbeat_instance(
    nimcp_health_agent_t* instance_agent, const char* operation, float progress)
{
    ...
    if (instance_agent && instance_agent != g_joy_substrate_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(instance_agent, operation, progress);
    }
}

#define LOG_MODULE "JOY_SUBSTRATE_BRIDGE"
...
joy_substrate_config_t joy_substrate_default_config(void) {
    ...
    joy_substrate_bridge_heartbeat("joy_substrat_joy_substrate_defaul", 0.0f);
...
joy_substrate_bridge_t* joy_substrate_bridge_create(void* joy, neural_substrate_t* substrate, const joy_substrate_config_t* config) {
    ...
    /* Phase 8: Heartbeat at operation start */
    joy_substrate_bridge_heartbeat("joy_substrat_create", 0.0f);
...
void joy_substrate_bridge_destroy(joy_substrate_bridge_t* bridge) {
    if (!bridge) return;
    NIMCP_LOGGING_DEBUG("Destroying %s bridge", "joy_substrate");
    /* Phase 8: Heartbeat at operation start */
    joy_substrate_bridge_heartbeat("joy_substrat_destroy", 0.0f);
...
int joy_substrate_bridge_update(joy_substrate_bridge_t* bridge) {
    if (!bridge || !bridge->substrate) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "joy_substrate_bridge_update: required parameter is NULL (bridge, bridge->substrate)");
        return -1;
    }

    /* Phase 8: Heartbeat at operation start */
    joy_substrate_bridge_heartbeat("joy_substrat_update", 0.0f);


    substrate_metabolic_state_t metabolic;
    if (substrate_get_metabolic_state(bridge->substrate, &metabolic) != 0) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_INVALID_PARAM, "joy_substrate_bridge_update: validation failed");
        return -1;
    }
...
int joy_substrate_bridge_apply_effects(joy_substrate_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "joy_substrate_bridge_apply_effects: bridge is NULL");
        return -1;
    }
    ...
    /* Phase 8: Heartbeat at operation start */
    joy_substrate_bridge_heartbeat("joy_substrat_apply_effects", 0.0f);
...
int joy_substrate_bridge_register_bio_async(joy_substrate_bridge_t* bridge, bio_router_t* router) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "joy_substrate_bridge_register_bio_async: bridge is NULL");
        return -1;
    }

    /* Phase 8: Heartbeat at operation start */
    joy_substrate_bridge_heartbeat("joy_substrat_register_bio_async", 0.0f);
...
int joy_substrate_bridge_query_self_knowledge(kg_reader_t* kg) {
    ...
    /* Phase 8: Heartbeat at operation start */
    joy_substrate_bridge_heartbeat("joy_substrat_query_self_knowledge", 0.0f);
    ...
    if (self) {
        for (uint32_t i = 0; i < self->num_observations; i++) {
            ...
            if ((i & 0xFF) == 0 && self->num_observations > 256) {
                joy_substrate_bridge_heartbeat("joy_substrat_loop",
