...
NIMCP_DECLARE_HEALTH_AGENT_ATOMIC(knowledge_snn_bridge)
//=============================================================================
// Mesh Participant Registration
//=============================================================================

static mesh_participant_id_t g_knowledge_snn_bridge_mesh_id = 0;
static mesh_participant_registry_t* g_knowledge_snn_bridge_mesh_registry = NULL;

nimcp_error_t knowledge_snn_bridge_mesh_register(mesh_participant_registry_t* registry) {
    if (!registry) return NIMCP_ERROR_NULL_POINTER;
    if (g_knowledge_snn_bridge_mesh_id != 0) return NIMCP_SUCCESS;
    mesh_participant_interface_t iface;
    mesh_participant_interface_init(&iface);
    strncpy(iface.module_name, "knowledge_snn_bridge", MESH_MAX_NAME_LEN - 1);
    iface.type = MESH_PARTICIPANT_MODULE;
    iface.home_channel = mesh_adapter_get_default_channel(MESH_ADAPTER_CATEGORY_COGNITIVE);
    mesh_participant_config_t config;
    mesh_participant_config_init(&config);
    config.module_name = "knowledge_snn_bridge";
    config.type = MESH_PARTICIPANT_MODULE;
    config.home_channel = iface.home_channel;
    nimcp_error_t err = mesh_participant_register(registry, &iface, &config, &g_knowledge_snn_bridge_mesh_id);
    if (err == NIMCP_SUCCESS) g_knowledge_snn_bridge_mesh_registry = registry;
    return err;
}

void knowledge_snn_bridge_mesh_unregister(void) {
    if (g_knowledge_snn_bridge_mesh_registry && g_knowledge_snn_bridge_mesh_id != 0) {
        mesh_participant_unregister(g_knowledge_snn_bridge_mesh_registry, g_knowledge_snn_bridge_mesh_id);
        g_knowledge_snn_bridge_mesh_id = 0;
        g_knowledge_snn_bridge_mesh_registry = NULL;
    }
}


static inline void knowledge_snn_bridge_heartbeat_instance(
    nimcp_health_agent_t* instance_agent, const char* operation, float progress)
{
    if (g_knowledge_snn_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_knowledge_snn_bridge_health_agent, operation, progress);
    }
    if (instance_agent && instance_agent != g_knowledge_snn_bridge_health_agent) {
...
knowledge_snn_bridge_t* knowledge_snn_create(const knowledge_snn_config_t* config) {
    ...
    if (!bridge->encoding_buffer || !bridge->output_buffer ||
        ...
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NO_MEMORY, "knowledge_snn_create: failed to allocate buffers");
...
void knowledge_snn_bridge_set_instance_health_agent(knowledge_snn_bridge_t* bridge, nimcp_health_agent_t* agent) {
    if (!bridge) {
        NIMCP_THROW(NIMCP_ERROR_NULL_POINTER,
                    "knowledge_snn_bridge_set_instance_health_agent: NULL bridge");
        return;
    }
...
int knowledge_snn_bridge_training_begin(knowledge_snn_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "knowledge_snn_bridge_training_begin: NULL argument");
        return -1;
    }
    knowledge_snn_bridge_heartbeat_instance(bridge->health_agent, "knowledge_snn_bridge_training_begin", 0.0f);
    return 0;
}

int knowledge_snn_bridge_training_end(knowledge_snn_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "knowledge_snn_bridge_training_end: NULL argument");
        return -1;
    }
    knowledge_snn_bridge_heartbeat_instance(bridge->health_agent, "knowledge_snn_bridge_training_end", 1.0f);
    return 0;
}

int knowledge_snn_bridge_training_step(knowledge_snn_bridge_t* bridge, float progress) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "knowledge_snn_bridge_training_step: NULL argument");
        return -1;
    }
    if (progress < 0.0f) progress = 0.0f;
    if (progress > 1.0f) progress = 1.0f;
    knowledge_snn_bridge_heartbeat_instance(bridge->health_agent, "knowledge_snn_bridge_training_step", progress);
    return 0;
}
