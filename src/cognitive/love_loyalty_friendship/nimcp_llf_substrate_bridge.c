...
llf_substrate_bridge_t* llf_substrate_bridge_create(void* llf, neural_substrate_t* substrate, const llf_substrate_config_t* config) {
    ...
    if (bridge_base_init(&bridge->base, 0, "llf_substrate") != 0) { nimcp_free(bridge); return NULL; }
...
int llf_substrate_bridge_update(llf_substrate_bridge_t* bridge) {
    if (!bridge || !bridge->substrate) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "llf_substrate_bridge_update: required parameter is NULL (bridge, bridge->substrate)");
        return -1;
    }
    /* Phase 8: Heartbeat at operation start */
    ...
    if (substrate_get_metabolic_state(bridge->substrate, &metabolic) != 0) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_INVALID_PARAM, "llf_substrate_bridge_update: validation failed");
        return -1;
    }
    ...
    if (bridge->config.enable_atp_modulation) {
        bridge->effects.attachment_strength = nimcp_clampf(atp * bridge->config.atp_sensitivity, min_cap, 1.0f);
        bridge->effects.trust_capacity = nimcp_clampf(atp * 1.05f * bridge->config.atp_sensitivity, min_cap, 1.0f);
    }
    /* Low fatigue enables social investment and loyalty */
    if (bridge->config.enable_fatigue_modulation) {
        bridge->effects.social_investment = nimcp_clampf(metabolic_cap * bridge->config.fatigue_sensitivity, min_cap, 1.0f);
        bridge->effects.loyalty_maintenance = nimcp_clampf(metabolic_cap * 0.9f * bridge->config.fatigue_sensitivity, min_cap, 1.0f);
    }
...
int llf_substrate_bridge_get_effects(const llf_substrate_bridge_t* bridge, llf_substrate_effects_t* effects) {
    if (!bridge || !effects) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "llf_substrate_bridge_get_effects: required parameter is NULL (bridge, effects)");
        return -1;
    }
    nimcp_platform_mutex_lock(bridge->base.mutex);
...
int llf_substrate_bridge_apply_effects(llf_substrate_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "llf_substrate_bridge_apply_effects: bridge is NULL");
        return -1;
    }
...
int llf_substrate_bridge_register_bio_async(llf_substrate_bridge_t* bridge, bio_router_t* router) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "llf_substrate_bridge_register_bio_async: bridge is NULL");
        return -1;
    }
    /* Phase 8: Heartbeat at operation start */
