...
NIMCP_DECLARE_HEALTH_AGENT_ATOMIC(pr_logging_bridge)
//=============================================================================
// Mesh Participant Registration
//=============================================================================

static mesh_participant_id_t g_pr_logging_bridge_mesh_id = 0;
static mesh_participant_registry_t* g_pr_logging_bridge_mesh_registry = NULL;

nimcp_error_t pr_logging_bridge_mesh_register(mesh_participant_registry_t* registry) {
    if (!registry) return NIMCP_ERROR_NULL_POINTER;
    if (g_pr_logging_bridge_mesh_id != 0) return NIMCP_SUCCESS;
    mesh_participant_interface_t iface;
    mesh_participant_interface_init(&iface);
    strncpy(iface.module_name, "pr_logging_bridge", MESH_MAX_NAME_LEN - 1);
    iface.type = MESH_PARTICIPANT_MODULE;
    iface.home_channel = mesh_adapter_get_default_channel(MESH_ADAPTER_CATEGORY_MEMORY);
    mesh_participant_config_t config;
    mesh_participant_config_init(&config);
    config.module_name = "pr_logging_bridge";
    config.type = MESH_PARTICIPANT_MODULE;
    config.home_channel = iface.home_channel;
    nimcp_error_t err = mesh_participant_register(registry, &iface, &config, &g_pr_logging_bridge_mesh_id);
    if (err == NIMCP_SUCCESS) g_pr_logging_bridge_mesh_registry = registry;
    return err;
}

void pr_logging_bridge_mesh_unregister(void) {
    if (g_pr_logging_bridge_mesh_registry && g_pr_logging_bridge_mesh_id != 0) {
        mesh_participant_unregister(g_pr_logging_bridge_mesh_registry, g_pr_logging_bridge_mesh_id);
        g_pr_logging_bridge_mesh_id = 0;
        g_pr_logging_bridge_mesh_registry = NULL;
    }
}
...
#include "utils/memory/nimcp_memory.h"
    typedef nimcp_mutex_t pr_log_mutex_t;
    #define PR_LOG_MUTEX_INIT(m) nimcp_mutex_init(&(m), NULL)
    #define PR_LOG_MUTEX_DESTROY(m) nimcp_mutex_destroy(&(m))
    #define PR_LOG_MUTEX_LOCK(m) nimcp_mutex_lock(&(m))
    #define PR_LOG_MUTEX_UNLOCK(m) nimcp_mutex_unlock(&(m))
...
NIMCP_EXPORT pr_logging_bridge_t pr_logging_bridge_create(
    ...
    /* Allocate bridge */
    pr_logging_bridge_t bridge = (pr_logging_bridge_t)nimcp_calloc(1, sizeof(*bridge));
    if (!bridge) {
    ...
    bridge->entries = (pr_log_entry_t*)nimcp_calloc(cfg.buffer_capacity,
...
int pr_logging_bridge_training_begin(pr_logging_bridge_t bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "pr_logging_bridge_training_begin: NULL argument");
        return -1;
    }
...
int pr_logging_bridge_training_end(pr_logging_bridge_t bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "pr_logging_bridge_training_end: NULL argument");
        return -1;
    }
...
int pr_logging_bridge_training_step(pr_logging_bridge_t bridge, float progress) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "pr_logging_bridge_training_step: NULL argument");
        return -1;
    }
