...
NIMCP_DECLARE_HEALTH_AGENT_ATOMIC(semantic_memory)
//=============================================================================
// Mesh Participant Registration
//=============================================================================

static mesh_participant_id_t g_semantic_memory_mesh_id = 0;
static mesh_participant_registry_t* g_semantic_memory_mesh_registry = NULL;

nimcp_error_t semantic_memory_mesh_register(mesh_participant_registry_t* registry) {
    if (!registry) return NIMCP_ERROR_NULL_POINTER;
    if (g_semantic_memory_mesh_id != 0) return NIMCP_SUCCESS;
    ...
    config.module_name = "semantic_memory";
    config.type = MESH_PARTICIPANT_MODULE;
    config.home_channel = iface.home_channel;
    nimcp_error_t err = mesh_participant_register(registry, &iface, &config, &g_semantic_memory_mesh_id);
    if (err == NIMCP_SUCCESS) g_semantic_memory_mesh_registry = registry;
    return err;
}

void semantic_memory_mesh_unregister(void) {
    if (g_semantic_memory_mesh_registry && g_semantic_memory_mesh_id != 0) {
        mesh_participant_unregister(g_semantic_memory_mesh_registry, g_semantic_memory_mesh_id);
        g_semantic_memory_mesh_id = 0;
        g_semantic_memory_mesh_registry = NULL;
    }
}


/** @brief Send heartbeat from semantic_memory module (instance-level) */
static inline void semantic_memory_heartbeat_instance(
    nimcp_health_agent_t* instance_agent, const char* operation, float progress)
{
    if (g_semantic_memory_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_semantic_memory_health_agent, operation, progress);
    }
    if (instance_agent && instance_agent != g_semantic_memory_health_agent) {
 ...
 */
static int semantic_memory_wiring_handler_callback(
    ...
    LOG_INFO(LOG_MODULE, "semantic_memory_wiring_handler_callback: registering %u handlers from KG",
             message_count);
    ...
    for (uint32_t i = 0; i < message_count; i++) {
        ...
        switch (message_types[i]) {
            case BIO_MSG_KNOWLEDGE_QUERY:
                ...
                LOG_DEBUG(LOG_MODULE, "  Registered handler for BIO_MSG_KNOWLEDGE_QUERY");
                break;
    ...
    if (bio_router_is_initialized()) {
        ...
        if (system->bio_ctx) {
            ...
            nimcp_error_t wiring_result = bio_router_register_wiring_callback(
                ...
                (void*)semantic_memory_wiring_handler_callback,
                system
            );
            ...
            if (wiring_result != NIMCP_SUCCESS) {
                ...
                LOG_INFO(LOG_MODULE, "Bio-async registered with legacy handlers (module_id=0x%04X)", BIO_MODULE_SEMANTIC_MEMORY);
            } else {
...
int semantic_memory_training_begin(void* instance) {
    if (!instance) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "semantic_memory_training_begin: NULL argument");
        return -1;
    }
    semantic_memory_heartbeat_instance(NULL, "semantic_memory_training_begin", 0.0f);
    (void)(activation_queue_node_t*)instance; /* Module state available for reset */
    return 0;
}

int semantic_memory_training_end(void* instance) {
    if (!instance) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "semantic_memory_training_end: NULL argument");
        return -1;
    }
    semantic_memory_heartbeat_instance(NULL, "semantic_memory_training_end", 1.0f);
    (void)(activation_queue_node_t*)instance; /* Module state available for finalization */
    return 0;
}

int semantic_memory_training_step(void* instance, float progress) {
    if (!instance) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "semantic_memory_training_step: NULL argument");
        return -1;
    }
    if (progress < 0.0f) progress = 0.0f;
    if (progress > 1.0f) progress = 1.0f;
    semantic_memory_heartbeat_instance(NULL, "semantic_memory_training_step", progress);
    (void)(activation_queue_node_t*)instance; /* Module state available for step adaptation */
    return 0;
}
