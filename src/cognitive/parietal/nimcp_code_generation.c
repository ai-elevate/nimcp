...
BRIDGE_BOILERPLATE(code_generation, MESH_ADAPTER_CATEGORY_COGNITIVE)
...
typedef struct {
    ...
    char epitope[NIMCP_ID_BUFFER_SIZE];                   /**< Error signature hash */
    bool success;
    float confidence;
    uint64_t timestamp;
} fix_history_entry_t;
...
int code_gen_match_historical_pattern(
    ...
    *similarity = 0.0f;
    ...
    char epitope[NIMCP_ID_BUFFER_SIZE];
