...
/** Global health agent for financial cognitive orchestrator module */
static nimcp_health_agent_t* g_fin_orch_health_agent = NULL;

void financial_cognitive_orchestrator_set_health_agent_global(void* agent) {
    g_fin_orch_health_agent = (nimcp_health_agent_t*)agent;
}

static inline void fin_orch_heartbeat_global(const char* operation, float progress) {
    if (g_fin_orch_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_fin_orch_health_agent, operation, progress);
    }
}
...
financial_cognitive_orchestrator_handle_t* financial_cognitive_orchestrator_create(
    ...
    financial_cognitive_orchestrator_handle_t* orch =
        (financial_cognitive_orchestrator_handle_t*)calloc(
