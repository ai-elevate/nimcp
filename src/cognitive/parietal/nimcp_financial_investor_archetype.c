...
static inline void fin_arch_heartbeat(const char* operation, float progress) {
    if (g_fin_arch_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_fin_arch_health_agent, operation, progress);
    }
}

/** @brief Send heartbeat from financial_investor_archetype module (instance-level) */
static inline void fin_arch_heartbeat_instance(
    void* instance_agent, const char* operation, float progress)
{
    if (g_fin_arch_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_fin_arch_health_agent, operation, progress);
    }
    if (instance_agent && instance_agent != (void*)g_fin_arch_health_agent) {
        nimcp_health_agent_heartbeat_ex((nimcp_health_agent_t*)instance_agent,
                                         operation, progress);
    }
}
...
static _Thread_local char fin_arch_last_error[256] = {0};
...
fin_archetype_config_t financial_investor_archetype_default_config(void) {
    ...
    cfg.fear_sensitivity = 1.0f;
    cfg.greed_sensitivity = 1.0f;
    cfg.stress_sensitivity = 1.0f;
