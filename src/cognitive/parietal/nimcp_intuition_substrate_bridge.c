...
NIMCP_DECLARE_HEALTH_AGENT_ATOMIC(intuition_substrate_bridge)
//=============================================================================
// Mesh Participant Registration
//=============================================================================

static mesh_participant_id_t g_intuition_substrate_bridge_mesh_id = 0;
static mesh_participant_registry_t* g_intuition_substrate_bridge_mesh_registry = NULL;

nimcp_error_t intuition_substrate_bridge_mesh_register(mesh_participant_registry_t* registry) {
    if (!registry) return NIMCP_ERROR_NULL_POINTER;
    if (g_intuition_substrate_bridge_mesh_id != 0) return NIMCP_SUCCESS;
    mesh_participant_interface_t iface;
    mesh_participant_interface_init(&iface);
    strncpy(iface.module_name, "intuition_substrate_bridge", MESH_MAX_NAME_LEN - 1);
    iface.type = MESH_PARTICIPANT_MODULE;
    iface.home_channel = mesh_adapter_get_default_channel(MESH_ADAPTER_CATEGORY_COGNITIVE);
    mesh_participant_config_t config;
    mesh_participant_config_init(&config);
    config.module_name = "intuition_substrate_bridge";
    config.type = MESH_PARTICIPANT_MODULE;
    config.home_channel = iface.home_channel;
    nimcp_error_t err = mesh_participant_register(registry, &iface, &config, &g_intuition_substrate_bridge_mesh_id);
    if (err == NIMCP_SUCCESS) g_intuition_substrate_bridge_mesh_registry = registry;
    return err;
}

void intuition_substrate_bridge_mesh_unregister(void) {
    if (g_intuition_substrate_bridge_mesh_registry && g_intuition_substrate_bridge_mesh_id != 0) {
        mesh_participant_unregister(g_intuition_substrate_bridge_mesh_registry, g_intuition_substrate_bridge_mesh_id);
        g_intuition_substrate_bridge_mesh_id = 0;
        g_intuition_substrate_bridge_mesh_registry = NULL;
    }
}


/** @brief Send heartbeat from intuition_substrate_bridge module (instance-level) */
static inline void intuition_substrate_bridge_heartbeat_instance(
    nimcp_health_agent_t* instance_agent, const char* operation, float progress)
{
    ...
    if (instance_agent && instance_agent != g_intuition_substrate_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(instance_agent, operation, progress);
    }
}

#define LOG_MODULE "INTUITION_SUBSTRATE_BRIDGE"
...
void intuition_substrate_bridge_set_instance_health_agent(
    intuition_substrate_bridge_t* bridge, nimcp_health_agent_t* agent)
{
    if (bridge) {
...
int intuition_substrate_bridge_training_begin(intuition_substrate_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "intuition_substrate_bridge_training_begin: NULL argument");
        return -1;
    }
    intuition_substrate_bridge_heartbeat_instance(bridge->health_agent, "intuition_substrate_bridge_training_begin", 0.0f);
    return 0;
}

int intuition_substrate_bridge_training_end(intuition_substrate_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "intuition_substrate_bridge_training_end: NULL argument");
        return -1;
    }
    intuition_substrate_bridge_heartbeat_instance(bridge->health_agent, "intuition_substrate_bridge_training_end", 1.0f);
    return 0;
}
...
int intuition_substrate_bridge_training_step(intuition_substrate_bridge_t* bridge, float progress) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "intuition_substrate_bridge_training_step: NULL argument");
        return -1;
    }
