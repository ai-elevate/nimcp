...
NIMCP_DECLARE_HEALTH_AGENT_ATOMIC(predictive_substrate_bridge)
//=============================================================================
// Mesh Participant Registration
//=============================================================================

static mesh_participant_id_t g_predictive_substrate_bridge_mesh_id = 0;
static mesh_participant_registry_t* g_predictive_substrate_bridge_mesh_registry = NULL;

nimcp_error_t predictive_substrate_bridge_mesh_register(mesh_participant_registry_t* registry) {
    if (!registry) return NIMCP_ERROR_NULL_POINTER;
    if (g_predictive_substrate_bridge_mesh_id != 0) return NIMCP_SUCCESS;
    mesh_participant_interface_t iface;
    mesh_participant_interface_init(&iface);
    strncpy(iface.module_name, "predictive_substrate_bridge", MESH_MAX_NAME_LEN - 1);
    iface.type = MESH_PARTICIPANT_MODULE;
    iface.home_channel = mesh_adapter_get_default_channel(MESH_ADAPTER_CATEGORY_COGNITIVE);
    mesh_participant_config_t config;
    mesh_participant_config_init(&config);
    config.module_name = "predictive_substrate_bridge";
    config.type = MESH_PARTICIPANT_MODULE;
    config.home_channel = iface.home_channel;
    nimcp_error_t err = mesh_participant_register(registry, &iface, &config, &g_predictive_substrate_bridge_mesh_id);
    if (err == NIMCP_SUCCESS) g_predictive_substrate_bridge_mesh_registry = registry;
    return err;
}

void predictive_substrate_bridge_mesh_unregister(void) {
    if (g_predictive_substrate_bridge_mesh_registry && g_predictive_substrate_bridge_mesh_id != 0) {
        mesh_participant_unregister(g_predictive_substrate_bridge_mesh_registry, g_predictive_substrate_bridge_mesh_id);
        g_predictive_substrate_bridge_mesh_id = 0;
        g_predictive_substrate_bridge_mesh_registry = NULL;
    }
}
...
predictive_substrate_bridge_t* predictive_substrate_bridge_create(void* predictive, neural_substrate_t* substrate, const predictive_substrate_config_t* config) {
    ...
    bridge->effects.error_sensitivity = NIMCP_SENSITIVITY_DEFAULT;
...
int predictive_substrate_bridge_update(predictive_substrate_bridge_t* bridge) {
    ...
    if (bridge->config.enable_fatigue_modulation) {
        bridge->effects.error_sensitivity = nimcp_clampf(metabolic_cap * bridge->config.fatigue_sensitivity, min_cap, 1.0f);
        bridge->effects.model_update_rate = nimcp_clampf(metabolic_cap * 0.9f * bridge->config.fatigue_sensitivity, min_cap, 1.0f);
    }
...
int predictive_substrate_bridge_training_begin(predictive_substrate_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "predictive_substrate_bridge_training_begin: NULL argument");
        return -1;
    }
...
int predictive_substrate_bridge_training_end(predictive_substrate_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "predictive_substrate_bridge_training_end: NULL argument");
        return -1;
    }
...
int predictive_substrate_bridge_training_step(predictive_substrate_bridge_t* bridge, float progress) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "predictive_substrate_bridge_training_step: NULL argument");
        return -1;
    }
