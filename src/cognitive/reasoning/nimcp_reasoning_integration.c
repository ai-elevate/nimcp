...
NIMCP_DECLARE_HEALTH_AGENT_ATOMIC(reasoning_integration)
//=============================================================================
// Mesh Participant Registration
//=============================================================================

static mesh_participant_id_t g_reasoning_integration_mesh_id = 0;
static mesh_participant_registry_t* g_reasoning_integration_mesh_registry = NULL;

nimcp_error_t reasoning_integration_mesh_register(mesh_participant_registry_t* registry) {
    if (!registry) return NIMCP_ERROR_NULL_POINTER;
    if (g_reasoning_integration_mesh_id != 0) return NIMCP_SUCCESS;
    mesh_participant_interface_t iface;
    mesh_participant_interface_init(&iface);
    strncpy(iface.module_name, "reasoning_integration", MESH_MAX_NAME_LEN - 1);
    iface.type = MESH_PARTICIPANT_MODULE;
    iface.home_channel = mesh_adapter_get_default_channel(MESH_ADAPTER_CATEGORY_COGNITIVE);
    mesh_participant_config_t config;
    mesh_participant_config_init(&config);
    config.module_name = "reasoning_integration";
    config.type = MESH_PARTICIPANT_MODULE;
    config.home_channel = iface.home_channel;
    nimcp_error_t err = mesh_participant_register(registry, &iface, &config, &g_reasoning_integration_mesh_id);
    if (err == NIMCP_SUCCESS) g_reasoning_integration_mesh_registry = registry;
    return err;
}

void reasoning_integration_mesh_unregister(void) {
    if (g_reasoning_integration_mesh_registry && g_reasoning_integration_mesh_id != 0) {
        mesh_participant_unregister(g_reasoning_integration_mesh_registry, g_reasoning_integration_mesh_id);
        g_reasoning_integration_mesh_id = 0;
        g_reasoning_integration_mesh_registry = NULL;
    }
}


/** @brief Send heartbeat from reasoning_integration module (instance-level) */
static inline void reasoning_integration_heartbeat_instance(
    nimcp_health_agent_t* instance_agent, const char* operation, float progress)
{
    ...
    if (instance_agent && instance_agent != g_reasoning_integration_health_agent) {
...
int reasoning_integration_training_begin(void* instance) {
    if (!instance) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "reasoning_integration_training_begin: NULL argument");
        return -1;
    }
    reasoning_integration_heartbeat_instance(NULL, "reasoning_integration_training_begin", 0.0f);
    (void)(struct reasoning_integration*)instance; /* Module state available for reset */
    return 0;
}

int reasoning_integration_training_end(void* instance) {
    if (!instance) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "reasoning_integration_training_end: NULL argument");
        return -1;
    }
    reasoning_integration_heartbeat_instance(NULL, "reasoning_integration_training_end", 1.0f);
    (void)(struct reasoning_integration*)instance; /* Module state available for finalization */
    return 0;
}

int reasoning_integration_training_step(void* instance, float progress) {
    if (!instance) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "reasoning_integration_training_step: NULL argument");
        return -1;
    }
    if (progress < 0.0f) progress = 0.0f;
    if (progress > 1.0f) progress = 1.0f;
    reasoning_integration_heartbeat_instance(NULL, "reasoning_integration_training_step", progress);
    (void)(struct reasoning_integration*)instance; /* Module state available for step adaptation */
    return 0;
}
