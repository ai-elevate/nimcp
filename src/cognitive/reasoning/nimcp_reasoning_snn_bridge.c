...
NIMCP_DECLARE_HEALTH_AGENT_ATOMIC(reasoning_snn_bridge)
//=============================================================================
// Mesh Participant Registration
//=============================================================================

static mesh_participant_id_t g_reasoning_snn_bridge_mesh_id = 0;
static mesh_participant_registry_t* g_reasoning_snn_bridge_mesh_registry = NULL;

nimcp_error_t reasoning_snn_bridge_mesh_register(mesh_participant_registry_t* registry) {
    if (!registry) return NIMCP_ERROR_NULL_POINTER;
    if (g_reasoning_snn_bridge_mesh_id != 0) return NIMCP_SUCCESS;
    mesh_participant_interface_t iface;
    mesh_participant_interface_init(&iface);
    strncpy(iface.module_name, "reasoning_snn_bridge", MESH_MAX_NAME_LEN - 1);
    iface.type = MESH_PARTICIPANT_MODULE;
    iface.home_channel = mesh_adapter_get_default_channel(MESH_ADAPTER_CATEGORY_COGNITIVE);
    mesh_participant_config_t config;
    mesh_participant_config_init(&config);
    config.module_name = "reasoning_snn_bridge";
    config.type = MESH_PARTICIPANT_MODULE;
    config.home_channel = iface.home_channel;
    nimcp_error_t err = mesh_participant_register(registry, &iface, &config, &g_reasoning_snn_bridge_mesh_id);
    if (err == NIMCP_SUCCESS) g_reasoning_snn_bridge_mesh_registry = registry;
    return err;
}

void reasoning_snn_bridge_mesh_unregister(void) {
    if (g_reasoning_snn_bridge_mesh_registry && g_reasoning_snn_bridge_mesh_id != 0) {
        mesh_participant_unregister(g_reasoning_snn_bridge_mesh_registry, g_reasoning_snn_bridge_mesh_id);
        g_reasoning_snn_bridge_mesh_id = 0;
        g_reasoning_snn_bridge_mesh_registry = NULL;
    }
}


/** @brief Send heartbeat from reasoning_snn_bridge module (instance-level) */
static inline void reasoning_snn_bridge_heartbeat_instance(
    nimcp_health_agent_t* instance_agent, const char* operation, float progress)
{
    ...
    if (instance_agent && instance_agent != g_reasoning_snn_bridge_health_agent) {
...
void reasoning_snn_bridge_set_instance_health_agent(reasoning_snn_bridge_t* bridge, nimcp_health_agent_t* agent) {
    if (!bridge) {
        NIMCP_THROW(NIMCP_ERROR_NULL_POINTER,
                    "reasoning_snn_bridge_set_instance_health_agent: NULL bridge");
        return;
    }
...
int reasoning_snn_bridge_training_begin(reasoning_snn_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "reasoning_snn_bridge_training_begin: NULL argument");
        return -1;
    }
    reasoning_snn_bridge_heartbeat_instance(bridge->health_agent, "reasoning_snn_bridge_training_begin", 0.0f);
    return 0;
}

int reasoning_snn_bridge_training_end(reasoning_snn_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "reasoning_snn_bridge_training_end: NULL argument");
        return -1;
    }
    reasoning_snn_bridge_heartbeat_instance(bridge->health_agent, "reasoning_snn_bridge_training_end", 1.0f);
    return 0;
}

int reasoning_snn_bridge_training_step(reasoning_snn_bridge_t* bridge, float progress) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "reasoning_snn_bridge_training_step: NULL argument");
        return -1;
    }
    if (progress < 0.0f) progress = 0.0f;
    if (progress > 1.0f) progress = 1.0f;
    reasoning_snn_bridge_heartbeat_instance(bridge->health_agent, "reasoning_snn_bridge_training_step", progress);
    return 0;
}
