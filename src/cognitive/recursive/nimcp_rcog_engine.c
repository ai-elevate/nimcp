...
NIMCP_DECLARE_HEALTH_AGENT_ATOMIC(rcog_engine)
//=============================================================================
// Mesh Participant Registration
//=============================================================================

static mesh_participant_id_t g_rcog_engine_mesh_id = 0;
static mesh_participant_registry_t* g_rcog_engine_mesh_registry = NULL;

nimcp_error_t rcog_engine_mesh_register(mesh_participant_registry_t* registry) {
    if (!registry) return NIMCP_ERROR_NULL_POINTER;
    if (g_rcog_engine_mesh_id != 0) return NIMCP_SUCCESS;
    mesh_participant_interface_t iface;
    mesh_participant_interface_init(&iface);
    strncpy(iface.module_name, "rcog_engine", MESH_MAX_NAME_LEN - 1);
    iface.type = MESH_PARTICIPANT_MODULE;
    iface.home_channel = mesh_adapter_get_default_channel(MESH_ADAPTER_CATEGORY_COGNITIVE);
    mesh_participant_config_t config;
    mesh_participant_config_init(&config);
    config.module_name = "rcog_engine";
    config.type = MESH_PARTICIPANT_MODULE;
    config.home_channel = iface.home_channel;
    nimcp_error_t err = mesh_participant_register(registry, &iface, &config, &g_rcog_engine_mesh_id);
    if (err == NIMCP_SUCCESS) g_rcog_engine_mesh_registry = registry;
    return err;
}

void rcog_engine_mesh_unregister(void) {
    if (g_rcog_engine_mesh_registry && g_rcog_engine_mesh_id != 0) {
        mesh_participant_unregister(g_rcog_engine_mesh_registry, g_rcog_engine_mesh_id);
        g_rcog_engine_mesh_id = 0;
        g_rcog_engine_mesh_registry = NULL;
    }
}
