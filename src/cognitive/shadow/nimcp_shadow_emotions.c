...
NIMCP_DECLARE_HEALTH_AGENT_ATOMIC(shadow_emotions)
//=============================================================================
// Mesh Participant Registration
//=============================================================================

static mesh_participant_id_t g_shadow_emotions_mesh_id = 0;
static mesh_participant_registry_t* g_shadow_emotions_mesh_registry = NULL;

nimcp_error_t shadow_emotions_mesh_register(mesh_participant_registry_t* registry) {
    if (!registry) return NIMCP_ERROR_NULL_POINTER;
    if (g_shadow_emotions_mesh_id != 0) return NIMCP_SUCCESS;
    mesh_participant_interface_t iface;
    mesh_participant_interface_init(&iface);
    strncpy(iface.module_name, "shadow_emotions", MESH_MAX_NAME_LEN - 1);
    iface.type = MESH_PARTICIPANT_MODULE;
    iface.home_channel = mesh_adapter_get_default_channel(MESH_ADAPTER_CATEGORY_COGNITIVE);
    mesh_participant_config_t config;
    mesh_participant_config_init(&config);
    config.module_name = "shadow_emotions";
    config.type = MESH_PARTICIPANT_MODULE;
    config.home_channel = iface.home_channel;
    nimcp_error_t err = mesh_participant_register(registry, &iface, &config, &g_shadow_emotions_mesh_id);
    if (err == NIMCP_SUCCESS) g_shadow_emotions_mesh_registry = registry;
    return err;
}

void shadow_emotions_mesh_unregister(void) {
    if (g_shadow_emotions_mesh_registry && g_shadow_emotions_mesh_id != 0) {
        mesh_participant_unregister(g_shadow_emotions_mesh_registry, g_shadow_emotions_mesh_id);
        g_shadow_emotions_mesh_id = 0;
        g_shadow_emotions_mesh_registry = NULL;
    }
}
 ...
 */
static int shadow_wiring_handler_callback(
    ...
    LOG_INFO(LOG_MODULE,
        "shadow_wiring_handler_callback: registering %u handlers from KG",
        message_count);
        ...
        switch (message_types[i]) {
            case BIO_MSG_INTROSPECTION_QUERY:
                ...
                LOG_DEBUG(LOG_MODULE,
                    "  Registered handler for BIO_MSG_INTROSPECTION_QUERY");
                break;
    ...
    if (bio_router_is_initialized()) {
        ...
        if (system->bio_ctx) {
            ...
            nimcp_error_t cb_result = bio_router_register_wiring_callback(
                ...
                (void*)shadow_wiring_handler_callback,
                system
            );
            ...
            if (cb_result != NIMCP_SUCCESS) {
                ...
                LOG_INFO(LOG_MODULE, "Bio-async registered (module_id=0x%04X, legacy)", BIO_MODULE_COGNITIVE_SHADOW);
            } else {
                LOG_INFO(LOG_MODULE, "Bio-async registered (module_id=0x%04X, KG-driven)", BIO_MODULE_COGNITIVE_SHADOW);
            }
        }
    }
