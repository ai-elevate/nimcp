/**
 * @file nimcp_shadow_plasticity_bridge.c
 * @brief Shadow Emotions - Plasticity Bidirectional Integration Bridge Implementation
 * @version 1.0.0
 * @date 2026-01-06
 */
...
NIMCP_DECLARE_HEALTH_AGENT_ATOMIC(shadow_plasticity_bridge)
//=============================================================================
// Mesh Participant Registration
//=============================================================================

static mesh_participant_id_t g_shadow_plasticity_bridge_mesh_id = 0;
static mesh_participant_registry_t* g_shadow_plasticity_bridge_mesh_registry = NULL;

nimcp_error_t shadow_plasticity_bridge_mesh_register(mesh_participant_registry_t* registry) {
    if (!registry) return NIMCP_ERROR_NULL_POINTER;
    if (g_shadow_plasticity_bridge_mesh_id != 0) return NIMCP_SUCCESS;
    mesh_participant_interface_t iface;
    mesh_participant_interface_init(&iface);
    strncpy(iface.module_name, "shadow_plasticity_bridge", MESH_MAX_NAME_LEN - 1);
    iface.type = MESH_PARTICIPANT_MODULE;
    iface.home_channel = mesh_adapter_get_default_channel(MESH_ADAPTER_CATEGORY_COGNITIVE);
    mesh_participant_config_t config;
    mesh_participant_config_init(&config);
    config.module_name = "shadow_plasticity_bridge";
    config.type = MESH_PARTICIPANT_MODULE;
    config.home_channel = iface.home_channel;
    nimcp_error_t err = mesh_participant_register(registry, &iface, &config, &g_shadow_plasticity_bridge_mesh_id);
    if (err == NIMCP_SUCCESS) g_shadow_plasticity_bridge_mesh_registry = registry;
    return err;
}

void shadow_plasticity_bridge_mesh_unregister(void) {
    if (g_shadow_plasticity_bridge_mesh_registry && g_shadow_plasticity_bridge_mesh_id != 0) {
        mesh_participant_unregister(g_shadow_plasticity_bridge_mesh_registry, g_shadow_plasticity_bridge_mesh_id);
        g_shadow_plasticity_bridge_mesh_id = 0;
        g_shadow_plasticity_bridge_mesh_registry = NULL;
    }
}
...
struct shadow_plasticity_bridge {
    bridge_base_t base;
    shadow_plasticity_config_t config;

    /* State */
    shadow_plasticity_state_t state;
    ...
    /* Integration state */
    shadow_integration_state_t integration_state;
    ...
    /* Callbacks */
    shadow_plasticity_learn_callback_t learn_callback;
    void* learn_callback_data;
    shadow_plasticity_integration_callback_t integration_callback;
    void* integration_callback_data;
...
static synapse_entry_t* find_synapse(shadow_plasticity_bridge_t* bridge, uint32_t synapse_id) {
...
static synapse_entry_t* find_free_slot(shadow_plasticity_bridge_t* bridge) {
    for (uint32_t i = 0; i < bridge->max_synapses; i++) {
        if (!bridge->synapses[i].in_use) {
            return &bridge->synapses[i];
        }
...
static bool is_protected_type(shadow_synapse_type_t type) {
    return type == SHADOW_SYNAPSE_DEFENSE ||
           type == SHADOW_SYNAPSE_INSIGHT;
}
...
shadow_plasticity_config_t shadow_plasticity_config_default(void) {
    shadow_plasticity_config_t config = {
        .base_learning_rate = SHADOW_PLASTICITY_DEFAULT_LR,
        ...
        .target_integration = 0.5f,

        .insight_learning_boost = 1.5f,
        .integration_learning_boost = 1.3f,
        .defense_modulation = 1.2f,

        .weight_min = 0.0f,
        .weight_max = 1.0f,

        .protect_defense_pathways = true,
        .protect_insight_pathways = true,
        .protection_strength = 1.0f,

        .max_synapses = SHADOW_PLASTICITY_MAX_SYNAPSES,
...
shadow_plasticity_bridge_t* shadow_plasticity_create(
    ...
    if (config) {
        ...
        bridge->config = shadow_plasticity_config_default();
    }
    ...
    bridge->integration_state.suppression_sensitivity = 1.0f;
    bridge->integration_state.defense_calibration = 0.5f;
    bridge->integration_state.integration_sensitivity = 1.0f;
    bridge->integration_state.insight_strength = 0.5f;
    bridge->integration_state.breakthrough_threshold = 0.7f;
    bridge->integration_state.learning_rate_mod = 1.0f;
    bridge->integration_state.last_learning_us = 0;
...
int shadow_plasticity_reset(shadow_plasticity_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "shadow_plasticity_reset: bridge is NULL");
        return -1;
    }
...
int shadow_plasticity_register_synapse(
    ...
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "shadow_plasticity_register_synapse: bridge is NULL");
        return -1;
    }
    ...
    if (find_synapse(bridge, synapse_id)) {
        nimcp_mutex_unlock(bridge->base.mutex);
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_INVALID_PARAM, "shadow_plasticity_register_synapse: validation failed");
        return -1;
    }
    ...
    if (!slot) {
        nimcp_mutex_unlock(bridge->base.mutex);
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "shadow_plasticity_register_synapse: slot is NULL");
        return -1;
    }
    ...
    /* Auto-protect defense and insight synapses */
    slot->synapse.is_protected = is_protected_type(type) &&
        (bridge->config.protect_defense_pathways || bridge->config.protect_insight_pathways);
...
int shadow_plasticity_unregister_synapse(
    ...
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "shadow_plasticity_unregister_synapse: bridge is NULL");
        return -1;
    }
    ...
    if (!entry) {
        nimcp_mutex_unlock(bridge->base.mutex);
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "shadow_plasticity_unregister_synapse: entry is NULL");
        return -1;
    }
...
int shadow_plasticity_get_synapse(
    ...
    if (!bridge || !synapse) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "shadow_plasticity_get_synapse: required parameter is NULL (bridge, synapse)");
        return -1;
    }
    ...
    if (!entry) {
        nimcp_mutex_unlock(bridge->base.mutex);
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "shadow_plasticity_get_synapse: entry is NULL");
        return -1;
    }
...
int shadow_plasticity_protect_synapse(
    ...
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "shadow_plasticity_protect_synapse: bridge is NULL");
        return -1;
    }
    ...
    if (!entry) {
        nimcp_mutex_unlock(bridge->base.mutex);
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "shadow_plasticity_protect_synapse: entry is NULL");
        return -1;
    }
...
int shadow_plasticity_learn(
    ...
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "shadow_plasticity_learn: bridge is NULL");
        return -1;
    }

    nimcp_mutex_lock(bridge->base.mutex);
    bridge->state = SHADOW_PLASTICITY_STATE_LEARNING;

    synapse_entry_t* entry = find_synapse(bridge, synapse_id);
    if (!entry) {
        bridge->state = SHADOW_PLASTICITY_STATE_IDLE;
        nimcp_mutex_unlock(bridge->base.mutex);
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "shadow_plasticity_learn: entry is NULL");
        return -1;
    }
    ...
    switch (event) {
        case SHADOW_LEARN_SUPPRESSION_SUCCESS:
            weight_change = lr * bridge->config.defense_modulation;
            bridge->stats.suppression_events++;
            break;

        case SHADOW_LEARN_SUPPRESSION_FAILURE:
            weight_change = -lr * 0.5f;
            bridge->stats.breakthrough_events++;
            break;

        case SHADOW_LEARN_DEFENSE_ACTIVATED:
            weight_change = lr * bridge->config.defense_modulation;
            break;

        case SHADOW_LEARN_DEFENSE_BYPASSED:
            weight_change = -lr * 0.3f;
            break;

        case SHADOW_LEARN_INTEGRATION_PROGRESS:
            weight_change = lr * bridge->config.integration_learning_boost;
            bridge->stats.integration_progress_events++;
            break;

        case SHADOW_LEARN_INTEGRATION_SETBACK:
            weight_change = -lr * 0.2f;
            break;

        case SHADOW_LEARN_INSIGHT_GAINED:
            weight_change = lr * bridge->config.insight_learning_boost;
            bridge->stats.insight_events++;
            break;

        case SHADOW_LEARN_REPRESSION_LIFTED:
            weight_change = lr * 0.8f;
            break;

        case SHADOW_LEARN_BREAKTHROUGH_EVENT:
            weight_change = lr * 1.0f;
            bridge->stats.breakthrough_events++;
            break;

        case SHADOW_LEARN_ACCEPTANCE:
            weight_change = lr * 0.5f;
            break;
    }
...
int shadow_plasticity_apply_reward(
    ...
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "shadow_plasticity_apply_reward: bridge is NULL");
        return -1;
    }
...
int shadow_plasticity_update_bcm(
    ...
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "shadow_plasticity_update_bcm: bridge is NULL");
        return -1;
    }
    if (dt_ms <= 0.0f) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_INVALID_PARAM, "shadow_plasticity_update_bcm: validation failed");
        return -1;
    }
...
int shadow_plasticity_homeostatic_update(
    ...
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "shadow_plasticity_homeostatic_update: bridge is NULL");
        return -1;
    }
    ...
    float mean_integration = 0.0f;
    uint32_t integration_count = 0;
    for (uint32_t i = 0; i < bridge->max_synapses; i++) {
        if (bridge->synapses[i].in_use &&
            bridge->synapses[i].synapse.type == SHADOW_SYNAPSE_INTEGRATION) {
            mean_integration += bridge->synapses[i].synapse.weight;
            integration_count++;
        }
    }
    if (integration_count > 0) {
        mean_integration /= integration_count;
    }

    /* Scale non-protected synapses toward target */
    float target = bridge->config.target_integration;
    float scale_factor = 1.0f;
    if (mean_integration > 0.0f) {
        scale_factor = target / mean_integration;
...
int shadow_plasticity_update_traces(
    ...
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "shadow_plasticity_update_traces: bridge is NULL");
        return -1;
    }
...
int shadow_plasticity_consolidate(shadow_plasticity_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "shadow_plasticity_consolidate: bridge is NULL");
        return -1;
    }

    nimcp_mutex_lock(bridge->base.mutex);
    bridge->state = SHADOW_PLASTICITY_STATE_CONSOLIDATING;
    ...
    /* Update integration state based on synapse weights */
    float suppression_sum = 0.0f, suppression_count = 0;
    float integration_sum = 0.0f, integration_count = 0;
    float insight_sum = 0.0f, insight_count = 0;
    float breakthrough_sum = 0.0f, breakthrough_count = 0;

    for (uint32_t i = 0; i < bridge->max_synapses; i++) {
        if (bridge->synapses[i].in_use) {
            switch (bridge->synapses[i].synapse.type) {
                case SHADOW_SYNAPSE_SUPPRESSION:
                    suppression_sum += bridge->synapses[i].synapse.weight;
                    suppression_count++;
                    break;
                case SHADOW_SYNAPSE_INTEGRATION:
                    integration_sum += bridge->synapses[i].synapse.weight;
                    integration_count++;
                    break;
                case SHADOW_SYNAPSE_INSIGHT:
                    insight_sum += bridge->synapses[i].synapse.weight;
                    insight_count++;
                    break;
                case SHADOW_SYNAPSE_BREAKTHROUGH:
                    breakthrough_sum += bridge->synapses[i].synapse.weight;
                    breakthrough_count++;
                    break;
                default:
                    break;
    ...
    if (suppression_count > 0) {
        bridge->integration_state.suppression_sensitivity = suppression_sum / suppression_count * 2.0f;
    }
    if (integration_count > 0) {
        bridge->integration_state.integration_sensitivity = integration_sum / integration_count * 2.0f;
    }
    if (insight_count > 0) {
        bridge->integration_state.insight_strength = insight_sum / insight_count;
    }
    if (breakthrough_count > 0) {
        bridge->integration_state.breakthrough_threshold = 1.0f - (breakthrough_sum / breakthrough_count);
    }

    bridge->integration_state.last_learning_us = bridge->current_time_us;
    bridge->state = SHADOW_PLASTICITY_STATE_IDLE;
...
int shadow_plasticity_get_integration_state(
    ...
    if (!bridge || !state) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "shadow_plasticity_get_integration_state: required parameter is NULL (bridge, state)");
        return -1;
    }

    nimcp_mutex_lock(bridge->base.mutex);
    *state = bridge->integration_state;
    nimcp_mutex_unlock(bridge->base.mutex);
...
int shadow_plasticity_get_state(
    ...
    if (!bridge || !state) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "shadow_plasticity_get_state: required parameter is NULL (bridge, state)");
        return -1;
    }
...
int shadow_plasticity_get_stats(
    ...
    if (!bridge || !stats) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "shadow_plasticity_get_stats: required parameter is NULL (bridge, stats)");
        return -1;
    }
...
int shadow_plasticity_reset_stats(shadow_plasticity_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "shadow_plasticity_reset_stats: bridge is NULL");
        return -1;
    }

    nimcp_mutex_lock(bridge->base.mutex);
    memset(&bridge->stats, 0, sizeof(shadow_plasticity_stats_t));
    nimcp_mutex_unlock(bridge->base.mutex);
...
int shadow_plasticity_register_learn_callback(
    ...
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "shadow_plasticity_register_learn_callback: bridge is NULL");
        return -1;
    }
...
int shadow_plasticity_register_integration_callback(
    ...
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "shadow_plasticity_register_integration_callback: bridge is NULL");
        return -1;
    }

    nimcp_mutex_lock(bridge->base.mutex);
    bridge->integration_callback = callback;
    bridge->integration_callback_data = user_data;
    nimcp_mutex_unlock(bridge->base.mutex);
...
int shadow_plasticity_bio_async_connect(shadow_plasticity_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "shadow_plasticity_bio_async_connect: bridge is NULL");
        return -1;
    }
    if (!bridge->config.enable_bio_async) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "shadow_plasticity_bio_async_connect: bridge->config is NULL");
        return -1;
    }
...
int shadow_plasticity_bio_async_disconnect(shadow_plasticity_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "shadow_plasticity_bio_async_disconnect: bridge is NULL");
        return -1;
    }
