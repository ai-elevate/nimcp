...
static mesh_participant_id_t g_sleep_wake_substrate_bridge_mesh_id = 0;
static mesh_participant_registry_t* g_sleep_wake_substrate_bridge_mesh_registry = NULL;

nimcp_error_t sleep_wake_substrate_bridge_mesh_register(mesh_participant_registry_t* registry) {
    if (!registry) return NIMCP_ERROR_NULL_POINTER;
    if (g_sleep_wake_substrate_bridge_mesh_id != 0) return NIMCP_SUCCESS;
    mesh_participant_interface_t iface;
    mesh_participant_interface_init(&iface);
    strncpy(iface.module_name, "sleep_wake_substrate_bridge", MESH_MAX_NAME_LEN - 1);
    iface.type = MESH_PARTICIPANT_MODULE;
    iface.home_channel = mesh_adapter_get_default_channel(MESH_ADAPTER_CATEGORY_COGNITIVE);
    mesh_participant_config_t config;
    mesh_participant_config_init(&config);
    config.module_name = "sleep_wake_substrate_bridge";
    config.type = MESH_PARTICIPANT_MODULE;
    config.home_channel = iface.home_channel;
    nimcp_error_t err = mesh_participant_register(registry, &iface, &config, &g_sleep_wake_substrate_bridge_mesh_id);
    if (err == NIMCP_SUCCESS) g_sleep_wake_substrate_bridge_mesh_registry = registry;
    return err;
}

void sleep_wake_substrate_bridge_mesh_unregister(void) {
    if (g_sleep_wake_substrate_bridge_mesh_registry && g_sleep_wake_substrate_bridge_mesh_id != 0) {
        mesh_participant_unregister(g_sleep_wake_substrate_bridge_mesh_registry, g_sleep_wake_substrate_bridge_mesh_id);
        g_sleep_wake_substrate_bridge_mesh_id = 0;
        g_sleep_wake_substrate_bridge_mesh_registry = NULL;
    }
}


/** @brief Send heartbeat from sleep_wake_substrate_bridge module (instance-level) */
static inline void sleep_wake_substrate_bridge_heartbeat_instance(
    nimcp_health_agent_t* instance_agent, const char* operation, float progress)
{
    if (g_sleep_wake_substrate_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_sleep_wake_substrate_bridge_health_agent, operation, progress);
    }
    if (instance_agent && instance_agent != g_sleep_wake_substrate_bridge_health_agent) {
...
int sleep_wake_substrate_bridge_update(sleep_wake_substrate_bridge_t* bridge) {
    if (!bridge || !bridge->substrate) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "sleep_wake_substrate_bridge_update: required parameter is NULL (bridge, bridge->substrate)");
        return -1;
    }
    substrate_metabolic_state_t metabolic;
    if (substrate_get_metabolic_state(bridge->substrate, &metabolic) != 0) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_INVALID_PARAM, "sleep_wake_substrate_bridge_update: validation failed");
        return -1;
    }
...
int sleep_wake_substrate_bridge_apply_effects(sleep_wake_substrate_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "sleep_wake_substrate_bridge_apply_effects: bridge is NULL");
        return -1;
    }
...
int sleep_wake_substrate_bridge_register_bio_async(sleep_wake_substrate_bridge_t* bridge, bio_router_t* router) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "sleep_wake_substrate_bridge_register_bio_async: bridge is NULL");
        return -1;
    }
...
void sleep_wake_substrate_bridge_set_instance_health_agent(sleep_wake_substrate_bridge_t* bridge, nimcp_health_agent_t* agent) {
    if (!bridge) {
        NIMCP_THROW(NIMCP_ERROR_NULL_POINTER,
                    "sleep_wake_substrate_bridge_set_instance_health_agent: NULL bridge");
        return;
    }
...
int sleep_wake_substrate_bridge_training_begin(sleep_wake_substrate_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "sleep_wake_substrate_bridge_training_begin: NULL argument");
        return -1;
    }
    sleep_wake_substrate_bridge_heartbeat_instance(bridge->health_agent, "sleep_wake_substrate_bridge_training_begin", 0.0f);
    return 0;
}

int sleep_wake_substrate_bridge_training_end(sleep_wake_substrate_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "sleep_wake_substrate_bridge_training_end: NULL argument");
        return -1;
    }
    sleep_wake_substrate_bridge_heartbeat_instance(bridge->health_agent, "sleep_wake_substrate_bridge_training_end", 1.0f);
    return 0;
}

int sleep_wake_substrate_bridge_training_step(sleep_wake_substrate_bridge_t* bridge, float progress) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                              "sleep_wake_substrate_bridge_training_step: NULL argument");
        return -1;
    }
    if (progress < 0.0f) progress = 0.0f;
    if (progress > 1.0f) progress = 1.0f;
    sleep_wake_substrate_bridge_heartbeat_instance(bridge->health_agent, "sleep_wake_substrate_bridge_training_step", progress);
    return 0;
}
