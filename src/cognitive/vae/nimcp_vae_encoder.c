...
void vae_encoder_set_global_health_agent(nimcp_health_agent_t* agent) {
    g_vae_encoder_health_agent = agent;
}

static inline void vae_encoder_heartbeat(const char* operation, float progress) {
    if (g_vae_encoder_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_vae_encoder_health_agent, operation, progress);
    }
}
