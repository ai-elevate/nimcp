...
/** Global health agent for brain_init_basal_ganglia module */
static nimcp_health_agent_t* g_brain_init_basal_ganglia_health_agent = NULL;

/**
 * @brief Set health agent for brain_init_basal_ganglia heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void brain_init_basal_ganglia_set_health_agent(nimcp_health_agent_t* agent) {
    g_brain_init_basal_ganglia_health_agent = agent;
}

/** @brief Send heartbeat from brain_init_basal_ganglia module */
static inline void brain_init_basal_ganglia_heartbeat(const char* operation, float progress) {
    if (g_brain_init_basal_ganglia_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_brain_init_basal_ganglia_health_agent, operation, progress);
    }
}
