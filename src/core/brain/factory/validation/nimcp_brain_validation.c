...
NIMCP_DECLARE_HEALTH_AGENT_ATOMIC(brain_validation)
//=============================================================================
// Mesh Participant Registration
//=============================================================================

static mesh_participant_id_t g_brain_validation_mesh_id = 0;
static mesh_participant_registry_t* g_brain_validation_mesh_registry = NULL;

nimcp_error_t brain_validation_mesh_register(mesh_participant_registry_t* registry) {
    if (!registry) return NIMCP_ERROR_NULL_POINTER;
    if (g_brain_validation_mesh_id != 0) return NIMCP_SUCCESS;
    mesh_participant_interface_t iface;
    mesh_participant_interface_init(&iface);
    strncpy(iface.module_name, "brain_validation", MESH_MAX_NAME_LEN - 1);
    iface.type = MESH_PARTICIPANT_MODULE;
    iface.home_channel = mesh_adapter_get_default_channel(MESH_ADAPTER_CATEGORY_SYSTEM);
    mesh_participant_config_t config;
    mesh_participant_config_init(&config);
    config.module_name = "brain_validation";
    config.type = MESH_PARTICIPANT_MODULE;
    config.home_channel = iface.home_channel;
    nimcp_error_t err = mesh_participant_register(registry, &iface, &config, &g_brain_validation_mesh_id);
    if (err == NIMCP_SUCCESS) g_brain_validation_mesh_registry = registry;
    return err;
}

void brain_validation_mesh_unregister(void) {
    if (g_brain_validation_mesh_registry && g_brain_validation_mesh_id != 0) {
        mesh_participant_unregister(g_brain_validation_mesh_registry, g_brain_validation_mesh_id);
        g_brain_validation_mesh_id = 0;
        g_brain_validation_mesh_registry = NULL;
    }
}
...
bool nimcp_brain_factory_is_cached_input(brain_t brain, const float* features, uint32_t num_features)
{
    if (!brain->last_input || !brain->cached_decision) {
        return false;  /* No cache yet - normal */
    }
    if (brain->input_size != num_features) {
        return false;  /* Size mismatch - normal cache miss */
    }
