...
/** Global health agent for corpus_callosum module */
static nimcp_health_agent_t* g_corpus_callosum_health_agent = NULL;

/**
 * @brief Set health agent for corpus_callosum heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void corpus_callosum_set_health_agent(nimcp_health_agent_t* agent) {
    g_corpus_callosum_health_agent = agent;
}

/** @brief Send heartbeat from corpus_callosum module */
static inline void corpus_callosum_heartbeat(const char* operation, float progress) {
    if (g_corpus_callosum_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_corpus_callosum_health_agent, operation, progress);
    }
}
