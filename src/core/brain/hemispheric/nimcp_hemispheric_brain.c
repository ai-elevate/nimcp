...
/** Global health agent for hemispheric_brain module */
static nimcp_health_agent_t* g_hemispheric_brain_health_agent = NULL;

/**
 * @brief Set health agent for hemispheric_brain heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void hemispheric_brain_set_health_agent(nimcp_health_agent_t* agent) {
    g_hemispheric_brain_health_agent = agent;
}

/** @brief Send heartbeat from hemispheric_brain module */
static inline void hemispheric_brain_heartbeat(const char* operation, float progress) {
    if (g_hemispheric_brain_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_hemispheric_brain_health_agent, operation, progress);
    }
}
