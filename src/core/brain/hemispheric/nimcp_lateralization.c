...
/** Global health agent for lateralization module */
static nimcp_health_agent_t* g_lateralization_health_agent = NULL;

/**
 * @brief Set health agent for lateralization heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void lateralization_set_health_agent(nimcp_health_agent_t* agent) {
    g_lateralization_health_agent = agent;
}

/** @brief Send heartbeat from lateralization module */
static inline void lateralization_heartbeat(const char* operation, float progress) {
    if (g_lateralization_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_lateralization_health_agent, operation, progress);
    }
}
