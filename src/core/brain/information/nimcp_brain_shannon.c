...
/** Global health agent for brain_shannon module */
static nimcp_health_agent_t* g_brain_shannon_health_agent = NULL;

/**
 * @brief Set health agent for brain_shannon heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void brain_shannon_set_health_agent(nimcp_health_agent_t* agent) {
    g_brain_shannon_health_agent = agent;
}

/** @brief Send heartbeat from brain_shannon module */
static inline void brain_shannon_heartbeat(const char* operation, float progress) {
    if (g_brain_shannon_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_brain_shannon_health_agent, operation, progress);
    }
}
