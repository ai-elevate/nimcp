...
/** Global health agent for brain_learning module */
static nimcp_health_agent_t* g_brain_learning_health_agent = NULL;

/**
 * @brief Set health agent for brain_learning heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void brain_learning_set_health_agent(nimcp_health_agent_t* agent) {
    g_brain_learning_health_agent = agent;
}

/** @brief Send heartbeat from brain_learning module */
static inline void brain_learning_heartbeat(const char* operation, float progress) {
    if (g_brain_learning_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_brain_learning_health_agent, operation, progress);
    }
}
