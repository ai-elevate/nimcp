...
/** Global health agent for distributed_cow module */
static nimcp_health_agent_t* g_distributed_cow_health_agent = NULL;

/**
 * @brief Set health agent for distributed_cow heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void distributed_cow_set_health_agent(nimcp_health_agent_t* agent) {
    g_distributed_cow_health_agent = agent;
}

/** @brief Send heartbeat from distributed_cow module */
static inline void distributed_cow_heartbeat(const char* operation, float progress) {
    if (g_distributed_cow_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_distributed_cow_health_agent, operation, progress);
    }
}
