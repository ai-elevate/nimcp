...
/** Global health agent for kg_analytics module */
static nimcp_health_agent_t* g_kg_analytics_health_agent = NULL;

/**
 * @brief Set health agent for kg_analytics heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void kg_analytics_set_health_agent(nimcp_health_agent_t* agent) {
    g_kg_analytics_health_agent = agent;
}

/** @brief Send heartbeat from kg_analytics module */
static inline void kg_analytics_heartbeat(const char* operation, float progress) {
    if (g_kg_analytics_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_kg_analytics_health_agent, operation, progress);
    }
}
