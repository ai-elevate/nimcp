...
/** Global health agent for kg_gc_agent module */
static nimcp_health_agent_t* g_kg_gc_agent_health_agent = NULL;

/**
 * @brief Set health agent for kg_gc_agent heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void kg_gc_agent_set_health_agent(nimcp_health_agent_t* agent) {
    g_kg_gc_agent_health_agent = agent;
}

/** @brief Send heartbeat from kg_gc_agent module */
static inline void kg_gc_agent_heartbeat(const char* operation, float progress) {
    if (g_kg_gc_agent_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_kg_gc_agent_health_agent, operation, progress);
    }
}
