...
#endif
#ifndef nimcp_calloc
#define nimcp_calloc(count, size) calloc(count, size)
#endif
#ifndef nimcp_free
...
/** Global health agent for kg_module_wiring module */
static nimcp_health_agent_t* g_kg_module_wiring_health_agent = NULL;

/**
 * @brief Set health agent for kg_module_wiring heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void kg_module_wiring_set_health_agent(nimcp_health_agent_t* agent) {
    g_kg_module_wiring_health_agent = agent;
}

/** @brief Send heartbeat from kg_module_wiring module */
static inline void kg_module_wiring_heartbeat(const char* operation, float progress) {
    if (g_kg_module_wiring_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_kg_module_wiring_health_agent, operation, progress);
    }
}
