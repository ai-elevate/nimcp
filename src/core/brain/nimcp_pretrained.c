...
/** Global health agent for pretrained module */
static nimcp_health_agent_t* g_pretrained_health_agent = NULL;

/**
 * @brief Set health agent for pretrained heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void pretrained_set_health_agent(nimcp_health_agent_t* agent) {
    g_pretrained_health_agent = agent;
}

/** @brief Send heartbeat from pretrained module */
static inline void pretrained_heartbeat(const char* operation, float progress) {
    if (g_pretrained_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_pretrained_health_agent, operation, progress);
    }
}
