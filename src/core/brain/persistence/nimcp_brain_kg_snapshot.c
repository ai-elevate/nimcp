...
/** Global health agent for brain_kg_snapshot module */
static nimcp_health_agent_t* g_brain_kg_snapshot_health_agent = NULL;

/**
 * @brief Set health agent for brain_kg_snapshot heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void brain_kg_snapshot_set_health_agent(nimcp_health_agent_t* agent) {
    g_brain_kg_snapshot_health_agent = agent;
}

/** @brief Send heartbeat from brain_kg_snapshot module */
static inline void brain_kg_snapshot_heartbeat(const char* operation, float progress) {
    if (g_brain_kg_snapshot_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_brain_kg_snapshot_health_agent, operation, progress);
    }
}
                              ...
                              const kg_snapshot_config_t* config)
{
    ...
    void* state_buffer = malloc(max_size);
    if (!state_buffer) {
