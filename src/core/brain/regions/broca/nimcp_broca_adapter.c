...
/** Global health agent for broca_adapter module */
static nimcp_health_agent_t* g_broca_adapter_health_agent = NULL;

/**
 * @brief Set health agent for broca_adapter heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void broca_adapter_set_health_agent(nimcp_health_agent_t* agent) {
    g_broca_adapter_health_agent = agent;
}

/** @brief Send heartbeat from broca_adapter module */
static inline void broca_adapter_heartbeat(const char* operation, float progress) {
    if (g_broca_adapter_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_broca_adapter_health_agent, operation, progress);
    }
}
