...
/** Global health agent for speech_motor module */
static nimcp_health_agent_t* g_speech_motor_health_agent = NULL;

/**
 * @brief Set health agent for speech_motor heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void speech_motor_set_health_agent(nimcp_health_agent_t* agent) {
    g_speech_motor_health_agent = agent;
}

/** @brief Send heartbeat from speech_motor module */
static inline void speech_motor_heartbeat(const char* operation, float progress) {
    if (g_speech_motor_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_speech_motor_health_agent, operation, progress);
    }
}
