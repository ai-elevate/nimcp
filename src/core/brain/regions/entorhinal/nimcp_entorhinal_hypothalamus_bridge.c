...
/** Global health agent for entorhinal_hypothalamus_bridge module */
static nimcp_health_agent_t* g_entorhinal_hypothalamus_bridge_health_agent = NULL;

/**
 * @brief Set health agent for entorhinal_hypothalamus_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void entorhinal_hypothalamus_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_entorhinal_hypothalamus_bridge_health_agent = agent;
}

/** @brief Send heartbeat from entorhinal_hypothalamus_bridge module */
static inline void entorhinal_hypothalamus_bridge_heartbeat(const char* operation, float progress) {
    if (g_entorhinal_hypothalamus_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_entorhinal_hypothalamus_bridge_health_agent, operation, progress);
    }
}
    ...
    const entorhinal_hypothalamus_config_t* config)
{
    ...
    if (bridge->config.enable_value_mapping) {
        bridge->value_map = (spatial_value_map_t*)calloc(1, sizeof(spatial_value_map_t));
        if (bridge->value_map) {
            bridge->value_map->bindings = (spatial_value_binding_t*)calloc(
                bridge->config.max_value_bindings, sizeof(spatial_value_binding_t));
            if (!bridge->value_map->bindings) {
                free(bridge->value_map);
    ...
    entorhinal_hypothalamus_bridge_state_t* bridge)
{
    ...
    if (bridge->value_map) {
        free(bridge->value_map->bindings);
        free(bridge->value_map);
    }
