...
NIMCP_DECLARE_HEALTH_AGENT_ATOMIC(hypo_predictive_fep_bridge)
//=============================================================================
// Mesh Participant Registration
//=============================================================================

static mesh_participant_id_t g_hypo_predictive_fep_bridge_mesh_id = 0;
static mesh_participant_registry_t* g_hypo_predictive_fep_bridge_mesh_registry = NULL;

nimcp_error_t hypo_predictive_fep_bridge_mesh_register(mesh_participant_registry_t* registry) {
    if (!registry) return NIMCP_ERROR_NULL_POINTER;
    if (g_hypo_predictive_fep_bridge_mesh_id != 0) return NIMCP_SUCCESS;
    mesh_participant_interface_t iface;
    mesh_participant_interface_init(&iface);
    strncpy(iface.module_name, "hypo_predictive_fep_bridge", MESH_MAX_NAME_LEN - 1);
    iface.type = MESH_PARTICIPANT_MODULE;
    iface.home_channel = mesh_adapter_get_default_channel(MESH_ADAPTER_CATEGORY_SYSTEM);
    mesh_participant_config_t config;
    mesh_participant_config_init(&config);
    config.module_name = "hypo_predictive_fep_bridge";
    config.type = MESH_PARTICIPANT_MODULE;
    config.home_channel = iface.home_channel;
    nimcp_error_t err = mesh_participant_register(registry, &iface, &config, &g_hypo_predictive_fep_bridge_mesh_id);
    if (err == NIMCP_SUCCESS) g_hypo_predictive_fep_bridge_mesh_registry = registry;
    return err;
}

void hypo_predictive_fep_bridge_mesh_unregister(void) {
    if (g_hypo_predictive_fep_bridge_mesh_registry && g_hypo_predictive_fep_bridge_mesh_id != 0) {
        mesh_participant_unregister(g_hypo_predictive_fep_bridge_mesh_registry, g_hypo_predictive_fep_bridge_mesh_id);
        g_hypo_predictive_fep_bridge_mesh_id = 0;
        g_hypo_predictive_fep_bridge_mesh_registry = NULL;
    }
}
...
hypo_pred_fep_bridge_t* hypo_pred_fep_create(
    ...
    if (!drive_system || !fep_system) {
        ...
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "hypo_pred_fep_create: required parameter is NULL (drive_system, fep_system)");
        return NULL;
    }
    ...
    if (!bridge) {
        ...
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NO_MEMORY, "hypo_pred_fep_create: bridge is NULL");
        return NULL;
    }
    ...
    if (!bridge->base.mutex) {
        ...
        nimcp_free(bridge);
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "hypo_pred_fep_create: bridge->base is NULL");
        return NULL;
    }
