...
/** Global health agent for hypothalamus_drives module */
static nimcp_health_agent_t* g_hypothalamus_drives_health_agent = NULL;

/**
 * @brief Set health agent for hypothalamus_drives heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void hypothalamus_drives_set_health_agent(nimcp_health_agent_t* agent) {
    g_hypothalamus_drives_health_agent = agent;
}

/** @brief Send heartbeat from hypothalamus_drives module */
static inline void hypothalamus_drives_heartbeat(const char* operation, float progress) {
    if (g_hypothalamus_drives_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_hypothalamus_drives_health_agent, operation, progress);
    }
}
