...
/** Global health agent for hypothalamus_homeostasis module */
static nimcp_health_agent_t* g_hypothalamus_homeostasis_health_agent = NULL;

/**
 * @brief Set health agent for hypothalamus_homeostasis heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void hypothalamus_homeostasis_set_health_agent(nimcp_health_agent_t* agent) {
    g_hypothalamus_homeostasis_health_agent = agent;
}

/** @brief Send heartbeat from hypothalamus_homeostasis module */
static inline void hypothalamus_homeostasis_heartbeat(const char* operation, float progress) {
    if (g_hypothalamus_homeostasis_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_hypothalamus_homeostasis_health_agent, operation, progress);
    }
}
...
bool hypo_homeostasis_set_gains(hypo_homeostasis_handle_t* system,
                                 hypo_variable_type_t var_type,
                                 const hypo_pid_gains_t* gains) {
    if (!system || !gains || var_type >= HYPO_VAR_COUNT) return false;
