...
/** Global health agent for insula_adapter module */
static nimcp_health_agent_t* g_insula_adapter_health_agent = NULL;

/**
 * @brief Set health agent for insula_adapter heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void insula_adapter_set_health_agent(nimcp_health_agent_t* agent) {
    g_insula_adapter_health_agent = agent;
}

/** @brief Send heartbeat from insula_adapter module */
static inline void insula_adapter_heartbeat(const char* operation, float progress) {
    if (g_insula_adapter_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_insula_adapter_health_agent, operation, progress);
    }
}
