...
NIMCP_DECLARE_HEALTH_AGENT_ATOMIC(insula_quantum_bridge)
//=============================================================================
// Mesh Participant Registration
//=============================================================================

static mesh_participant_id_t g_insula_quantum_bridge_mesh_id = 0;
static mesh_participant_registry_t* g_insula_quantum_bridge_mesh_registry = NULL;

nimcp_error_t insula_quantum_bridge_mesh_register(mesh_participant_registry_t* registry) {
    if (!registry) return NIMCP_ERROR_NULL_POINTER;
    if (g_insula_quantum_bridge_mesh_id != 0) return NIMCP_SUCCESS;
    mesh_participant_interface_t iface;
    mesh_participant_interface_init(&iface);
    strncpy(iface.module_name, "insula_quantum_bridge", MESH_MAX_NAME_LEN - 1);
    iface.type = MESH_PARTICIPANT_MODULE;
    iface.home_channel = mesh_adapter_get_default_channel(MESH_ADAPTER_CATEGORY_SYSTEM);
    mesh_participant_config_t config;
    mesh_participant_config_init(&config);
    config.module_name = "insula_quantum_bridge";
    config.type = MESH_PARTICIPANT_MODULE;
    config.home_channel = iface.home_channel;
    nimcp_error_t err = mesh_participant_register(registry, &iface, &config, &g_insula_quantum_bridge_mesh_id);
    if (err == NIMCP_SUCCESS) g_insula_quantum_bridge_mesh_registry = registry;
    return err;
}

void insula_quantum_bridge_mesh_unregister(void) {
    if (g_insula_quantum_bridge_mesh_registry && g_insula_quantum_bridge_mesh_id != 0) {
        mesh_participant_unregister(g_insula_quantum_bridge_mesh_registry, g_insula_quantum_bridge_mesh_id);
        g_insula_quantum_bridge_mesh_id = 0;
        g_insula_quantum_bridge_mesh_registry = NULL;
    }
}


#define LOG_MODULE "INSULA_QUANTUM_BRIDGE"
...
void insula_quantum_bridge_destroy(insula_quantum_bridge_t* bridge) {
    if (!bridge) return;
    NIMCP_LOGGING_DEBUG("Destroying %s bridge", "insula_quantum");
