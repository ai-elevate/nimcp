...
/** Global health agent for lc_bio_async_bridge module */
static nimcp_health_agent_t* g_lc_bio_async_bridge_health_agent = NULL;

/**
 * @brief Set health agent for lc_bio_async_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void lc_bio_async_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_lc_bio_async_bridge_health_agent = agent;
}

/** @brief Send heartbeat from lc_bio_async_bridge module */
static inline void lc_bio_async_bridge_heartbeat(const char* operation, float progress) {
    if (g_lc_bio_async_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_lc_bio_async_bridge_health_agent, operation, progress);
    }
}
...
lc_bio_async_bridge_t* lc_bio_async_bridge_create(const lc_bio_async_config_t* config) {
    ...
    bridge->subscriptions = calloc(bridge->subscription_capacity, sizeof(lc_bio_subscription_t));
