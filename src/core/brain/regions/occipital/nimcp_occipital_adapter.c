...
/** Global health agent for occipital_adapter module */
static nimcp_health_agent_t* g_occipital_adapter_health_agent = NULL;

/**
 * @brief Set health agent for occipital_adapter heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void occipital_adapter_set_health_agent(nimcp_health_agent_t* agent) {
    g_occipital_adapter_health_agent = agent;
}

/** @brief Send heartbeat from occipital_adapter module */
static inline void occipital_adapter_heartbeat(const char* operation, float progress) {
    if (g_occipital_adapter_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_occipital_adapter_health_agent, operation, progress);
    }
}
