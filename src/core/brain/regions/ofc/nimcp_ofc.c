...
/** Global health agent for ofc module */
static nimcp_health_agent_t* g_ofc_health_agent = NULL;

/**
 * @brief Set health agent for ofc heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void ofc_set_health_agent(nimcp_health_agent_t* agent) {
    g_ofc_health_agent = agent;
}

/** @brief Send heartbeat from ofc module */
static inline void ofc_heartbeat(const char* operation, float progress) {
    if (g_ofc_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_ofc_health_agent, operation, progress);
    }
}
