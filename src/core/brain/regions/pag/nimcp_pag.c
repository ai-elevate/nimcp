...
/** Global health agent for pag module */
static nimcp_health_agent_t* g_pag_health_agent = NULL;

/**
 * @brief Set health agent for pag heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void pag_set_health_agent(nimcp_health_agent_t* agent) {
    g_pag_health_agent = agent;
}

/** @brief Send heartbeat from pag module */
static inline void pag_heartbeat(const char* operation, float progress) {
    if (g_pag_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_pag_health_agent, operation, progress);
    }
}
