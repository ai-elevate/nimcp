...
/** Global health agent for parietal_adapter module */
static nimcp_health_agent_t* g_parietal_adapter_health_agent = NULL;

/**
 * @brief Set health agent for parietal_adapter heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void parietal_adapter_set_health_agent(nimcp_health_agent_t* agent) {
    g_parietal_adapter_health_agent = agent;
}

/** @brief Send heartbeat from parietal_adapter module */
static inline void parietal_adapter_heartbeat(const char* operation, float progress) {
    if (g_parietal_adapter_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_parietal_adapter_health_agent, operation, progress);
    }
}
