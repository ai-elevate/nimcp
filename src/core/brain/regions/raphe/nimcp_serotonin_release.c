...
/** Global health agent for serotonin_release module */
static nimcp_health_agent_t* g_serotonin_release_health_agent = NULL;

/**
 * @brief Set health agent for serotonin_release heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void serotonin_release_set_health_agent(nimcp_health_agent_t* agent) {
    g_serotonin_release_health_agent = agent;
}

/** @brief Send heartbeat from serotonin_release module */
static inline void serotonin_release_heartbeat(const char* operation, float progress) {
    if (g_serotonin_release_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_serotonin_release_health_agent, operation, progress);
    }
}
