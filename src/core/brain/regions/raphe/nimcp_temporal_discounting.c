...
/** Global health agent for temporal_discounting module */
static nimcp_health_agent_t* g_temporal_discounting_health_agent = NULL;

/**
 * @brief Set health agent for temporal_discounting heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void temporal_discounting_set_health_agent(nimcp_health_agent_t* agent) {
    g_temporal_discounting_health_agent = agent;
}

/** @brief Send heartbeat from temporal_discounting module */
static inline void temporal_discounting_heartbeat(const char* operation, float progress) {
    if (g_temporal_discounting_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_temporal_discounting_health_agent, operation, progress);
    }
}
