...
/** Global health agent for reticular_bio_async_bridge module */
static nimcp_health_agent_t* g_reticular_bio_async_bridge_health_agent = NULL;

/**
 * @brief Set health agent for reticular_bio_async_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void reticular_bio_async_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_reticular_bio_async_bridge_health_agent = agent;
}

/** @brief Send heartbeat from reticular_bio_async_bridge module */
static inline void reticular_bio_async_bridge_heartbeat(const char* operation, float progress) {
    if (g_reticular_bio_async_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_reticular_bio_async_bridge_health_agent, operation, progress);
    }
}
...
reticular_bridge_t* reticular_bridge_create(const reticular_bridge_config_t* config) {
    ...
    bridge->subscriptions = calloc(bridge->subscription_capacity, sizeof(*bridge->subscriptions));
