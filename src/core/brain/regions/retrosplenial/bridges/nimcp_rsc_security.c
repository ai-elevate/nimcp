...
/** Global health agent for rsc_security module */
static nimcp_health_agent_t* g_rsc_security_health_agent = NULL;

/**
 * @brief Set health agent for rsc_security heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void rsc_security_set_health_agent(nimcp_health_agent_t* agent) {
    g_rsc_security_health_agent = agent;
}

/** @brief Send heartbeat from rsc_security module */
static inline void rsc_security_heartbeat(const char* operation, float progress) {
    if (g_rsc_security_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_rsc_security_health_agent, operation, progress);
    }
}
