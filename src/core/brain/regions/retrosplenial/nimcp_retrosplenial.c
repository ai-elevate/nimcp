...
/** Global health agent for retrosplenial module */
static nimcp_health_agent_t* g_retrosplenial_health_agent = NULL;

/**
 * @brief Set health agent for retrosplenial heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void retrosplenial_set_health_agent(nimcp_health_agent_t* agent) {
    g_retrosplenial_health_agent = agent;
}

/** @brief Send heartbeat from retrosplenial module */
static inline void retrosplenial_heartbeat(const char* operation, float progress) {
    if (g_retrosplenial_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_retrosplenial_health_agent, operation, progress);
    }
}
