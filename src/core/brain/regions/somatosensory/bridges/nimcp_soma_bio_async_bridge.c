...
/** Global health agent for soma_bio_async_bridge module */
static nimcp_health_agent_t* g_soma_bio_async_bridge_health_agent = NULL;

/**
 * @brief Set health agent for soma_bio_async_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void soma_bio_async_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_soma_bio_async_bridge_health_agent = agent;
}

/** @brief Send heartbeat from soma_bio_async_bridge module */
static inline void soma_bio_async_bridge_heartbeat(const char* operation, float progress) {
    if (g_soma_bio_async_bridge_health_agent) {
...
soma_bio_router_t* soma_bio_async_bridge_create(const soma_bio_async_config_t* config) {
    ...
    bridge->subscriptions = calloc(bridge->subscription_capacity, sizeof(soma_bio_subscription_t));
