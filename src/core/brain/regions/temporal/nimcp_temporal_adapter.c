...
/** Global health agent for temporal_adapter module */
static nimcp_health_agent_t* g_temporal_adapter_health_agent = NULL;

/**
 * @brief Set health agent for temporal_adapter heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void temporal_adapter_set_health_agent(nimcp_health_agent_t* agent) {
    g_temporal_adapter_health_agent = agent;
}

/** @brief Send heartbeat from temporal_adapter module */
static inline void temporal_adapter_heartbeat(const char* operation, float progress) {
    if (g_temporal_adapter_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_temporal_adapter_health_agent, operation, progress);
    }
}
