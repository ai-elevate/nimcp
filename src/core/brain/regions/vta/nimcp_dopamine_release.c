...
/** Global health agent for dopamine_release module */
static nimcp_health_agent_t* g_dopamine_release_health_agent = NULL;

/**
 * @brief Set health agent for dopamine_release heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void dopamine_release_set_health_agent(nimcp_health_agent_t* agent) {
    g_dopamine_release_health_agent = agent;
}

/** @brief Send heartbeat from dopamine_release module */
static inline void dopamine_release_heartbeat(const char* operation, float progress) {
    if (g_dopamine_release_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_dopamine_release_health_agent, operation, progress);
    }
}
