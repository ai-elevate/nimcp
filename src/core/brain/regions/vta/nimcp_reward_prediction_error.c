...
/** Global health agent for reward_prediction_error module */
static nimcp_health_agent_t* g_reward_prediction_error_health_agent = NULL;

/**
 * @brief Set health agent for reward_prediction_error heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void reward_prediction_error_set_health_agent(nimcp_health_agent_t* agent) {
    g_reward_prediction_error_health_agent = agent;
}

/** @brief Send heartbeat from reward_prediction_error module */
static inline void reward_prediction_error_heartbeat(const char* operation, float progress) {
    if (g_reward_prediction_error_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_reward_prediction_error_health_agent, operation, progress);
    }
}
