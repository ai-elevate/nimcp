...
/** Global health agent for vta module */
static nimcp_health_agent_t* g_vta_health_agent = NULL;

/**
 * @brief Set health agent for vta heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void vta_set_health_agent(nimcp_health_agent_t* agent) {
    g_vta_health_agent = agent;
}

/** @brief Send heartbeat from vta module */
static inline void vta_heartbeat(const char* operation, float progress) {
    if (g_vta_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_vta_health_agent, operation, progress);
    }
}
