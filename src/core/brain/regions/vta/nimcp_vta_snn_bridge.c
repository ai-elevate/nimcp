...
/** Global health agent for vta_snn_bridge module */
static nimcp_health_agent_t* g_vta_snn_bridge_health_agent = NULL;

/**
 * @brief Set health agent for vta_snn_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void vta_snn_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_vta_snn_bridge_health_agent = agent;
}

/** @brief Send heartbeat from vta_snn_bridge module */
static inline void vta_snn_bridge_heartbeat(const char* operation, float progress) {
    if (g_vta_snn_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_vta_snn_bridge_health_agent, operation, progress);
    }
}
...
nimcp_vta_snn_bridge_t* nimcp_vta_snn_create(const nimcp_vta_snn_config_t* config) {
    ...
    bridge->input_spikes = calloc(bridge->spike_buffer_size, sizeof(float));
    bridge->output_spikes = calloc(bridge->spike_buffer_size, sizeof(float));
...
int nimcp_vta_snn_connect_vta(
    ...
    if (!bridge || !vta_adapter) return -1;
