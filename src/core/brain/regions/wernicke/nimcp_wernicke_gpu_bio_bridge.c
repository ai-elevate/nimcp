...
/** Global health agent for wernicke_gpu_bio_bridge module */
static nimcp_health_agent_t* g_wernicke_gpu_bio_bridge_health_agent = NULL;

/**
 * @brief Set health agent for wernicke_gpu_bio_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void wernicke_gpu_bio_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_wernicke_gpu_bio_bridge_health_agent = agent;
}

/** @brief Send heartbeat from wernicke_gpu_bio_bridge module */
static inline void wernicke_gpu_bio_bridge_heartbeat(const char* operation, float progress) {
    if (g_wernicke_gpu_bio_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_wernicke_gpu_bio_bridge_health_agent, operation, progress);
    }
}
    ...
    const wernicke_gpu_bio_config_t* config)
{
    ...
    bridge->batch_frames = calloc(MAX_GPU_BATCH_SIZE,
                                   sizeof(wernicke_gpu_spectral_frame_t));
    bridge->batch_phonemes = calloc(MAX_GPU_BATCH_SIZE,
                                     sizeof(wernicke_gpu_phoneme_result_t));
    bridge->batch_words = calloc(bridge->config.max_batch_size,
                                  sizeof(wernicke_gpu_word_candidate_t));
    bridge->batch_activations = calloc(bridge->config.max_batch_size,
...
void wernicke_gpu_bio_destroy(wernicke_gpu_bio_bridge_t* bridge) {
    ...
    /* Free batch buffers */
    free(bridge->batch_frames);
    free(bridge->batch_phonemes);
    free(bridge->batch_words);
    free(bridge->batch_activations);
