...
/** Global health agent for wernicke_immune module */
static nimcp_health_agent_t* g_wernicke_immune_health_agent = NULL;

/**
 * @brief Set health agent for wernicke_immune heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void wernicke_immune_set_health_agent(nimcp_health_agent_t* agent) {
    g_wernicke_immune_health_agent = agent;
}

/** @brief Send heartbeat from wernicke_immune module */
static inline void wernicke_immune_heartbeat(const char* operation, float progress) {
    if (g_wernicke_immune_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_wernicke_immune_health_agent, operation, progress);
    }
}
