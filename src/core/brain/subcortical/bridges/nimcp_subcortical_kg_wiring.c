...
/** Global health agent for subcortical_kg_wiring module */
static nimcp_health_agent_t* g_subcortical_kg_wiring_health_agent = NULL;

/**
 * @brief Set health agent for subcortical_kg_wiring heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void subcortical_kg_wiring_set_health_agent(nimcp_health_agent_t* agent) {
    g_subcortical_kg_wiring_health_agent = agent;
}

/** @brief Send heartbeat from subcortical_kg_wiring module */
static inline void subcortical_kg_wiring_heartbeat(const char* operation, float progress) {
    if (g_subcortical_kg_wiring_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_subcortical_kg_wiring_health_agent, operation, progress);
    }
}
