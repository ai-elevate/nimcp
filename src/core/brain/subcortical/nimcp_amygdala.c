...
/** Global health agent for amygdala module */
static nimcp_health_agent_t* g_amygdala_health_agent = NULL;

/**
 * @brief Set health agent for amygdala heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void amygdala_set_health_agent(nimcp_health_agent_t* agent) {
    g_amygdala_health_agent = agent;
}

/** @brief Send heartbeat from amygdala module */
static inline void amygdala_heartbeat(const char* operation, float progress) {
    if (g_amygdala_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_amygdala_health_agent, operation, progress);
    }
}
