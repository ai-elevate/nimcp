...
/** Global health agent for basal_ganglia_amygdala_bridge module */
static nimcp_health_agent_t* g_basal_ganglia_amygdala_bridge_health_agent = NULL;

/**
 * @brief Set health agent for basal_ganglia_amygdala_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void basal_ganglia_amygdala_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_basal_ganglia_amygdala_bridge_health_agent = agent;
}

/** @brief Send heartbeat from basal_ganglia_amygdala_bridge module */
static inline void basal_ganglia_amygdala_bridge_heartbeat(const char* operation, float progress) {
    if (g_basal_ganglia_amygdala_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_basal_ganglia_amygdala_bridge_health_agent, operation, progress);
    }
}
