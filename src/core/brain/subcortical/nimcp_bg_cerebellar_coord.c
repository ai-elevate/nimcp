...
NIMCP_DECLARE_HEALTH_AGENT_ATOMIC(bg_cerebellar_coord)
//=============================================================================
// Mesh Participant Registration
//=============================================================================

static mesh_participant_id_t g_bg_cerebellar_coord_mesh_id = 0;
static mesh_participant_registry_t* g_bg_cerebellar_coord_mesh_registry = NULL;

nimcp_error_t bg_cerebellar_coord_mesh_register(mesh_participant_registry_t* registry) {
    if (!registry) return NIMCP_ERROR_NULL_POINTER;
    if (g_bg_cerebellar_coord_mesh_id != 0) return NIMCP_SUCCESS;
    mesh_participant_interface_t iface;
    mesh_participant_interface_init(&iface);
    strncpy(iface.module_name, "bg_cerebellar_coord", MESH_MAX_NAME_LEN - 1);
    iface.type = MESH_PARTICIPANT_MODULE;
    iface.home_channel = mesh_adapter_get_default_channel(MESH_ADAPTER_CATEGORY_SUBCORTICAL);
    mesh_participant_config_t config;
    mesh_participant_config_init(&config);
    config.module_name = "bg_cerebellar_coord";
    config.type = MESH_PARTICIPANT_MODULE;
    config.home_channel = iface.home_channel;
    nimcp_error_t err = mesh_participant_register(registry, &iface, &config, &g_bg_cerebellar_coord_mesh_id);
    if (err == NIMCP_SUCCESS) g_bg_cerebellar_coord_mesh_registry = registry;
    return err;
}

void bg_cerebellar_coord_mesh_unregister(void) {
    if (g_bg_cerebellar_coord_mesh_registry && g_bg_cerebellar_coord_mesh_id != 0) {
        mesh_participant_unregister(g_bg_cerebellar_coord_mesh_registry, g_bg_cerebellar_coord_mesh_id);
        g_bg_cerebellar_coord_mesh_id = 0;
        g_bg_cerebellar_coord_mesh_registry = NULL;
    }
}
...
bg_cerebellar_coord_t* bgcb_create(const bgcb_config_t* config) {
    ...
    if (!coord->motor_output) {
        nimcp_free(coord);
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NO_MEMORY, "bgcb_create: coord->motor_output is NULL");
        return NULL;
    }
...
int bgcb_reset(bg_cerebellar_coord_t* coord) {
    if (!coord) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "bgcb_reset: coord is NULL");
        return -1;
    }
...
int bgcb_update_bg_rpe(bg_cerebellar_coord_t* coord, float rpe) {
    if (!coord) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "bgcb_update_bg_rpe: coord is NULL");
        return -1;
    }
...
int bgcb_update_cb_error(bg_cerebellar_coord_t* coord, float error) {
    if (!coord) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "bgcb_update_cb_error: coord is NULL");
        return -1;
    }
...
int bgcb_coordinate(bg_cerebellar_coord_t* coord) {
    if (!coord) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "bgcb_coordinate: coord is NULL");
        return -1;
    }
...
int bgcb_step(bg_cerebellar_coord_t* coord, float dt_ms) {
    if (!coord || dt_ms <= 0) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_INVALID_PARAM, "bgcb_step: coord is NULL");
        return -1;
    }
...
int bgcb_adapt_weights(bg_cerebellar_coord_t* coord, float performance) {
    if (!coord) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "bgcb_adapt_weights: coord is NULL");
        return -1;
    }
...
int bgcb_get_stats(const bg_cerebellar_coord_t* coord, bgcb_stats_t* stats) {
    if (!coord || !stats) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "bgcb_get_stats: required parameter is NULL (coord, stats)");
        return -1;
    }
    *stats = coord->stats;
    return 0;
