...
static nimcp_health_agent_t* g_bgsc_health_agent = NULL;

void bgsc_set_health_agent(nimcp_health_agent_t* agent) {
    g_bgsc_health_agent = agent;
}

static inline void bgsc_heartbeat(const char* operation, float progress) {
    if (g_bgsc_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_bgsc_health_agent, operation, progress);
    }
}
