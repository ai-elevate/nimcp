...
/** Global health agent for globus_pallidus module */
static nimcp_health_agent_t* g_globus_pallidus_health_agent = NULL;

/**
 * @brief Set health agent for globus_pallidus heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void globus_pallidus_set_health_agent(nimcp_health_agent_t* agent) {
    g_globus_pallidus_health_agent = agent;
}

/** @brief Send heartbeat from globus_pallidus module */
static inline void globus_pallidus_heartbeat(const char* operation, float progress) {
    if (g_globus_pallidus_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_globus_pallidus_health_agent, operation, progress);
    }
}
