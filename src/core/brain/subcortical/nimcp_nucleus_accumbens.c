...
/** Global health agent for nucleus_accumbens module */
static nimcp_health_agent_t* g_nucleus_accumbens_health_agent = NULL;

/**
 * @brief Set health agent for nucleus_accumbens heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void nucleus_accumbens_set_health_agent(nimcp_health_agent_t* agent) {
    g_nucleus_accumbens_health_agent = agent;
}

/** @brief Send heartbeat from nucleus_accumbens module */
static inline void nucleus_accumbens_heartbeat(const char* operation, float progress) {
    if (g_nucleus_accumbens_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_nucleus_accumbens_health_agent, operation, progress);
    }
}
