...
/** Global health agent for striatum module */
static nimcp_health_agent_t* g_striatum_health_agent = NULL;

/**
 * @brief Set health agent for striatum heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void striatum_set_health_agent(nimcp_health_agent_t* agent) {
    g_striatum_health_agent = agent;
}

/** @brief Send heartbeat from striatum module */
static inline void striatum_heartbeat(const char* operation, float progress) {
    if (g_striatum_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_striatum_health_agent, operation, progress);
    }
}
