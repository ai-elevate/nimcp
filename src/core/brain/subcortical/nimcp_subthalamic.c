...
/** Global health agent for subthalamic module */
static nimcp_health_agent_t* g_subthalamic_health_agent = NULL;

/**
 * @brief Set health agent for subthalamic heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void subthalamic_set_health_agent(nimcp_health_agent_t* agent) {
    g_subthalamic_health_agent = agent;
}

/** @brief Send heartbeat from subthalamic module */
static inline void subthalamic_heartbeat(const char* operation, float progress) {
    if (g_subthalamic_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_subthalamic_health_agent, operation, progress);
    }
}
