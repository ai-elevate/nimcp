...
/** Global health agent for thalamus module */
static nimcp_health_agent_t* g_thalamus_health_agent = NULL;

/**
 * @brief Set health agent for thalamus heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void thalamus_set_health_agent(nimcp_health_agent_t* agent) {
    g_thalamus_health_agent = agent;
}

/** @brief Send heartbeat from thalamus module */
static inline void thalamus_heartbeat(const char* operation, float progress) {
    if (g_thalamus_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_thalamus_health_agent, operation, progress);
    }
}
