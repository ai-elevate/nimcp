...
NIMCP_DECLARE_HEALTH_AGENT_ATOMIC(thalamus)
//=============================================================================
// Mesh Participant Registration
//=============================================================================

static mesh_participant_id_t g_thalamus_mesh_id = 0;
static mesh_participant_registry_t* g_thalamus_mesh_registry = NULL;

nimcp_error_t thalamus_mesh_register(mesh_participant_registry_t* registry) {
    if (!registry) return NIMCP_ERROR_NULL_POINTER;
    if (g_thalamus_mesh_id != 0) return NIMCP_SUCCESS;
    mesh_participant_interface_t iface;
    mesh_participant_interface_init(&iface);
    strncpy(iface.module_name, "thalamus", MESH_MAX_NAME_LEN - 1);
    ...
    nimcp_error_t err = mesh_participant_register(registry, &iface, &config, &g_thalamus_mesh_id);
    if (err == NIMCP_SUCCESS) g_thalamus_mesh_registry = registry;
    return err;
}

void thalamus_mesh_unregister(void) {
    if (g_thalamus_mesh_registry && g_thalamus_mesh_id != 0) {
        mesh_participant_unregister(g_thalamus_mesh_registry, g_thalamus_mesh_id);
        g_thalamus_mesh_id = 0;
        g_thalamus_mesh_registry = NULL;
    }
}
