...
/** Global health agent for command_compliance module */
static nimcp_health_agent_t* g_command_compliance_health_agent = NULL;

/**
 * @brief Set health agent for command_compliance heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void command_compliance_set_health_agent(nimcp_health_agent_t* agent) {
    g_command_compliance_health_agent = agent;
}

/** @brief Send heartbeat from command_compliance module */
static inline void command_compliance_heartbeat(const char* operation, float progress) {
    if (g_command_compliance_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_command_compliance_health_agent, operation, progress);
    }
}
