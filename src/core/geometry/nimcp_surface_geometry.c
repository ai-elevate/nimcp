...
/** Global health agent for surface_geometry module */
static nimcp_health_agent_t* g_surface_geometry_health_agent = NULL;

/**
 * @brief Set health agent for surface_geometry heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void surface_geometry_set_health_agent(nimcp_health_agent_t* agent) {
    g_surface_geometry_health_agent = agent;
}

/** @brief Send heartbeat from surface_geometry module */
static inline void surface_geometry_heartbeat(const char* operation, float progress) {
    if (g_surface_geometry_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_surface_geometry_health_agent, operation, progress);
    }
}
