...
/** Global health agent for arousal_state module */
static nimcp_health_agent_t* g_arousal_state_health_agent = NULL;

/**
 * @brief Set health agent for arousal_state heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void arousal_state_set_health_agent(nimcp_health_agent_t* agent) {
    g_arousal_state_health_agent = agent;
}

/** @brief Send heartbeat from arousal_state module */
static inline void arousal_state_heartbeat(const char* operation, float progress) {
    if (g_arousal_state_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_arousal_state_health_agent, operation, progress);
    }
}
