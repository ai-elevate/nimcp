...
/** Global health agent for neural_substrate module */
static nimcp_health_agent_t* g_neural_substrate_health_agent = NULL;

/**
 * @brief Set health agent for neural_substrate heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void neural_substrate_set_health_agent(nimcp_health_agent_t* agent) {
    g_neural_substrate_health_agent = agent;
}

/** @brief Send heartbeat from neural_substrate module */
static inline void neural_substrate_heartbeat(const char* operation, float progress) {
    if (g_neural_substrate_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_neural_substrate_health_agent, operation, progress);
    }
}
