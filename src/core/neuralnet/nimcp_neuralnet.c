...
/** Global health agent for neuralnet module */
static nimcp_health_agent_t* g_neuralnet_health_agent = NULL;

/**
 * @brief Set health agent for neuralnet heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void neuralnet_set_health_agent(nimcp_health_agent_t* agent) {
    g_neuralnet_health_agent = agent;
}

/** @brief Send heartbeat from neuralnet module */
static inline void neuralnet_heartbeat(const char* operation, float progress) {
    if (g_neuralnet_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_neuralnet_health_agent, operation, progress);
    }
}
