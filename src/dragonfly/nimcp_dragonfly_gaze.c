...
/** Global health agent for dragonfly_gaze module */
static nimcp_health_agent_t* g_dragonfly_gaze_health_agent = NULL;

/**
 * @brief Set health agent for dragonfly_gaze heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void dragonfly_gaze_set_health_agent(nimcp_health_agent_t* agent) {
    g_dragonfly_gaze_health_agent = agent;
}

/** @brief Send heartbeat from dragonfly_gaze module */
static inline void dragonfly_gaze_heartbeat(const char* operation, float progress) {
    if (g_dragonfly_gaze_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_dragonfly_gaze_health_agent, operation, progress);
    }
}
