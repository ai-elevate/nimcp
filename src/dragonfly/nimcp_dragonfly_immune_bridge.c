...
/** Global health agent for dragonfly_immune_bridge module */
static nimcp_health_agent_t* g_dragonfly_immune_bridge_health_agent = NULL;

/**
 * @brief Set health agent for dragonfly_immune_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void dragonfly_immune_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_dragonfly_immune_bridge_health_agent = agent;
}

/** @brief Send heartbeat from dragonfly_immune_bridge module */
static inline void dragonfly_immune_bridge_heartbeat(const char* operation, float progress) {
    if (g_dragonfly_immune_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_dragonfly_immune_bridge_health_agent, operation, progress);
    }
}

#define LOG_MODULE "DRAGONFLY_IMMUNE_BRIDGE"
...
void dragonfly_immune_bridge_destroy(dragonfly_immune_bridge_t bridge) {
    if (!bridge) return;
    NIMCP_LOGGING_DEBUG("Destroying %s bridge", "dragonfly_immune");
