...
/** Global health agent for dragonfly_medulla_bridge module */
static nimcp_health_agent_t* g_dragonfly_medulla_bridge_health_agent = NULL;

/**
 * @brief Set health agent for dragonfly_medulla_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void dragonfly_medulla_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_dragonfly_medulla_bridge_health_agent = agent;
}

/** @brief Send heartbeat from dragonfly_medulla_bridge module */
static inline void dragonfly_medulla_bridge_heartbeat(const char* operation, float progress) {
    if (g_dragonfly_medulla_bridge_health_agent) {
...
dragonfly_medulla_bridge_t dragonfly_medulla_bridge_create(
    ...
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NO_MEMORY,
            "dragonfly_medulla_bridge_create: failed to allocate bridge");
        return NULL;
    }
    ...
    if (bridge_base_init(&bridge->base, 0, "dragonfly_medulla") != 0) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_OPERATION_FAILED,
            "dragonfly_medulla_bridge_create: failed to initialize base bridge");
        nimcp_free(bridge);
        return NULL;
    }
