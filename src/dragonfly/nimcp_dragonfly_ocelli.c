...
/** Global health agent for dragonfly_ocelli module */
static nimcp_health_agent_t* g_dragonfly_ocelli_health_agent = NULL;

/**
 * @brief Set health agent for dragonfly_ocelli heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void dragonfly_ocelli_set_health_agent(nimcp_health_agent_t* agent) {
    g_dragonfly_ocelli_health_agent = agent;
}

/** @brief Send heartbeat from dragonfly_ocelli module */
static inline void dragonfly_ocelli_heartbeat(const char* operation, float progress) {
    if (g_dragonfly_ocelli_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_dragonfly_ocelli_health_agent, operation, progress);
    }
}
