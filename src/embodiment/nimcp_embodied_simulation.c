...
/** Global health agent for embodied_simulation module */
static nimcp_health_agent_t* g_embodied_simulation_health_agent = NULL;

/**
 * @brief Set health agent for embodied_simulation heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void embodied_simulation_set_health_agent(nimcp_health_agent_t* agent) {
    g_embodied_simulation_health_agent = agent;
}

/** @brief Send heartbeat from embodied_simulation module */
static inline void embodied_simulation_heartbeat(const char* operation, float progress) {
    if (g_embodied_simulation_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_embodied_simulation_health_agent, operation, progress);
    }
}
