...
/** Global health agent for astrocytes_immune_bridge module */
static nimcp_health_agent_t* g_astrocytes_immune_bridge_health_agent = NULL;

/**
 * @brief Set health agent for astrocytes_immune_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void astrocytes_immune_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_astrocytes_immune_bridge_health_agent = agent;
}

/** @brief Send heartbeat from astrocytes_immune_bridge module */
static inline void astrocytes_immune_bridge_heartbeat(const char* operation, float progress) {
    if (g_astrocytes_immune_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_astrocytes_immune_bridge_health_agent, operation, progress);
    }
}
...
{
    NIMCP_CHECK_THROW(bridge, NIMCP_ERROR_NULL_POINTER, "bridge is NULL");
    NIMCP_CHECK_THROW(bridge->base.mutex, NIMCP_ERROR_NULL_POINTER, "bridge mutex is NULL");
...
{
    NIMCP_CHECK_THROW(bridge, NIMCP_ERROR_NULL_POINTER, "bridge is NULL");
    NIMCP_CHECK_THROW(dt_ms > 0.0f && isfinite(dt_ms), NIMCP_ERROR_INVALID_PARAM, "dt_ms must be positive and finite");
    NIMCP_CHECK_THROW(bridge->base.mutex, NIMCP_ERROR_NULL_POINTER, "bridge mutex is NULL");
...
{
    NIMCP_CHECK_THROW(bridge, NIMCP_ERROR_NULL_POINTER, "bridge is NULL");
    NIMCP_CHECK_THROW(bridge->initialized, NIMCP_ERROR_INVALID_PARAM, "bridge not initialized");
    NIMCP_CHECK_THROW(dt_ms > 0.0f && isfinite(dt_ms), NIMCP_ERROR_INVALID_PARAM, "dt_ms must be positive and finite");
...
int astro_cell_get_stats(const astro_immune_bridge_t* bridge, astro_immune_stats_t* stats)
{
    ...
    NIMCP_CHECK_THROW(bridge->base.mutex, NIMCP_ERROR_INVALID_STATE, "bridge mutex is NULL");
