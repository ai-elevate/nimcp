...
/** Global health agent for myelin_sheath module */
static nimcp_health_agent_t* g_myelin_sheath_health_agent = NULL;

/**
 * @brief Set health agent for myelin_sheath heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void myelin_sheath_set_health_agent(nimcp_health_agent_t* agent) {
    g_myelin_sheath_health_agent = agent;
}

/** @brief Send heartbeat from myelin_sheath module */
static inline void myelin_sheath_heartbeat(const char* operation, float progress) {
    if (g_myelin_sheath_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_myelin_sheath_health_agent, operation, progress);
    }
}
