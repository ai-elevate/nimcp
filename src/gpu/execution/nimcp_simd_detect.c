...
/** Global health agent for simd_detect module */
static nimcp_health_agent_t* g_simd_detect_health_agent = NULL;

/**
 * @brief Set health agent for simd_detect heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void simd_detect_set_health_agent(nimcp_health_agent_t* agent) {
    g_simd_detect_health_agent = agent;
}

/** @brief Send heartbeat from simd_detect module */
static inline void simd_detect_heartbeat(const char* operation, float progress) {
    if (g_simd_detect_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_simd_detect_health_agent, operation, progress);
    }
}
