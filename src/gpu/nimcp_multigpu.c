...
/** Global health agent for multigpu module */
static nimcp_health_agent_t* g_multigpu_health_agent = NULL;

/**
 * @brief Set health agent for multigpu heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void multigpu_set_health_agent(nimcp_health_agent_t* agent) {
    g_multigpu_health_agent = agent;
}

/** @brief Send heartbeat from multigpu module */
static inline void multigpu_heartbeat(const char* operation, float progress) {
    if (g_multigpu_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_multigpu_health_agent, operation, progress);
    }
}
