...
/** Global health agent for substrate_gpu module */
static nimcp_health_agent_t* g_substrate_gpu_health_agent = NULL;

/**
 * @brief Set health agent for substrate_gpu heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void substrate_gpu_set_health_agent(nimcp_health_agent_t* agent) {
    g_substrate_gpu_health_agent = agent;
}

/** @brief Send heartbeat from substrate_gpu module */
static inline void substrate_gpu_heartbeat(const char* operation, float progress) {
    if (g_substrate_gpu_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_substrate_gpu_health_agent, operation, progress);
    }
}
