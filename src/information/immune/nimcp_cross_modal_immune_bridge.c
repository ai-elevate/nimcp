...
/** Global health agent for cross_modal_immune_bridge module */
static nimcp_health_agent_t* g_cross_modal_immune_bridge_health_agent = NULL;

/**
 * @brief Set health agent for cross_modal_immune_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void cross_modal_immune_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_cross_modal_immune_bridge_health_agent = agent;
}

/** @brief Send heartbeat from cross_modal_immune_bridge module */
static inline void cross_modal_immune_bridge_heartbeat(const char* operation, float progress) {
    if (g_cross_modal_immune_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_cross_modal_immune_bridge_health_agent, operation, progress);
    }
}
...
static float get_inflammation_transfer_factor(brain_inflammation_level_t level) {
    switch (level) {
        case INFLAMMATION_NONE:     return INFLAMMATION_NONE_TRANSFER_FACTOR;
        case INFLAMMATION_LOCAL:    return INFLAMMATION_LOCAL_TRANSFER_FACTOR;
        case INFLAMMATION_REGIONAL: return INFLAMMATION_REGIONAL_TRANSFER_FACTOR;
        case INFLAMMATION_SYSTEMIC: return INFLAMMATION_SYSTEMIC_TRANSFER_FACTOR;
        case INFLAMMATION_STORM:    return INFLAMMATION_STORM_TRANSFER_FACTOR;
        default:                    return 1.0f;
    }
}
