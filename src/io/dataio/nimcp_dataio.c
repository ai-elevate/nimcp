...
/** Global health agent for dataio module */
static nimcp_health_agent_t* g_dataio_health_agent = NULL;

/**
 * @brief Set health agent for dataio heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void dataio_set_health_agent(nimcp_health_agent_t* agent) {
    g_dataio_health_agent = agent;
}

/** @brief Send heartbeat from dataio module */
static inline void dataio_heartbeat(const char* operation, float progress) {
    if (g_dataio_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_dataio_health_agent, operation, progress);
    }
}
