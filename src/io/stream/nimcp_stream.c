...
/** Global health agent for stream module */
static nimcp_health_agent_t* g_stream_health_agent = NULL;

/**
 * @brief Set health agent for stream heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void stream_set_health_agent(nimcp_health_agent_t* agent) {
    g_stream_health_agent = agent;
}

/** @brief Send heartbeat from stream module */
static inline void stream_heartbeat(const char* operation, float progress) {
    if (g_stream_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_stream_health_agent, operation, progress);
    }
}
