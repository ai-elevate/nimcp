...
/** Global health agent for language_immune_bridge module */
static nimcp_health_agent_t* g_language_immune_bridge_health_agent = NULL;

/**
 * @brief Set health agent for language_immune_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void language_immune_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_language_immune_bridge_health_agent = agent;
}

/** @brief Send heartbeat from language_immune_bridge module */
static inline void language_immune_bridge_heartbeat(const char* operation, float progress) {
    if (g_language_immune_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_language_immune_bridge_health_agent, operation, progress);
    }
}
    ...
    const language_immune_config_t* config)
{
    ...
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NO_MEMORY, "language_immune_bridge_create: allocation failed");
