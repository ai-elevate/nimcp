...
/** Global health agent for language_config module */
static nimcp_health_agent_t* g_language_config_health_agent = NULL;

/**
 * @brief Set health agent for language_config heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void language_config_set_health_agent(nimcp_health_agent_t* agent) {
    g_language_config_health_agent = agent;
}

/** @brief Send heartbeat from language_config module */
static inline void language_config_heartbeat(const char* operation, float progress) {
    if (g_language_config_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_language_config_health_agent, operation, progress);
    }
}
