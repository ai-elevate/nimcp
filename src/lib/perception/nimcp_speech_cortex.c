...
/** Global health agent for speech_cortex module */
static nimcp_health_agent_t* g_speech_cortex_health_agent = NULL;

/**
 * @brief Set health agent for speech_cortex heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void speech_cortex_set_health_agent(nimcp_health_agent_t* agent) {
    g_speech_cortex_health_agent = agent;
}

/** @brief Send heartbeat from speech_cortex module */
static inline void speech_cortex_heartbeat(const char* operation, float progress) {
    if (g_speech_cortex_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_speech_cortex_health_agent, operation, progress);
    }
}
