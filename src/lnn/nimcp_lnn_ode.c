...
/** Global health agent for lnn_ode module */
static nimcp_health_agent_t* g_lnn_ode_health_agent = NULL;

/**
 * @brief Set health agent for lnn_ode heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void lnn_ode_set_health_agent(nimcp_health_agent_t* agent) {
    g_lnn_ode_health_agent = agent;
}

/** @brief Send heartbeat from lnn_ode module */
static inline void lnn_ode_heartbeat(const char* operation, float progress) {
    if (g_lnn_ode_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_lnn_ode_health_agent, operation, progress);
    }
}
