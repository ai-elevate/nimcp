...
sequence_immune_bridge_t* sequence_immune_bridge_create(
    ...
    if (!immune_system || !sequence_detector) {
        nimcp_log(NIMCP_LOG_ERROR, "sequence_immune_bridge",
    ...
    if (!bridge) {
        nimcp_log(NIMCP_LOG_ERROR, "sequence_immune_bridge", "Allocation failed");
        return NULL;
    }
    ...
    nimcp_log(NIMCP_LOG_INFO, "sequence_immune_bridge", "Bridge created successfully");
    return bridge;
}
...
void sequence_immune_bridge_destroy(sequence_immune_bridge_t* bridge) {
    ...
    nimcp_free(bridge);
    nimcp_log(NIMCP_LOG_INFO, "sequence_immune_bridge", "Bridge destroyed");
}
