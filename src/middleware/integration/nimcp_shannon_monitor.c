...
/** Global health agent for shannon_monitor module */
static nimcp_health_agent_t* g_shannon_monitor_health_agent = NULL;

/**
 * @brief Set health agent for shannon_monitor heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void shannon_monitor_set_health_agent(nimcp_health_agent_t* agent) {
    g_shannon_monitor_health_agent = agent;
}

/** @brief Send heartbeat from shannon_monitor module */
static inline void shannon_monitor_heartbeat(const char* operation, float progress) {
    if (g_shannon_monitor_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_shannon_monitor_health_agent, operation, progress);
    }
}
