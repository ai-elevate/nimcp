...
/** Global health agent for middleware_context module */
static nimcp_health_agent_t* g_middleware_context_health_agent = NULL;

/**
 * @brief Set health agent for middleware_context heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void middleware_context_set_health_agent(nimcp_health_agent_t* agent) {
    g_middleware_context_health_agent = agent;
}

/** @brief Send heartbeat from middleware_context module */
static inline void middleware_context_heartbeat(const char* operation, float progress) {
    if (g_middleware_context_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_middleware_context_health_agent, operation, progress);
    }
}
