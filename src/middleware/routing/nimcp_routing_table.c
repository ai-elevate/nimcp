...
/** Global health agent for routing_table module */
static nimcp_health_agent_t* g_routing_table_health_agent = NULL;

/**
 * @brief Set health agent for routing_table heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void routing_table_set_health_agent(nimcp_health_agent_t* agent) {
    g_routing_table_health_agent = agent;
}

/** @brief Send heartbeat from routing_table module */
static inline void routing_table_heartbeat(const char* operation, float progress) {
    if (g_routing_table_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_routing_table_health_agent, operation, progress);
    }
}
