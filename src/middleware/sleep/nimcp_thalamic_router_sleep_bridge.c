...
/** Global health agent for thalamic_router_sleep_bridge module */
static nimcp_health_agent_t* g_thalamic_router_sleep_bridge_health_agent = NULL;

/**
 * @brief Set health agent for thalamic_router_sleep_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void thalamic_router_sleep_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_thalamic_router_sleep_bridge_health_agent = agent;
}

/** @brief Send heartbeat from thalamic_router_sleep_bridge module */
static inline void thalamic_router_sleep_bridge_heartbeat(const char* operation, float progress) {
    if (g_thalamic_router_sleep_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_thalamic_router_sleep_bridge_health_agent, operation, progress);
    }
}
    ...
    sleep_system_t sleep_system)
{
    ...
    if (bridge_base_init(&bridge->base, 0, "thalamic_router_sleep") != 0) { nimcp_free(bridge); return NULL; }
