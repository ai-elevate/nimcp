...
/** Global health agent for lr_scheduler module */
static nimcp_health_agent_t* g_lr_scheduler_health_agent = NULL;

/**
 * @brief Set health agent for lr_scheduler heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void lr_scheduler_set_health_agent(nimcp_health_agent_t* agent) {
    g_lr_scheduler_health_agent = agent;
}

/** @brief Send heartbeat from lr_scheduler module */
static inline void lr_scheduler_heartbeat(const char* operation, float progress) {
    if (g_lr_scheduler_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_lr_scheduler_health_agent, operation, progress);
    }
}
