...
/** Global health agent for omni_training_bridge module */
static nimcp_health_agent_t* g_omni_training_bridge_health_agent = NULL;

/**
 * @brief Set health agent for omni_training_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void omni_training_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_omni_training_bridge_health_agent = agent;
}

/** @brief Send heartbeat from omni_training_bridge module */
static inline void omni_training_bridge_heartbeat(const char* operation, float progress) {
    if (g_omni_training_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_omni_training_bridge_health_agent, operation, progress);
    }
}
