...
/** Global health agent for regularization module */
static nimcp_health_agent_t* g_regularization_health_agent = NULL;

/**
 * @brief Set health agent for regularization heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void regularization_set_health_agent(nimcp_health_agent_t* agent) {
    g_regularization_health_agent = agent;
}

/** @brief Send heartbeat from regularization module */
static inline void regularization_heartbeat(const char* operation, float progress) {
    if (g_regularization_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_regularization_health_agent, operation, progress);
    }
}
