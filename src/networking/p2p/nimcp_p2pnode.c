...
/** Global health agent for p2pnode module */
static nimcp_health_agent_t* g_p2pnode_health_agent = NULL;

/**
 * @brief Set health agent for p2pnode heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void p2pnode_set_health_agent(nimcp_health_agent_t* agent) {
    g_p2pnode_health_agent = agent;
}

/** @brief Send heartbeat from p2pnode module */
static inline void p2pnode_heartbeat(const char* operation, float progress) {
    if (g_p2pnode_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_p2pnode_health_agent, operation, progress);
    }
}
