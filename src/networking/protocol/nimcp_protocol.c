...
/** Global health agent for protocol module */
static nimcp_health_agent_t* g_protocol_health_agent = NULL;

/**
 * @brief Set health agent for protocol heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void protocol_set_health_agent(nimcp_health_agent_t* agent) {
    g_protocol_health_agent = agent;
}

/** @brief Send heartbeat from protocol module */
static inline void protocol_heartbeat(const char* operation, float progress) {
    if (g_protocol_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_protocol_health_agent, operation, progress);
    }
}
