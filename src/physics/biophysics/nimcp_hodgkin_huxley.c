...
/** Global health agent for hodgkin_huxley module */
static nimcp_health_agent_t* g_hodgkin_huxley_health_agent = NULL;

/**
 * @brief Set health agent for hodgkin_huxley heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void hodgkin_huxley_set_health_agent(nimcp_health_agent_t* agent) {
    g_hodgkin_huxley_health_agent = agent;
}

/** @brief Send heartbeat from hodgkin_huxley module */
static inline void hodgkin_huxley_heartbeat(const char* operation, float progress) {
    if (g_hodgkin_huxley_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_hodgkin_huxley_health_agent, operation, progress);
    }
}
