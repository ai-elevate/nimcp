...
/** Global health agent for bcm module */
static nimcp_health_agent_t* g_bcm_health_agent = NULL;

/**
 * @brief Set health agent for bcm heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void bcm_set_health_agent(nimcp_health_agent_t* agent) {
    g_bcm_health_agent = agent;
}

/** @brief Send heartbeat from bcm module */
static inline void bcm_heartbeat(const char* operation, float progress) {
    if (g_bcm_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_bcm_health_agent, operation, progress);
    }
}
