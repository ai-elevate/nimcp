...
/** Global health agent for calcium_sleep_bridge module */
static nimcp_health_agent_t* g_calcium_sleep_bridge_health_agent = NULL;

/**
 * @brief Set health agent for calcium_sleep_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void calcium_sleep_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_calcium_sleep_bridge_health_agent = agent;
}

/** @brief Send heartbeat from calcium_sleep_bridge module */
static inline void calcium_sleep_bridge_heartbeat(const char* operation, float progress) {
    if (g_calcium_sleep_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_calcium_sleep_bridge_health_agent, operation, progress);
    }
}
