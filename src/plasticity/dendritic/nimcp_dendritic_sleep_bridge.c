...
/** Global health agent for dendritic_sleep_bridge module */
static nimcp_health_agent_t* g_dendritic_sleep_bridge_health_agent = NULL;

/**
 * @brief Set health agent for dendritic_sleep_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void dendritic_sleep_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_dendritic_sleep_bridge_health_agent = agent;
}

/** @brief Send heartbeat from dendritic_sleep_bridge module */
static inline void dendritic_sleep_bridge_heartbeat(const char* operation, float progress) {
    if (g_dendritic_sleep_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_dendritic_sleep_bridge_health_agent, operation, progress);
    }
}
...
BRIDGE_DEFINE_SECURITY_SETTERS_TYPE(dendritic_sleep_bridge, struct dendritic_sleep_bridge_struct)
