...
/** Global health agent for heterosynaptic module */
static nimcp_health_agent_t* g_heterosynaptic_health_agent = NULL;

/**
 * @brief Set health agent for heterosynaptic heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void heterosynaptic_set_health_agent(nimcp_health_agent_t* agent) {
    g_heterosynaptic_health_agent = agent;
}

/** @brief Send heartbeat from heterosynaptic module */
static inline void heterosynaptic_heartbeat(const char* operation, float progress) {
    if (g_heterosynaptic_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_heterosynaptic_health_agent, operation, progress);
    }
}
