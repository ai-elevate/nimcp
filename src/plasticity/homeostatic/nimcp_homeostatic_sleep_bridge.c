...
/** Global health agent for homeostatic_sleep_bridge module */
static nimcp_health_agent_t* g_homeostatic_sleep_bridge_health_agent = NULL;

/**
 * @brief Set health agent for homeostatic_sleep_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void homeostatic_sleep_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_homeostatic_sleep_bridge_health_agent = agent;
}

/** @brief Send heartbeat from homeostatic_sleep_bridge module */
static inline void homeostatic_sleep_bridge_heartbeat(const char* operation, float progress) {
    if (g_homeostatic_sleep_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_homeostatic_sleep_bridge_health_agent, operation, progress);
    }
}
...
int homeostatic_sleep_default_config(homeostatic_sleep_config_t* config) {
    NIMCP_API_CHECK_NULL(config, -1, "Homeostatic-sleep config is NULL");
    ...
    sleep_system_t sleep_system)
{
    ...
    NIMCP_API_CHECK_ALLOC(bridge, "Homeostatic-sleep bridge allocation failed");
