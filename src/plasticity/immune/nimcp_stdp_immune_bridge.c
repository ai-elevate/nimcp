...
/** Global health agent for stdp_immune_bridge module */
static nimcp_health_agent_t* g_stdp_immune_bridge_health_agent = NULL;

/**
 * @brief Set health agent for stdp_immune_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void stdp_immune_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_stdp_immune_bridge_health_agent = agent;
}

/** @brief Send heartbeat from stdp_immune_bridge module */
static inline void stdp_immune_bridge_heartbeat(const char* operation, float progress) {
    if (g_stdp_immune_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_stdp_immune_bridge_health_agent, operation, progress);
    }
}

/* Security integration */
BRIDGE_DEFINE_SECURITY_SETTERS(stdp_immune_bridge)
...
stdp_immune_bridge_t* stdp_immune_bridge_create(
    ...
    /* Create mutex */
    if (bridge_base_init(&bridge->base, 0, "stdp_immune") != 0) { nimcp_free(bridge); return NULL; }
