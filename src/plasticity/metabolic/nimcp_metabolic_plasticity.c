...
/** Global health agent for metabolic_plasticity module */
static nimcp_health_agent_t* g_metabolic_plasticity_health_agent = NULL;

/**
 * @brief Set health agent for metabolic_plasticity heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void metabolic_plasticity_set_health_agent(nimcp_health_agent_t* agent) {
    g_metabolic_plasticity_health_agent = agent;
}

/** @brief Send heartbeat from metabolic_plasticity module */
static inline void metabolic_plasticity_heartbeat(const char* operation, float progress) {
    if (g_metabolic_plasticity_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_metabolic_plasticity_health_agent, operation, progress);
    }
}
