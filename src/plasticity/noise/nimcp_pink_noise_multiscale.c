...
/** Global health agent for pink_noise_multiscale module */
static nimcp_health_agent_t* g_pink_noise_multiscale_health_agent = NULL;

/**
 * @brief Set health agent for pink_noise_multiscale heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void pink_noise_multiscale_set_health_agent(nimcp_health_agent_t* agent) {
    g_pink_noise_multiscale_health_agent = agent;
}

/** @brief Send heartbeat from pink_noise_multiscale module */
static inline void pink_noise_multiscale_heartbeat(const char* operation, float progress) {
    if (g_pink_noise_multiscale_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_pink_noise_multiscale_health_agent, operation, progress);
    }
}
