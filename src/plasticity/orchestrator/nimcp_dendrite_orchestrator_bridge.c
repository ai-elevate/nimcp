...
/** Global health agent for dendrite_orchestrator_bridge module */
static nimcp_health_agent_t* g_dendrite_orchestrator_bridge_health_agent = NULL;

/**
 * @brief Set health agent for dendrite_orchestrator_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void dendrite_orchestrator_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_dendrite_orchestrator_bridge_health_agent = agent;
}

/** @brief Send heartbeat from dendrite_orchestrator_bridge module */
static inline void dendrite_orchestrator_bridge_heartbeat(const char* operation, float progress) {
    if (g_dendrite_orchestrator_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_dendrite_orchestrator_bridge_health_agent, operation, progress);
    }
}
