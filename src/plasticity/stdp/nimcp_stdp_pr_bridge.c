...
/** Global health agent for stdp_pr_bridge module */
static nimcp_health_agent_t* g_stdp_pr_bridge_health_agent = NULL;

/**
 * @brief Set health agent for stdp_pr_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void stdp_pr_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_stdp_pr_bridge_health_agent = agent;
}

/** @brief Send heartbeat from stdp_pr_bridge module */
static inline void stdp_pr_bridge_heartbeat(const char* operation, float progress) {
    if (g_stdp_pr_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_stdp_pr_bridge_health_agent, operation, progress);
    }
}
