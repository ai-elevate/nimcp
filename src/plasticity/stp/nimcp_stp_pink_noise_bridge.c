...
/** Global health agent for stp_pink_noise_bridge module */
static nimcp_health_agent_t* g_stp_pink_noise_bridge_health_agent = NULL;

/**
 * @brief Set health agent for stp_pink_noise_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void stp_pink_noise_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_stp_pink_noise_bridge_health_agent = agent;
}

/** @brief Send heartbeat from stp_pink_noise_bridge module */
static inline void stp_pink_noise_bridge_heartbeat(const char* operation, float progress) {
    if (g_stp_pink_noise_bridge_health_agent) {
...
int stp_pink_noise_default_config(stp_pink_noise_config_t* config) {
    NIMCP_API_CHECK_NULL(config, -1, "STP-pink noise config is NULL");
