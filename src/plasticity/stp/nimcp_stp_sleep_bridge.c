...
/** Global health agent for stp_sleep_bridge module */
static nimcp_health_agent_t* g_stp_sleep_bridge_health_agent = NULL;

/**
 * @brief Set health agent for stp_sleep_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void stp_sleep_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_stp_sleep_bridge_health_agent = agent;
}

/** @brief Send heartbeat from stp_sleep_bridge module */
static inline void stp_sleep_bridge_heartbeat(const char* operation, float progress) {
    if (g_stp_sleep_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_stp_sleep_bridge_health_agent, operation, progress);
    }
}
