...
/** Global health agent for portia_learning_immune_bridge module */
static nimcp_health_agent_t* g_portia_learning_immune_bridge_health_agent = NULL;

/**
 * @brief Set health agent for portia_learning_immune_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void portia_learning_immune_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_portia_learning_immune_bridge_health_agent = agent;
}

/** @brief Send heartbeat from portia_learning_immune_bridge module */
static inline void portia_learning_immune_bridge_heartbeat(const char* operation, float progress) {
    if (g_portia_learning_immune_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_portia_learning_immune_bridge_health_agent, operation, progress);
    }
}
...
portia_learning_immune_bridge_t* portia_learning_immune_create(
    ...
    /* Create mutex */
    if (bridge_base_init(&bridge->base, 0, "portia_learning_immune") != 0) { nimcp_free(bridge); return NULL; }
