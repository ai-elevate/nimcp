...
/** Global health agent for portia_attention module */
static nimcp_health_agent_t* g_portia_attention_health_agent = NULL;

/**
 * @brief Set health agent for portia_attention heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void __attribute__((unused)) portia_attention_set_health_agent(nimcp_health_agent_t* agent) {
    g_portia_attention_health_agent = agent;
}

/** @brief Send heartbeat from portia_attention module */
static inline void portia_attention_heartbeat(const char* operation, float progress) {
    if (g_portia_attention_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_portia_attention_health_agent, operation, progress);
    }
}
