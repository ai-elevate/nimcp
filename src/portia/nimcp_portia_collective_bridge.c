...
/** Global health agent for portia_collective_bridge module */
static nimcp_health_agent_t* g_portia_collective_bridge_health_agent = NULL;

/**
 * @brief Set health agent for portia_collective_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void __attribute__((unused)) portia_collective_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_portia_collective_bridge_health_agent = agent;
}

/** @brief Send heartbeat from portia_collective_bridge module */
static inline void portia_collective_bridge_heartbeat(const char* operation, float progress) {
    if (g_portia_collective_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_portia_collective_bridge_health_agent, operation, progress);
    }
}
