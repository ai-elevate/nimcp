...
/** Global health agent for portia_monitoring module */
static nimcp_health_agent_t* g_portia_monitoring_health_agent = NULL;

/**
 * @brief Set health agent for portia_monitoring heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void __attribute__((unused)) portia_monitoring_set_health_agent(nimcp_health_agent_t* agent) {
    g_portia_monitoring_health_agent = agent;
}

/** @brief Send heartbeat from portia_monitoring module */
static inline void portia_monitoring_heartbeat(const char* operation, float progress) {
    if (g_portia_monitoring_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_portia_monitoring_health_agent, operation, progress);
    }
}
...
void portia_monitor_shutdown(portia_monitor_t monitor) {
    ...
    // Destroy mutex
    if (monitor->mutex_init) {
        nimcp_platform_mutex_destroy(&monitor->mutex);
    }
...
float portia_monitor_get_cpu_temp(portia_monitor_t monitor) {
    ...
    nimcp_platform_mutex_lock(&monitor->mutex);
