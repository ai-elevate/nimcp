...
/** Global health agent for portia_planning module */
static nimcp_health_agent_t* g_portia_planning_health_agent = NULL;
...
/** @brief Send heartbeat from portia_planning module */
static inline void portia_planning_heartbeat(const char* operation, float progress) {
    if (g_portia_planning_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_portia_planning_health_agent, operation, progress);
    }
}
