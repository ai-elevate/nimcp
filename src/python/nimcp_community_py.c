...
/** Global health agent for community_py module */
static nimcp_health_agent_t* g_community_py_health_agent = NULL;

/**
 * @brief Set health agent for community_py heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void community_py_set_health_agent(nimcp_health_agent_t* agent) {
    g_community_py_health_agent = agent;
}

/** @brief Send heartbeat from community_py module */
static inline void community_py_heartbeat(const char* operation, float progress) {
    if (g_community_py_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_community_py_health_agent, operation, progress);
    }
}
