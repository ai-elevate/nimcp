...
/** Global health agent for training_py module */
static nimcp_health_agent_t* g_training_py_health_agent = NULL;

/**
 * @brief Set health agent for training_py heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void training_py_set_health_agent(nimcp_health_agent_t* agent) {
    g_training_py_health_agent = agent;
}

/** @brief Send heartbeat from training_py module */
static inline void training_py_heartbeat(const char* operation, float progress) {
    if (g_training_py_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_training_py_health_agent, operation, progress);
    }
}
