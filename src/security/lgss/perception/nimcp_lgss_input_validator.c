...
/** Global health agent for lgss_input_validator module */
static nimcp_health_agent_t* g_lgss_input_validator_health_agent = NULL;

/**
 * @brief Set health agent for lgss_input_validator heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void lgss_input_validator_set_health_agent(nimcp_health_agent_t* agent) {
    g_lgss_input_validator_health_agent = agent;
}

/** @brief Send heartbeat from lgss_input_validator module */
static inline void lgss_input_validator_heartbeat(const char* operation, float progress) {
    if (g_lgss_input_validator_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_lgss_input_validator_health_agent, operation, progress);
    }
}
