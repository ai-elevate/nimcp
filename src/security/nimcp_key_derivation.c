...
/** Global health agent for key_derivation module */
static nimcp_health_agent_t* g_key_derivation_health_agent = NULL;

/**
 * @brief Set health agent for key_derivation heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void key_derivation_set_health_agent(nimcp_health_agent_t* agent) {
    g_key_derivation_health_agent = agent;
}

/** @brief Send heartbeat from key_derivation module */
static inline void key_derivation_heartbeat(const char* operation, float progress) {
    if (g_key_derivation_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_key_derivation_health_agent, operation, progress);
    }
}
