...
/** Global health agent for policy_ast module */
static nimcp_health_agent_t* g_policy_ast_health_agent = NULL;

/**
 * @brief Set health agent for policy_ast heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void policy_ast_set_health_agent(nimcp_health_agent_t* agent) {
    g_policy_ast_health_agent = agent;
}

/** @brief Send heartbeat from policy_ast module */
static inline void policy_ast_heartbeat(const char* operation, float progress) {
    if (g_policy_ast_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_policy_ast_health_agent, operation, progress);
    }
}
