...
/** Global health agent for security_orchestrator module */
static nimcp_health_agent_t* g_security_orchestrator_health_agent = NULL;

/**
 * @brief Set health agent for security_orchestrator heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void security_orchestrator_set_health_agent(nimcp_health_agent_t* agent) {
    g_security_orchestrator_health_agent = agent;
}

/** @brief Send heartbeat from security_orchestrator module */
static inline void security_orchestrator_heartbeat(const char* operation, float progress) {
    if (g_security_orchestrator_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_security_orchestrator_health_agent, operation, progress);
    }
}
