...
/** Global health agent for shadow_stack module */
static nimcp_health_agent_t* g_shadow_stack_health_agent = NULL;

/**
 * @brief Set health agent for shadow_stack heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void shadow_stack_set_health_agent(nimcp_health_agent_t* agent) {
    g_shadow_stack_health_agent = agent;
}

/** @brief Send heartbeat from shadow_stack module */
static inline void shadow_stack_heartbeat(const char* operation, float progress) {
    if (g_shadow_stack_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_shadow_stack_health_agent, operation, progress);
    }
}
