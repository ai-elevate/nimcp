...
/** Global health agent for snn_attention_bridge module */
static nimcp_health_agent_t* g_snn_attention_bridge_health_agent = NULL;

/**
 * @brief Set health agent for snn_attention_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void snn_attention_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_snn_attention_bridge_health_agent = agent;
}

/** @brief Send heartbeat from snn_attention_bridge module */
static inline void snn_attention_bridge_heartbeat(const char* operation, float progress) {
    if (g_snn_attention_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_snn_attention_bridge_health_agent, operation, progress);
    }
}
