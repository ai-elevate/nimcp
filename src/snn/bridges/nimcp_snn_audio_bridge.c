...
/** Global health agent for snn_audio_bridge module */
static nimcp_health_agent_t* g_snn_audio_bridge_health_agent = NULL;

/**
 * @brief Set health agent for snn_audio_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void snn_audio_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_snn_audio_bridge_health_agent = agent;
}

/** @brief Send heartbeat from snn_audio_bridge module */
static inline void snn_audio_bridge_heartbeat(const char* operation, float progress) {
    if (g_snn_audio_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_snn_audio_bridge_health_agent, operation, progress);
    }
}
