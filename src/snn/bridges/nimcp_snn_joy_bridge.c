...
/** Global health agent for snn_joy_bridge module */
static nimcp_health_agent_t* g_snn_joy_bridge_health_agent = NULL;

/**
 * @brief Set health agent for snn_joy_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void snn_joy_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_snn_joy_bridge_health_agent = agent;
}

/** @brief Send heartbeat from snn_joy_bridge module */
static inline void snn_joy_bridge_heartbeat(const char* operation, float progress) {
    if (g_snn_joy_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_snn_joy_bridge_health_agent, operation, progress);
    }
}
...
int snn_joy_bridge_disconnect_bio_async(snn_joy_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                             "snn_joy_bridge_disconnect_bio_async: bridge is NULL");
        return SNN_ERROR_NULL_POINTER;
    }
    if (!bridge->base.bio_async_enabled) return 0;
...
bool snn_joy_bridge_is_bio_async_connected(const snn_joy_bridge_t* bridge) {
    if (!bridge) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER,
                             "snn_joy_bridge_is_bio_async_connected: bridge is NULL");
        return false;
    }
