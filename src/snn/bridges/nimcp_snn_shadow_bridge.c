...
/** Global health agent for snn_shadow_bridge module */
static nimcp_health_agent_t* g_snn_shadow_bridge_health_agent = NULL;

/**
 * @brief Set health agent for snn_shadow_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void snn_shadow_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_snn_shadow_bridge_health_agent = agent;
}

/** @brief Send heartbeat from snn_shadow_bridge module */
static inline void snn_shadow_bridge_heartbeat(const char* operation, float progress) {
    if (g_snn_shadow_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_snn_shadow_bridge_health_agent, operation, progress);
    }
}
...
snn_shadow_bridge_t* snn_shadow_bridge_create(
    ...
    if (!config || !snn) {
        ...
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "snn_shadow_bridge_create: config/snn is NULL");
        return NULL;
    }
    ...
    if (!bridge) {
        ...
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NO_MEMORY, "snn_shadow_bridge_create: failed to allocate bridge");
        return NULL;
    }
