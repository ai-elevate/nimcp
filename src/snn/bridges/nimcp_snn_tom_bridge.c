...
/** Global health agent for snn_tom_bridge module */
static nimcp_health_agent_t* g_snn_tom_bridge_health_agent = NULL;

/**
 * @brief Set health agent for snn_tom_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void snn_tom_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_snn_tom_bridge_health_agent = agent;
}

/** @brief Send heartbeat from snn_tom_bridge module */
static inline void snn_tom_bridge_heartbeat(const char* operation, float progress) {
    if (g_snn_tom_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_snn_tom_bridge_health_agent, operation, progress);
    }
}
...
snn_tom_bridge_t* snn_tom_bridge_create(
    ...
    if (!config) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "snn_tom_bridge_create: config is NULL");
        return NULL;
    }
    if (!snn) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "snn_tom_bridge_create: snn is NULL");
        return NULL;
    }
