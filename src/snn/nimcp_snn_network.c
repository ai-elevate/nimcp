...
int snn_network_set_inputs(snn_network_t* network,
                           const float* inputs,
                           uint32_t n_inputs) {
    if (!network || !inputs) return SNN_ERROR_NULL_POINTER;
    if (!network->input_pop) return SNN_ERROR_INVALID_STATE;
...
int snn_network_set_input_tensor(snn_network_t* network,
    ...
    if (!network || !input) return SNN_ERROR_NULL_POINTER;
...
int snn_network_get_outputs(snn_network_t* network,
                            float* outputs,
                            uint32_t n_outputs) {
    if (!network || !outputs) return SNN_ERROR_NULL_POINTER;
    if (!network->output_pop) return SNN_ERROR_INVALID_STATE;
...
int snn_network_get_output_tensor(snn_network_t* network,
    ...
    if (!network || !output) return SNN_ERROR_NULL_POINTER;
...
int snn_network_apply_stdp(snn_network_t* network) {
    if (!network) return SNN_ERROR_NULL_POINTER;
    if (!network->neural_net) return SNN_ERROR_INVALID_STATE;
...
int snn_network_apply_rstdp(snn_network_t* network, float reward) {
    if (!network) return SNN_ERROR_NULL_POINTER;
    if (!network->train_ctx) return SNN_ERROR_INVALID_STATE;
...
int snn_network_compute_gradients(snn_network_t* network,
                                  const float* target,
                                  uint32_t n_targets) {
    if (!network || !target) return SNN_ERROR_NULL_POINTER;
    if (!network->train_ctx) return SNN_ERROR_INVALID_STATE;
...
float snn_network_train_step(snn_network_t* network,
                             ...
                             uint32_t n_targets,
                             float duration_ms) {
    if (!network || !inputs || !targets) return -1.0f;
...
int snn_network_add_population(snn_network_t* network,
                               ...
                               const char* name) {
    if (!network) return SNN_ERROR_NULL_POINTER;
    if (n_neurons == 0) return SNN_ERROR_INVALID_DIMENSION;
...
int snn_network_connect_populations(snn_network_t* network,
    ...
    if (!src || !dst) return SNN_ERROR_INVALID_POPULATION;
...
snn_population_t* snn_network_get_population(snn_network_t* network,
    ...
    if (pop_id >= network->n_populations) return NULL;
...
int snn_network_get_stats(snn_network_t* network, snn_stats_t* stats) {
    if (!network || !stats) return SNN_ERROR_NULL_POINTER;
...
snn_state_health_t snn_network_check_health(snn_network_t* network) {
    if (!network) return SNN_STATE_SILENT;
...
float snn_network_get_firing_rate(snn_network_t* network,
    ...
    if (pop_id >= network->n_populations) return 0.0f;
