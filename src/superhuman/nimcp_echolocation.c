...
/** Global health agent for echolocation module */
static nimcp_health_agent_t* g_echolocation_health_agent = NULL;

/**
 * @brief Set health agent for echolocation heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void echolocation_set_health_agent(nimcp_health_agent_t* agent) {
    g_echolocation_health_agent = agent;
}

/** @brief Send heartbeat from echolocation module */
static inline void echolocation_heartbeat(const char* operation, float progress) {
    if (g_echolocation_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_echolocation_health_agent, operation, progress);
    }
}
