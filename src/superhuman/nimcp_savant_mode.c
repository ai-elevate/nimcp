...
/** Global health agent for savant_mode module */
static nimcp_health_agent_t* g_savant_mode_health_agent = NULL;

/**
 * @brief Set health agent for savant_mode heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void savant_mode_set_health_agent(nimcp_health_agent_t* agent) {
    g_savant_mode_health_agent = agent;
}

/** @brief Send heartbeat from savant_mode module */
static inline void savant_mode_heartbeat(const char* operation, float progress) {
    if (g_savant_mode_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_savant_mode_health_agent, operation, progress);
    }
}
