...
/** Global health agent for swarm_brain_immune_bridge module */
static nimcp_health_agent_t* g_swarm_brain_immune_bridge_health_agent = NULL;

/**
 * @brief Set health agent for swarm_brain_immune_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void swarm_brain_immune_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_swarm_brain_immune_bridge_health_agent = agent;
}

/** @brief Send heartbeat from swarm_brain_immune_bridge module */
static inline void swarm_brain_immune_bridge_heartbeat(const char* operation, float progress) {
    if (g_swarm_brain_immune_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_swarm_brain_immune_bridge_health_agent, operation, progress);
    }
}
    ...
    void* swarm_brain)
{
    ...
    if (bridge_base_init(&bridge->base, 0, "swarm_brain_immune") != 0) { nimcp_free(bridge); return NULL; }
