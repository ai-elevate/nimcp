...
void swarm_consensus_fep_destroy(swarm_consensus_fep_bridge_t* bridge) {
    if (!bridge) return;
    if (bridge->bio_async_enabled) swarm_consensus_fep_disconnect_bio_async(bridge);
...
int swarm_consensus_fep_connect_bio_async(swarm_consensus_fep_bridge_t* bridge) {
    ...
    bio_module_info_t info = { .module_id = BIO_MODULE_FEP_SWARM_CONSENSUS, .module_name = "swarm_consensus_fep_bridge", .inbox_capacity = 32, .user_data = bridge };
