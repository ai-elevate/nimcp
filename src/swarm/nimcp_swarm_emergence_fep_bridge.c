...
/** Global health agent for swarm_emergence_fep_bridge module */
static nimcp_health_agent_t* g_swarm_emergence_fep_bridge_health_agent = NULL;

/**
 * @brief Set health agent for swarm_emergence_fep_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void swarm_emergence_fep_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_swarm_emergence_fep_bridge_health_agent = agent;
}

/** @brief Send heartbeat from swarm_emergence_fep_bridge module */
static inline void swarm_emergence_fep_bridge_heartbeat(const char* operation, float progress) {
    if (g_swarm_emergence_fep_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_swarm_emergence_fep_bridge_health_agent, operation, progress);
    }
}
...
swarm_emergence_fep_bridge_t* swarm_emergence_fep_create(const swarm_emergence_fep_config_t* config, swarm_emergence_ctx_t* emergence_ctx, fep_system_t* fep_system) {
    if (!emergence_ctx) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "swarm_emergence_fep_create: emergence_ctx is NULL");
        return NULL;
    }
    if (!fep_system) {
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NULL_POINTER, "swarm_emergence_fep_create: fep_system is NULL");
        return NULL;
    }
...
int swarm_emergence_fep_connect_bio_async(swarm_emergence_fep_bridge_t* bridge) {
    ...
    bio_module_info_t info = { .module_id = BIO_MODULE_FEP_SWARM_EMERGENCE, .module_name = "swarm_emergence_fep_bridge", .inbox_capacity = NIMCP_INBOX_CAPACITY_SMALL, .user_data = bridge };
