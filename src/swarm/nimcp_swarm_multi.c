...
/** Global health agent for swarm_multi module */
static nimcp_health_agent_t* g_swarm_multi_health_agent = NULL;

/**
 * @brief Set health agent for swarm_multi heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void swarm_multi_set_health_agent(nimcp_health_agent_t* agent) {
    g_swarm_multi_health_agent = agent;
}

/** @brief Send heartbeat from swarm_multi module */
static inline void swarm_multi_heartbeat(const char* operation, float progress) {
    if (g_swarm_multi_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_swarm_multi_health_agent, operation, progress);
    }
}
...
static nimcp_error_t register_bio_async_handlers(
    ...
    nimcp_error_t result = bio_router_register_wiring_callback(
        ...
        (void*)multi_swarm_handler_callback,
        coordinator
    );
