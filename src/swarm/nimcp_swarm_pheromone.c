...
/** Global health agent for swarm_pheromone module */
static nimcp_health_agent_t* g_swarm_pheromone_health_agent = NULL;

/**
 * @brief Set health agent for swarm_pheromone heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void swarm_pheromone_set_health_agent(nimcp_health_agent_t* agent) {
    g_swarm_pheromone_health_agent = agent;
}

/** @brief Send heartbeat from swarm_pheromone module */
static inline void swarm_pheromone_heartbeat(const char* operation, float progress) {
    if (g_swarm_pheromone_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_swarm_pheromone_health_agent, operation, progress);
    }
}
 ...
 * HOW:  Query KG for Swarm_Pheromone entity and its relations
 *
 * @param kg Knowledge graph reader
 * @return 1 if self-knowledge found, 0 otherwise
 */
int swarm_pheromone_query_self_knowledge(kg_reader_t* kg)
{
    if (!kg) return 0;

    const kg_entity_t* self = kg_reader_get_entity(kg, "Swarm_Pheromone");
    if (self) {
        for (uint32_t i = 0; i < self->num_observations; i++) {
            LOG_DEBUG("Pheromone self-knowledge: %s", self->observations[i]);
        }
    }

    kg_relation_list_t* connections = kg_reader_get_relations_from(kg, "Swarm_Pheromone");
    if (connections) {
        kg_relation_list_destroy(connections);
    }

    kg_relation_list_t* incoming = kg_reader_get_relations_to(kg, "Swarm_Pheromone");
    if (incoming) {
