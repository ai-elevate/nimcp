...
/** Global health agent for swarm_task module */
static nimcp_health_agent_t* g_swarm_task_health_agent = NULL;

/**
 * @brief Set health agent for swarm_task heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void swarm_task_set_health_agent(nimcp_health_agent_t* agent) {
    g_swarm_task_health_agent = agent;
}

/** @brief Send heartbeat from swarm_task module */
static inline void swarm_task_heartbeat(const char* operation, float progress) {
    if (g_swarm_task_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_swarm_task_health_agent, operation, progress);
    }
}
