...
NIMCP_DECLARE_HEALTH_AGENT_ATOMIC(swarm_flocking_sleep_bridge)
    ...
    sleep_system_t sleep_system)
{
    ...
    if (!bridge) {
        ...
        NIMCP_THROW_TO_IMMUNE(NIMCP_ERROR_NO_MEMORY, "swarm_flocking_sleep_bridge_create: allocation failed");
