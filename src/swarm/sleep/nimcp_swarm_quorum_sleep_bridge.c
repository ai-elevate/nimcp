...
/** Global health agent for swarm_quorum_sleep_bridge module */
static nimcp_health_agent_t* g_swarm_quorum_sleep_bridge_health_agent = NULL;

/**
 * @brief Set health agent for swarm_quorum_sleep_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void swarm_quorum_sleep_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_swarm_quorum_sleep_bridge_health_agent = agent;
}

/** @brief Send heartbeat from swarm_quorum_sleep_bridge module */
static inline void swarm_quorum_sleep_bridge_heartbeat(const char* operation, float progress) {
    if (g_swarm_quorum_sleep_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_swarm_quorum_sleep_bridge_health_agent, operation, progress);
    }
}
    ...
    sleep_system_t sleep_system)
{
    ...
    if (bridge_base_init(&bridge->base, 0, "swarm_quorum_sleep") != 0) { nimcp_free(bridge); return NULL; }
