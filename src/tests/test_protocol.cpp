...
TEST_F(ProtocolSerializationTest, BasicSerialize) {
    int result = protocol_serialize_message(MSG_TYPE_HELLO,
                                          test_payload,
                                          strlen(test_payload),
                                          buffer,
                                          sizeof(buffer));
...
TEST_F(ProtocolDeserializationTest, BasicDeserialize) {
    // First serialize
    int written = protocol_serialize_message(MSG_TYPE_HELLO,
                                           test_payload,
                                           strlen(test_payload),
                                           buffer,
                                           sizeof(buffer));
    ...
    // Then deserialize
    int result = protocol_deserialize_message(buffer,
                                            written,
                                            &header,
                                            payload_buffer,
...
//        .length = strlen(test_payload),
    ...
    uint32_t checksum = protocol_calculate_checksum(&test_header,
                                                  test_payload,
                                                  strlen(test_payload));
    ...
    uint32_t checksum2 = protocol_calculate_checksum(&test_header,
                                                   test_payload,
                                                   strlen(test_payload));
...
//      .length = strlen(payload1),
    ...
    uint32_t checksum1 = protocol_calculate_checksum(&test_header,
                                                   payload1,
                                                   strlen(payload1));
...
TEST_F(ProtocolIntegrationTest, MessageRoundtrip) {
    ...
    // Deserialize message
    int read = protocol_deserialize_message(buffer,
                                          written,
                                          &header,
                                          payload_buffer,
                                          sizeof(payload_buffer));
