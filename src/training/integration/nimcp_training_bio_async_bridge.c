...
/** Global health agent for training_bio_async_bridge module */
static nimcp_health_agent_t* g_training_bio_async_bridge_health_agent = NULL;

/**
 * @brief Set health agent for training_bio_async_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void training_bio_async_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_training_bio_async_bridge_health_agent = agent;
}

/** @brief Send heartbeat from training_bio_async_bridge module */
static inline void training_bio_async_bridge_heartbeat(const char* operation, float progress) {
    if (g_training_bio_async_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_training_bio_async_bridge_health_agent, operation, progress);
    }
}

#define LOG_MODULE "TRAINING_BIO_ASYNC_BRIDGE"
...
void training_bio_bridge_destroy(training_bio_async_bridge_t* bridge) {
    if (!bridge) {
        return;
        NIMCP_LOGGING_DEBUG("Destroying %s bridge", "training_bio_async");
    }
