...
/** Global health agent for training_state_manager module */
static nimcp_health_agent_t* g_training_state_manager_health_agent = NULL;

/**
 * @brief Set health agent for training_state_manager heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void training_state_manager_set_health_agent(nimcp_health_agent_t* agent) {
    g_training_state_manager_health_agent = agent;
}

/** @brief Send heartbeat from training_state_manager module */
static inline void training_state_manager_heartbeat(const char* operation, float progress) {
    if (g_training_state_manager_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_training_state_manager_health_agent, operation, progress);
    }
}
