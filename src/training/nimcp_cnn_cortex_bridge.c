...
/** Global health agent for cnn_cortex_bridge module */
static nimcp_health_agent_t* g_cnn_cortex_bridge_health_agent = NULL;

/**
 * @brief Set health agent for cnn_cortex_bridge heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void cnn_cortex_bridge_set_health_agent(nimcp_health_agent_t* agent) {
    g_cnn_cortex_bridge_health_agent = agent;
}

/** @brief Send heartbeat from cnn_cortex_bridge module */
static inline void cnn_cortex_bridge_heartbeat(const char* operation, float progress) {
    if (g_cnn_cortex_bridge_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_cnn_cortex_bridge_health_agent, operation, progress);
    }
}
