...
/** Global health agent for gradient_scaling module */
static nimcp_health_agent_t* g_gradient_scaling_health_agent = NULL;

/**
 * @brief Set health agent for gradient_scaling heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void gradient_scaling_set_health_agent(nimcp_health_agent_t* agent) {
    g_gradient_scaling_health_agent = agent;
}

/** @brief Send heartbeat from gradient_scaling module */
static inline void gradient_scaling_heartbeat(const char* operation, float progress) {
    if (g_gradient_scaling_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_gradient_scaling_health_agent, operation, progress);
    }
}
