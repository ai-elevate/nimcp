...
/** Global health agent for louvain module */
static nimcp_health_agent_t* g_louvain_health_agent = NULL;

/**
 * @brief Set health agent for louvain heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void louvain_set_health_agent(nimcp_health_agent_t* agent) {
    g_louvain_health_agent = agent;
}

/** @brief Send heartbeat from louvain module */
static inline void louvain_heartbeat(const char* operation, float progress) {
    if (g_louvain_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_louvain_health_agent, operation, progress);
    }
}
