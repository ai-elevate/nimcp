...
/** Global health agent for config_hash module */
static nimcp_health_agent_t* g_config_hash_health_agent = NULL;

/**
 * @brief Set health agent for config_hash heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void config_hash_set_health_agent(nimcp_health_agent_t* agent) {
    g_config_hash_health_agent = agent;
}

/** @brief Send heartbeat from config_hash module */
static inline void config_hash_heartbeat(const char* operation, float progress) {
    if (g_config_hash_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_config_hash_health_agent, operation, progress);
    }
}
