...
/** Global health agent for dynamic_config module */
static nimcp_health_agent_t* g_dynamic_config_health_agent = NULL;

/**
 * @brief Set health agent for dynamic_config heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void dynamic_config_set_health_agent(nimcp_health_agent_t* agent) {
    g_dynamic_config_health_agent = agent;
}

/** @brief Send heartbeat from dynamic_config module */
static inline void dynamic_config_heartbeat(const char* operation, float progress) {
    if (g_dynamic_config_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_dynamic_config_health_agent, operation, progress);
    }
}
