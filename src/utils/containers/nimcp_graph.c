...
/** Global health agent for graph module */
static nimcp_health_agent_t* g_graph_health_agent = NULL;

/**
 * @brief Set health agent for graph heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void graph_set_health_agent(nimcp_health_agent_t* agent) {
    g_graph_health_agent = agent;
}

/** @brief Send heartbeat from graph module */
static inline void graph_heartbeat(const char* operation, float progress) {
    if (g_graph_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_graph_health_agent, operation, progress);
    }
}
