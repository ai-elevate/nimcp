...
/** Global health agent for graph_ternary module */
static nimcp_health_agent_t* g_graph_ternary_health_agent = NULL;

/**
 * @brief Set health agent for graph_ternary heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void graph_ternary_set_health_agent(nimcp_health_agent_t* agent) {
    g_graph_ternary_health_agent = agent;
}

/** @brief Send heartbeat from graph_ternary module */
static inline void graph_ternary_heartbeat(const char* operation, float progress) {
    if (g_graph_ternary_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_graph_ternary_health_agent, operation, progress);
    }
}
