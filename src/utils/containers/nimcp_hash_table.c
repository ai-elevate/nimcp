...
/** Global health agent for hash_table module */
static nimcp_health_agent_t* g_hash_table_health_agent = NULL;

/**
 * @brief Set health agent for hash_table heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void hash_table_set_health_agent(nimcp_health_agent_t* agent) {
    g_hash_table_health_agent = agent;
}

/** @brief Send heartbeat from hash_table module */
static inline void hash_table_heartbeat(const char* operation, float progress) {
    if (g_hash_table_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_hash_table_health_agent, operation, progress);
    }
}
