...
/** Global health agent for queue module */
static nimcp_health_agent_t* g_queue_health_agent = NULL;

/**
 * @brief Set health agent for queue heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void queue_set_health_agent(nimcp_health_agent_t* agent) {
    g_queue_health_agent = agent;
}

/** @brief Send heartbeat from queue module */
static inline void queue_heartbeat(const char* operation, float progress) {
    if (g_queue_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_queue_health_agent, operation, progress);
    }
}
