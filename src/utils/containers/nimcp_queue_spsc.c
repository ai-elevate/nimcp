...
/** Global health agent for queue_spsc module */
static nimcp_health_agent_t* g_queue_spsc_health_agent = NULL;

/**
 * @brief Set health agent for queue_spsc heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void queue_spsc_set_health_agent(nimcp_health_agent_t* agent) {
    g_queue_spsc_health_agent = agent;
}

/** @brief Send heartbeat from queue_spsc module */
static inline void queue_spsc_heartbeat(const char* operation, float progress) {
    if (g_queue_spsc_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_queue_spsc_health_agent, operation, progress);
    }
}
