...
/** Global health agent for ring_buffer module */
static nimcp_health_agent_t* g_ring_buffer_health_agent = NULL;

/**
 * @brief Set health agent for ring_buffer heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void ring_buffer_set_health_agent(nimcp_health_agent_t* agent) {
    g_ring_buffer_health_agent = agent;
}

/** @brief Send heartbeat from ring_buffer module */
static inline void ring_buffer_heartbeat(const char* operation, float progress) {
    if (g_ring_buffer_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_ring_buffer_health_agent, operation, progress);
    }
}
