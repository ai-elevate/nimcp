...
/** Global health agent for error_codes module */
static nimcp_health_agent_t* g_error_codes_health_agent = NULL;

/**
 * @brief Set health agent for error_codes heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void error_codes_set_health_agent(nimcp_health_agent_t* agent) {
    g_error_codes_health_agent = agent;
}

/** @brief Send heartbeat from error_codes module */
static inline void error_codes_heartbeat(const char* operation, float progress) {
    if (g_error_codes_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_error_codes_health_agent, operation, progress);
    }
}
