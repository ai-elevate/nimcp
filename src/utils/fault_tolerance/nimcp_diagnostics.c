...
/** Global health agent for diagnostics module */
static nimcp_health_agent_t* g_diagnostics_health_agent = NULL;

/**
 * @brief Set health agent for diagnostics heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void diagnostics_set_health_agent(nimcp_health_agent_t* agent) {
    g_diagnostics_health_agent = agent;
}

/** @brief Send heartbeat from diagnostics module */
static inline void diagnostics_heartbeat(const char* operation, float progress) {
    if (g_diagnostics_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_diagnostics_health_agent, operation, progress);
    }
}
