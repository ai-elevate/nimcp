...
/** Global health agent for fast_recovery module */
static nimcp_health_agent_t* g_fast_recovery_health_agent = NULL;

/**
 * @brief Set health agent for fast_recovery heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void fast_recovery_set_health_agent(nimcp_health_agent_t* agent) {
    g_fast_recovery_health_agent = agent;
}

/** @brief Send heartbeat from fast_recovery module */
static inline void fast_recovery_heartbeat(const char* operation, float progress) {
    if (g_fast_recovery_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_fast_recovery_health_agent, operation, progress);
    }
}
