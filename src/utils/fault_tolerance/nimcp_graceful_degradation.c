...
/** Global health agent for graceful_degradation module */
static nimcp_health_agent_t* g_graceful_degradation_health_agent = NULL;

/**
 * @brief Set health agent for graceful_degradation heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void graceful_degradation_set_health_agent(nimcp_health_agent_t* agent) {
    g_graceful_degradation_health_agent = agent;
}

/** @brief Send heartbeat from graceful_degradation module */
static inline void graceful_degradation_heartbeat(const char* operation, float progress) {
    if (g_graceful_degradation_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_graceful_degradation_health_agent, operation, progress);
    }
}
