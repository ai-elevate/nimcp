...
/** Global health agent for health_monitor module */
static nimcp_health_agent_t* g_health_monitor_health_agent = NULL;

/**
 * @brief Set health agent for health_monitor heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void health_monitor_set_health_agent(nimcp_health_agent_t* agent) {
    g_health_monitor_health_agent = agent;
}

/** @brief Send heartbeat from health_monitor module */
static inline void health_monitor_heartbeat(const char* operation, float progress) {
    if (g_health_monitor_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_health_monitor_health_agent, operation, progress);
    }
}
