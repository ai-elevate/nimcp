...
/** Global health agent for lockfree_metrics module */
static nimcp_health_agent_t* g_lockfree_metrics_health_agent = NULL;

/**
 * @brief Set health agent for lockfree_metrics heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void lockfree_metrics_set_health_agent(nimcp_health_agent_t* agent) {
    g_lockfree_metrics_health_agent = agent;
}

/** @brief Send heartbeat from lockfree_metrics module */
static inline void lockfree_metrics_heartbeat(const char* operation, float progress) {
    if (g_lockfree_metrics_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_lockfree_metrics_health_agent, operation, progress);
    }
}
