...
/** Global health agent for metrics_aggregator module */
static nimcp_health_agent_t* g_metrics_aggregator_health_agent = NULL;

/**
 * @brief Set health agent for metrics_aggregator heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void metrics_aggregator_set_health_agent(nimcp_health_agent_t* agent) {
    g_metrics_aggregator_health_agent = agent;
}

/** @brief Send heartbeat from metrics_aggregator module */
static inline void metrics_aggregator_heartbeat(const char* operation, float progress) {
    if (g_metrics_aggregator_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_metrics_aggregator_health_agent, operation, progress);
    }
}
