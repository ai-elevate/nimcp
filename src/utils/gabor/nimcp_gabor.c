...
/** Global health agent for gabor module */
static nimcp_health_agent_t* g_gabor_health_agent = NULL;

/**
 * @brief Set health agent for gabor heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void gabor_set_health_agent(nimcp_health_agent_t* agent) {
    g_gabor_health_agent = agent;
}

/** @brief Send heartbeat from gabor module */
static inline void gabor_heartbeat(const char* operation, float progress) {
    if (g_gabor_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_gabor_health_agent, operation, progress);
    }
}
