...
/** Global health agent for metrics module */
static nimcp_health_agent_t* g_metrics_health_agent = NULL;

/**
 * @brief Set health agent for metrics heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void metrics_set_health_agent(nimcp_health_agent_t* agent) {
    g_metrics_health_agent = agent;
}

/** @brief Send heartbeat from metrics module */
static inline void metrics_heartbeat(const char* operation, float progress) {
    if (g_metrics_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_metrics_health_agent, operation, progress);
    }
}
