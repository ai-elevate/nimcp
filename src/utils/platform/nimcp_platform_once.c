...
/** Global health agent for platform_once module */
static nimcp_health_agent_t* g_platform_once_health_agent = NULL;

/**
 * @brief Set health agent for platform_once heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void platform_once_set_health_agent(nimcp_health_agent_t* agent) {
    g_platform_once_health_agent = agent;
}

/** @brief Send heartbeat from platform_once module */
static inline void platform_once_heartbeat(const char* operation, float progress) {
    if (g_platform_once_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_platform_once_health_agent, operation, progress);
    }
}
