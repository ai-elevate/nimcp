...
/** Global health agent for platform_rwlock module */
static nimcp_health_agent_t* g_platform_rwlock_health_agent = NULL;

/**
 * @brief Set health agent for platform_rwlock heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void platform_rwlock_set_health_agent(nimcp_health_agent_t* agent) {
    g_platform_rwlock_health_agent = agent;
}

/** @brief Send heartbeat from platform_rwlock module */
static inline void platform_rwlock_heartbeat(const char* operation, float progress) {
    if (g_platform_rwlock_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_platform_rwlock_health_agent, operation, progress);
    }
}
