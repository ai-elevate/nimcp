...
/** Global health agent for platform_tier module */
static nimcp_health_agent_t* g_platform_tier_health_agent = NULL;

/**
 * @brief Set health agent for platform_tier heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void platform_tier_set_health_agent(nimcp_health_agent_t* agent) {
    g_platform_tier_health_agent = agent;
}

/** @brief Send heartbeat from platform_tier module */
static inline void platform_tier_heartbeat(const char* operation, float progress) {
    if (g_platform_tier_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_platform_tier_health_agent, operation, progress);
    }
}
