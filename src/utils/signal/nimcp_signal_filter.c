...
/** Global health agent for signal_filter module */
static nimcp_health_agent_t* g_signal_filter_health_agent = NULL;

/**
 * @brief Set health agent for signal_filter heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void signal_filter_set_health_agent(nimcp_health_agent_t* agent) {
    g_signal_filter_health_agent = agent;
}

/** @brief Send heartbeat from signal_filter module */
static inline void signal_filter_heartbeat(const char* operation, float progress) {
    if (g_signal_filter_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_signal_filter_health_agent, operation, progress);
    }
}
