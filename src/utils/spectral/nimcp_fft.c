...
/** Global health agent for fft module */
static nimcp_health_agent_t* g_fft_health_agent = NULL;

/**
 * @brief Set health agent for fft heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void fft_set_health_agent(nimcp_health_agent_t* agent) {
    g_fft_health_agent = agent;
}

/** @brief Send heartbeat from fft module */
static inline void fft_heartbeat(const char* operation, float progress) {
    if (g_fft_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_fft_health_agent, operation, progress);
    }
}
