...
/** Global health agent for atomic module */
static nimcp_health_agent_t* g_atomic_health_agent = NULL;

/**
 * @brief Set health agent for atomic heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void atomic_set_health_agent(nimcp_health_agent_t* agent) {
    g_atomic_health_agent = agent;
}

/** @brief Send heartbeat from atomic module */
static inline void atomic_heartbeat(const char* operation, float progress) {
    if (g_atomic_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_atomic_health_agent, operation, progress);
    }
}
