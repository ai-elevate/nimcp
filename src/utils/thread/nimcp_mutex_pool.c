...
/** Global health agent for mutex_pool module */
static nimcp_health_agent_t* g_mutex_pool_health_agent = NULL;

/**
 * @brief Set health agent for mutex_pool heartbeats
 * @param agent Health agent (can be NULL to disable)
 */
static void mutex_pool_set_health_agent(nimcp_health_agent_t* agent) {
    g_mutex_pool_health_agent = agent;
}

/** @brief Send heartbeat from mutex_pool module */
static inline void mutex_pool_heartbeat(const char* operation, float progress) {
    if (g_mutex_pool_health_agent) {
        nimcp_health_agent_heartbeat_ex(g_mutex_pool_health_agent, operation, progress);
    }
}
