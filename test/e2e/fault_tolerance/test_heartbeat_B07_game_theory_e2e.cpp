/**
 * @file test_heartbeat_B07_game_theory_e2e.cpp
 * @brief E2E tests for B07 (cognitive/game_theory) heartbeat full lifecycle
 *
 * WHAT: End-to-end heartbeat lifecycle tests for 17 cognitive game theory modules
...
class HeartbeatB07E2ETest : public ::testing::Test {
protected:
...
TEST_F(HeartbeatB07E2ETest, FullLifecycleAllModules) {
    for (size_t i = 0; i < B07_MODULE_COUNT; i++) {
    ...
    for (size_t i = 0; i < B07_MODULE_COUNT; i++) {
        char op[64];
        snprintf(op, sizeof(op), "%s_process", B07_MODULES[i].name);
    ...
    EXPECT_GE(stats.heartbeats_received, static_cast<uint64_t>(B07_MODULE_COUNT * 3));
...
TEST_F(HeartbeatB07E2ETest, ConcurrentModulesMultipleThreads) {
    int result = nimcp_health_agent_start(agent);
    ASSERT_EQ(result, 0);
    ...
    for (int t = 0; t < num_threads; t++) {
        threads.emplace_back([this, t, heartbeats_per_thread]() {
            for (int i = 0; i < heartbeats_per_thread; i++) {
                size_t mod_idx = (t * heartbeats_per_thread + i) % B07_MODULE_COUNT;
                char op[64];
                snprintf(op, sizeof(op), "thread%d_%s", t, B07_MODULES[mod_idx].name);
...
TEST_F(HeartbeatB07E2ETest, HighFrequencyBurst1000Heartbeats) {
    int result = nimcp_health_agent_start(agent);
    ASSERT_EQ(result, 0);
...
TEST_F(HeartbeatB07E2ETest, TimeoutDetectionAfterSilence) {
    int result = nimcp_health_agent_start(agent);
    ASSERT_EQ(result, 0);
...
TEST_F(HeartbeatB07E2ETest, MultiPhaseOperation) {
    int result = nimcp_health_agent_start(agent);
    ASSERT_EQ(result, 0);
    ...
    for (size_t i = 0; i < B07_MODULE_COUNT; i++) {
        char op[64];
        snprintf(op, sizeof(op), "%s_finalize", B07_MODULES[i].name);
        nimcp_health_agent_heartbeat_ex(agent, op, 1.0f);
    }

    health_agent_stats_t stats;
    nimcp_health_agent_get_stats(agent, &stats);
    EXPECT_GE(stats.heartbeats_received, static_cast<uint64_t>(B07_MODULE_COUNT * 5));
}

TEST_F(HeartbeatB07E2ETest, ModuleHotSwapDuringOperation) {
    int result = nimcp_health_agent_start(agent);
    ASSERT_EQ(result, 0);
...
TEST_F(HeartbeatB07E2ETest, SustainedOperationOverTime) {
    int result = nimcp_health_agent_start(agent);
    ASSERT_EQ(result, 0);
    ...
    for (int interval = 0; interval < intervals; interval++) {
        for (size_t i = 0; i < B07_MODULE_COUNT; i++) {
            char op[64];
            snprintf(op, sizeof(op), "%s_tick", B07_MODULES[i].name);
    ...
    nimcp_health_agent_get_stats(agent, &stats);
    EXPECT_GE(stats.heartbeats_received,
              static_cast<uint64_t>(B07_MODULE_COUNT * intervals));
}

TEST_F(HeartbeatB07E2ETest, GracefulShutdownSequence) {
    int result = nimcp_health_agent_start(agent);
    ASSERT_EQ(result, 0);
    ...
    for (size_t i = 0; i < B07_MODULE_COUNT; i++) {
        char op[64];
        snprintf(op, sizeof(op), "%s_shutdown", B07_MODULES[i].name);
        nimcp_health_agent_heartbeat_ex(agent, op, 1.0f);
    }

    for (int i = (int)B07_MODULE_COUNT - 1; i >= 0; i--) {
        B07_MODULES[i].set_fn(nullptr);
    }
