/**
 * @file test_heartbeat_B10_fep_ft_e2e.cpp
 * @brief E2E tests for B10 heartbeat (cognitive/free_energy + cognitive/fault_tolerance)
 *
...
class HeartbeatB10E2ETest : public ::testing::Test {
protected:
...
TEST_F(HeartbeatB10E2ETest, FullLifecycleAllModules) {
...
TEST_F(HeartbeatB10E2ETest, ConcurrentModulesMultipleThreads) {
    nimcp_health_agent_start(agent_);
...
TEST_F(HeartbeatB10E2ETest, HighFrequencyBurst1000Heartbeats) {
    nimcp_health_agent_start(agent_);
    for (size_t i = 0; i < B10_MODULE_COUNT; i++) {
        B10_MODULES[i].set_fn(agent_);
    }
    ...
    for (int j = 0; j < 1000; j++) {
        nimcp_health_agent_heartbeat_ex(agent_, "B10_burst", 0);
    }
...
TEST_F(HeartbeatB10E2ETest, TimeoutDetectionAfterSilence) {
    nimcp_health_agent_start(agent_);
    for (size_t i = 0; i < B10_MODULE_COUNT; i++) {
        B10_MODULES[i].set_fn(agent_);
    }
    nimcp_health_agent_heartbeat_ex(agent_, "B10_timeout_test", 0);
...
TEST_F(HeartbeatB10E2ETest, MultiPhaseOperation) {
    nimcp_health_agent_start(agent_);
    for (size_t i = 0; i < B10_MODULE_COUNT / 2; i++) {
        B10_MODULES[i].set_fn(agent_);
        nimcp_health_agent_heartbeat_ex(agent_, B10_MODULES[i].name, 0);
    }
    for (size_t i = B10_MODULE_COUNT / 2; i < B10_MODULE_COUNT; i++) {
        B10_MODULES[i].set_fn(agent_);
        nimcp_health_agent_heartbeat_ex(agent_, B10_MODULES[i].name, 0);
    }
    for (size_t i = 0; i < B10_MODULE_COUNT / 2; i++) {
...
TEST_F(HeartbeatB10E2ETest, ModuleHotSwapDuringOperation) {
    nimcp_health_agent_start(agent_);
    ...
    for (size_t i = 0; i < B10_MODULE_COUNT; i++) {
        ...
        nimcp_health_agent_heartbeat_ex(agent2, B10_MODULES[i].name, 0);
    }

    health_agent_stats_t stats2;
    nimcp_health_agent_get_stats(agent2, &stats2);
    EXPECT_GE(stats2.heartbeats_received, B10_MODULE_COUNT);
...
TEST_F(HeartbeatB10E2ETest, SustainedOperationOverTime) {
    nimcp_health_agent_start(agent_);
    for (size_t i = 0; i < B10_MODULE_COUNT; i++) {
        B10_MODULES[i].set_fn(agent_);
    }
    ...
    while (std::chrono::steady_clock::now() - start < std::chrono::milliseconds(250)) {
        nimcp_health_agent_heartbeat_ex(agent_, "B10_sustained", 0);
        count++;
    }
    EXPECT_GT(count, 0u);
    health_agent_stats_t stats;
...
TEST_F(HeartbeatB10E2ETest, GracefulShutdownSequence) {
    nimcp_health_agent_start(agent_);
    for (size_t i = 0; i < B10_MODULE_COUNT; i++) {
        B10_MODULES[i].set_fn(agent_);
    }
