/**
 * @file test_heartbeat_B13_knowledge_mentalhealth_imagination_e2e.cpp
 * @brief E2E tests for B13 heartbeat
...
class HeartbeatB13E2ETest : public ::testing::Test {
protected:
...
TEST_F(HeartbeatB13E2ETest, FullLifecycleAllModules) {
...
TEST_F(HeartbeatB13E2ETest, ConcurrentModulesMultipleThreads) {
    nimcp_health_agent_start(agent_);
...
TEST_F(HeartbeatB13E2ETest, HighFrequencyBurst1000Heartbeats) {
    nimcp_health_agent_start(agent_);
    for (size_t i = 0; i < B13_MODULE_COUNT; i++) B13_MODULES[i].set_fn(agent_);
    health_agent_stats_t before;
    nimcp_health_agent_get_stats(agent_, &before);
    for (int j = 0; j < 1000; j++)
        nimcp_health_agent_heartbeat_ex(agent_, "B13_burst", 0);
...
TEST_F(HeartbeatB13E2ETest, TimeoutDetectionAfterSilence) {
    nimcp_health_agent_start(agent_);
    for (size_t i = 0; i < B13_MODULE_COUNT; i++) B13_MODULES[i].set_fn(agent_);
    nimcp_health_agent_heartbeat_ex(agent_, "B13_timeout_test", 0);
...
TEST_F(HeartbeatB13E2ETest, MultiPhaseOperation) {
    nimcp_health_agent_start(agent_);
    for (size_t i = 0; i < B13_MODULE_COUNT / 2; i++) {
        B13_MODULES[i].set_fn(agent_);
        nimcp_health_agent_heartbeat_ex(agent_, B13_MODULES[i].name, 0);
    }
    for (size_t i = B13_MODULE_COUNT / 2; i < B13_MODULE_COUNT; i++) {
        B13_MODULES[i].set_fn(agent_);
        nimcp_health_agent_heartbeat_ex(agent_, B13_MODULES[i].name, 0);
    }
...
TEST_F(HeartbeatB13E2ETest, ModuleHotSwapDuringOperation) {
    nimcp_health_agent_start(agent_);
    ...
    for (size_t i = 0; i < B13_MODULE_COUNT; i++) {
        ...
        nimcp_health_agent_heartbeat_ex(agent2, B13_MODULES[i].name, 0);
    }
    health_agent_stats_t stats2;
    nimcp_health_agent_get_stats(agent2, &stats2);
    EXPECT_GE(stats2.heartbeats_received, B13_MODULE_COUNT);
    for (size_t i = 0; i < B13_MODULE_COUNT; i++) B13_MODULES[i].set_fn(nullptr);
...
TEST_F(HeartbeatB13E2ETest, SustainedOperationOverTime) {
    nimcp_health_agent_start(agent_);
    for (size_t i = 0; i < B13_MODULE_COUNT; i++) B13_MODULES[i].set_fn(agent_);
    auto start = std::chrono::steady_clock::now();
    uint64_t count = 0;
    while (std::chrono::steady_clock::now() - start < std::chrono::milliseconds(250)) {
        nimcp_health_agent_heartbeat_ex(agent_, "B13_sustained", 0);
        count++;
    }
    EXPECT_GT(count, 0u);
    health_agent_stats_t stats;
...
TEST_F(HeartbeatB13E2ETest, GracefulShutdownSequence) {
    nimcp_health_agent_start(agent_);
    for (size_t i = 0; i < B13_MODULE_COUNT; i++) B13_MODULES[i].set_fn(agent_);
    for (size_t i = 0; i < B13_MODULE_COUNT; i++)
        nimcp_health_agent_heartbeat_ex(agent_, B13_MODULES[i].name, 0);
