/**
 * @file test_heartbeat_B14_curiosity_collective_logic_e2e.cpp
 * @brief E2E tests for B14 heartbeat
...
class HeartbeatB14E2ETest : public ::testing::Test {
protected:
...
TEST_F(HeartbeatB14E2ETest, FullLifecycleAllModules) {
    ...
    EXPECT_GE(stats.heartbeats_received, B14_MODULE_COUNT);
    for (size_t i = 0; i < B14_MODULE_COUNT; i++) B14_MODULES[i].set_fn(nullptr);
    nimcp_health_agent_stop(agent_);
}

TEST_F(HeartbeatB14E2ETest, ConcurrentModulesMultipleThreads) {
    nimcp_health_agent_start(agent_);
...
TEST_F(HeartbeatB14E2ETest, HighFrequencyBurst1000Heartbeats) {
    nimcp_health_agent_start(agent_);
    for (size_t i = 0; i < B14_MODULE_COUNT; i++) B14_MODULES[i].set_fn(agent_);
    health_agent_stats_t before;
    nimcp_health_agent_get_stats(agent_, &before);
    for (int j = 0; j < 1000; j++)
        nimcp_health_agent_heartbeat_ex(agent_, "B14_burst", 0);
...
TEST_F(HeartbeatB14E2ETest, TimeoutDetectionAfterSilence) {
    nimcp_health_agent_start(agent_);
    for (size_t i = 0; i < B14_MODULE_COUNT; i++) B14_MODULES[i].set_fn(agent_);
    nimcp_health_agent_heartbeat_ex(agent_, "B14_timeout_test", 0);
...
TEST_F(HeartbeatB14E2ETest, MultiPhaseOperation) {
    nimcp_health_agent_start(agent_);
    for (size_t i = 0; i < B14_MODULE_COUNT / 2; i++) {
        B14_MODULES[i].set_fn(agent_);
        nimcp_health_agent_heartbeat_ex(agent_, B14_MODULES[i].name, 0);
    }
    for (size_t i = B14_MODULE_COUNT / 2; i < B14_MODULE_COUNT; i++) {
        B14_MODULES[i].set_fn(agent_);
        nimcp_health_agent_heartbeat_ex(agent_, B14_MODULES[i].name, 0);
    }
    for (size_t i = 0; i < B14_MODULE_COUNT / 2; i++) B14_MODULES[i].set_fn(nullptr);
    health_agent_stats_t stats;
    nimcp_health_agent_get_stats(agent_, &stats);
    EXPECT_GE(stats.heartbeats_received, B14_MODULE_COUNT);
    nimcp_health_agent_stop(agent_);
...
TEST_F(HeartbeatB14E2ETest, ModuleHotSwapDuringOperation) {
    nimcp_health_agent_start(agent_);
    ...
    for (size_t i = 0; i < B14_MODULE_COUNT; i++) {
        ...
        nimcp_health_agent_heartbeat_ex(agent2, B14_MODULES[i].name, 0);
    }
    health_agent_stats_t stats2;
    nimcp_health_agent_get_stats(agent2, &stats2);
    EXPECT_GE(stats2.heartbeats_received, B14_MODULE_COUNT);
    for (size_t i = 0; i < B14_MODULE_COUNT; i++) B14_MODULES[i].set_fn(nullptr);
...
TEST_F(HeartbeatB14E2ETest, SustainedOperationOverTime) {
    nimcp_health_agent_start(agent_);
    for (size_t i = 0; i < B14_MODULE_COUNT; i++) B14_MODULES[i].set_fn(agent_);
    auto start = std::chrono::steady_clock::now();
    uint64_t count = 0;
    while (std::chrono::steady_clock::now() - start < std::chrono::milliseconds(250)) {
        nimcp_health_agent_heartbeat_ex(agent_, "B14_sustained", 0);
        count++;
    }
    EXPECT_GT(count, 0u);
    health_agent_stats_t stats;
...
TEST_F(HeartbeatB14E2ETest, GracefulShutdownSequence) {
    nimcp_health_agent_start(agent_);
    for (size_t i = 0; i < B14_MODULE_COUNT; i++) B14_MODULES[i].set_fn(agent_);
    for (size_t i = 0; i < B14_MODULE_COUNT; i++)
        nimcp_health_agent_heartbeat_ex(agent_, B14_MODULES[i].name, 0);
