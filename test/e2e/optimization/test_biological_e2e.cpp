/**
 * @file test_biological_e2e.cpp
 * @brief End-to-end tests for Phase 3 biological connectivity enhancements
 *
 * Full pipeline tests: classifier with bio connectivity, grounded language,
 * consolidation cycle, mixed workload, throughput benchmarks.
 */
...
class BiologicalE2E : public ::testing::Test {
protected:
    static nimcp_brain_t brain;
    static void SetUpTestSuite() {
        brain = nimcp_brain_create_fast("bio_e2e",
...
nimcp_brain_t BiologicalE2E::brain = nullptr;

TEST_F(BiologicalE2E, BrainCreated) {
    ASSERT_NE(brain, nullptr);
}

TEST_F(BiologicalE2E, TrainClassifierWithBioConnectivity) {
    if (!brain) GTEST_SKIP();
...
TEST_F(BiologicalE2E, InferenceAfterConsolidation) {
    if (!brain) GTEST_SKIP();

    // Train past consolidation boundary
    ...
    // Inference should work well after consolidation
...
TEST_F(BiologicalE2E, GroundedLanguageWithBioConnectivity) {
    if (!brain) GTEST_SKIP();
    ...
    nimcp_brain_learn_language(brain, "the fast dog runs with strong synapses", nullptr);
    nimcp_brain_learn_language(brain, "the slow cat rests after consolidation", nullptr);
...
TEST_F(BiologicalE2E, InferenceThroughputWithBioConnectivity) {
    if (!brain) GTEST_SKIP();
...
TEST_F(BiologicalE2E, MixedBioWorkloadStability) {
    if (!brain) GTEST_SKIP();
    ...
    for (int round = 0; round < MIXED_ROUNDS; round++) {
        ...
        nimcp_brain_learn_language(brain, "biological neural pathways grow", nullptr);
...
TEST_F(BiologicalE2E, SparsityTrackingDuringBioWorkload) {
    if (!brain) GTEST_SKIP();
...
TEST_F(BiologicalE2E, TrainingThroughputWithBioConnectivity) {
    if (!brain) GTEST_SKIP();
