/**
 * @file test_optimization_e2e.cpp
 * @brief End-to-end tests for 40-watt brain optimizations
 */
...
static constexpr int      SPARSITY_CHECK_ITERS = 20;
static constexpr int      THROUGHPUT_ITERS     = 100;
static constexpr double   THROUGHPUT_MAX_MS    = 60000.0;
static constexpr int      MIXED_WORKLOAD_ROUNDS = 5;

class OptimizationE2E : public ::testing::Test {
protected:
    static nimcp_brain_t brain;
    static void SetUpTestSuite() {
        brain = nimcp_brain_create_fast("optim_e2e",
...
nimcp_brain_t OptimizationE2E::brain = nullptr;

TEST_F(OptimizationE2E, BrainCreated) {
    ASSERT_NE(brain, nullptr);
}

TEST_F(OptimizationE2E, TrainClassifierEndToEnd) {
    if (!brain) GTEST_SKIP();
...
TEST_F(OptimizationE2E, InferenceAfterTraining) {
    if (!brain) GTEST_SKIP();
...
TEST_F(OptimizationE2E, GroundedLanguagePipeline) {
    if (!brain) GTEST_SKIP();
    ...
    nimcp_brain_learn_language(brain, "the small cat sits quietly", nullptr);

    float semantic[SEMANTIC_DIM] = {};
    float conf = 0;
    nimcp_status_t s = nimcp_brain_comprehend(brain, "the big dog",
...
TEST_F(OptimizationE2E, SparsityTrackingDuringWorkload) {
    if (!brain) GTEST_SKIP();

    for (int i = 0; i < SPARSITY_CHECK_ITERS; i++) {
        float input[INPUT_SIZE];
...
TEST_F(OptimizationE2E, InferenceThroughput) {
    if (!brain) GTEST_SKIP();
...
TEST_F(OptimizationE2E, MixedWorkloadStability) {
    if (!brain) GTEST_SKIP();

    for (int round = 0; round < MIXED_WORKLOAD_ROUNDS; round++) {
        float input[INPUT_SIZE];
        for (uint32_t j = 0; j < INPUT_SIZE; j++)
        ...
        // Language
        nimcp_brain_learn_language(brain, "the quick brown fox", nullptr);
