/**
 * @file test_resource_e2e.cpp
 * @brief End-to-end tests for Phase 3 resource optimizations
 *
...
class ResourceE2E : public ::testing::Test {
protected:
    static nimcp_brain_t brain;
    static void SetUpTestSuite() {
        brain = nimcp_brain_create_fast("resource_e2e",
...
nimcp_brain_t ResourceE2E::brain = nullptr;

TEST_F(ResourceE2E, BrainCreated) {
    ASSERT_NE(brain, nullptr);
}

TEST_F(ResourceE2E, TrainClassifierEndToEnd) {
    if (!brain) GTEST_SKIP();
...
TEST_F(ResourceE2E, InferenceAfterHeavyTraining) {
    if (!brain) GTEST_SKIP();
...
TEST_F(ResourceE2E, GroundedLanguagePipeline) {
    if (!brain) GTEST_SKIP();
...
TEST_F(ResourceE2E, InferenceThroughputBenchmark) {
    if (!brain) GTEST_SKIP();
...
TEST_F(ResourceE2E, MixedWorkloadStability) {
    if (!brain) GTEST_SKIP();
    ...
    for (int round = 0; round < MIXED_ROUNDS; round++) {
        ...
        nimcp_brain_learn_language(brain, "optimized resource paths", nullptr);
...
TEST_F(ResourceE2E, TrainingThroughputBenchmark) {
    if (!brain) GTEST_SKIP();
