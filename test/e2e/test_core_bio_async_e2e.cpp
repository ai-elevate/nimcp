...
TEST_F(CoreBioAsyncE2ETest, BrainLifecyclePipeline) {
    ...
    brain_t brain = brain_create("e2e_test_brain", BRAIN_SIZE_TINY,
    ...
    // Test creates 3 explicit promise/future pairs (create, resize, destroy)
    // Some internal operations may add more, but >= 3 is the minimum expected
    E2E_ASSERT(stats.total_futures_created >= 3, "Should have created futures for lifecycle ops");
    E2E_ASSERT(stats.total_futures_completed >= 3, "Should have completed lifecycle futures");
...
TEST_F(CoreBioAsyncE2ETest, PretrainedModelWorkflow) {
    ...
    brain_t brain = brain_create("pretrained_mock", BRAIN_SIZE_TINY,
                                  BRAIN_TASK_CLASSIFICATION, 10, 5);
...
TEST_F(CoreBioAsyncE2ETest, MultiModuleCoreIntegration) {
    ...
    // Create brain using simple API
    brain_t brain = brain_create("multi_module_brain", BRAIN_SIZE_TINY,
                                  BRAIN_TASK_CLASSIFICATION, 10, 5);
    E2E_ASSERT_NOT_NULL(brain, "Brain creation failed");
