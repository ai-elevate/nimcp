...
protected:
    ...
    brain_t create_test_brain(uint32_t num_neurons = 100) {
        // Use simple brain_create API with preset size
        // num_neurons is used to select appropriate preset size
        brain_size_t size = BRAIN_SIZE_TINY;
        if (num_neurons >= 500) size = BRAIN_SIZE_MEDIUM;
        else if (num_neurons >= 200) size = BRAIN_SIZE_SMALL;

        return brain_create("test_brain", size, BRAIN_TASK_CLASSIFICATION, 10, 5);
    }
};
...
TEST_F(BrainBioAsyncIntegrationTest, MultipleBrains_IsolatedMessaging) {
    ...
    // Use IDs 0x0110 and 0x0111 to avoid conflict with fixture's BIO_MODULE_BRAIN (0x0100)
    bio_module_info_t brain1_info;
    brain1_info.module_id = static_cast<bio_module_id_t>(0x0110);
    brain1_info.module_name = "brain1";
    brain1_info.inbox_capacity = 64;
    brain1_info.user_data = nullptr;

    bio_module_info_t brain2_info;
    brain2_info.module_id = static_cast<bio_module_id_t>(0x0111);
    ...
    bio_msg_init_header(&msg1, BIO_MSG_BRAIN_STEP_REQUEST,
                       BIO_MODULE_SYSTEM, static_cast<bio_module_id_t>(0x0110),
                       sizeof(msg1));
    ...
    bio_msg_init_header(&msg2, BIO_MSG_BRAIN_STEP_REQUEST,
                       BIO_MODULE_SYSTEM, static_cast<bio_module_id_t>(0x0111),
                       sizeof(msg2));
...
TEST_F(BrainBioAsyncIntegrationTest, ConcurrentBrainModules_MessageIsolation) {
    ...
    // Create brain modules with IDs 0x0120+i to avoid conflict with fixture's BIO_MODULE_BRAIN (0x0100)
    for (int i = 0; i < NUM_BRAIN_MODULES; i++) {
        bio_module_info_t info;
        info.module_id = static_cast<bio_module_id_t>(0x0120 + i);
    ...
    for (int i = 0; i < NUM_BRAIN_MODULES; i++) {
        for (int j = 0; j < i + 1; j++) {
            ...
            bio_msg_init_header(&msg, BIO_MSG_BRAIN_STEP_REQUEST,
                               BIO_MODULE_SYSTEM, static_cast<bio_module_id_t>(0x0120 + i),
                               sizeof(msg));
