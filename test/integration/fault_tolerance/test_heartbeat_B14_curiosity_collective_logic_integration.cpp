/**
 * @file test_heartbeat_B14_curiosity_collective_logic_integration.cpp
 * @brief Integration tests for B14 heartbeat
...
class HeartbeatB14IntegrationTest : public ::testing::Test {
protected:
...
TEST_F(HeartbeatB14IntegrationTest, ConnectAllModulesAndVerifyHeartbeatFlow) {
    ...
    EXPECT_GE(after.heartbeats_received, before.heartbeats_received + B14_MODULE_COUNT);
    nimcp_health_agent_stop(agent_);
}

TEST_F(HeartbeatB14IntegrationTest, MultipleModulesShareSingleAgent) {
    for (size_t i = 0; i < B14_MODULE_COUNT; i++) B14_MODULES[i].set_fn(agent_);
...
TEST_F(HeartbeatB14IntegrationTest, DisconnectModulesWhileAgentRunning) {
    nimcp_health_agent_start(agent_);
    for (size_t i = 0; i < B14_MODULE_COUNT; i++) B14_MODULES[i].set_fn(agent_);
...
TEST_F(HeartbeatB14IntegrationTest, DisconnectAllModulesWhileAgentRunning) {
    nimcp_health_agent_start(agent_);
    for (size_t i = 0; i < B14_MODULE_COUNT; i++) B14_MODULES[i].set_fn(agent_);
    for (size_t i = 0; i < B14_MODULE_COUNT; i++) B14_MODULES[i].set_fn(nullptr);
    nimcp_health_agent_stop(agent_);
...
TEST_F(HeartbeatB14IntegrationTest, AgentRestartWithModulesConnected) {
    for (size_t i = 0; i < B14_MODULE_COUNT; i++) B14_MODULES[i].set_fn(agent_);
    nimcp_health_agent_start(agent_);
    nimcp_health_agent_stop(agent_);
    nimcp_health_agent_start(agent_);
...
TEST_F(HeartbeatB14IntegrationTest, ConcurrentConnectionDuringHeartbeats) {
    nimcp_health_agent_start(agent_);
...
TEST_F(HeartbeatB14IntegrationTest, ReplaceAgentOnAllModulesAtomically) {
    ...
    for (size_t i = 0; i < B14_MODULE_COUNT; i++) B14_MODULES[i].set_fn(agent_);
    for (size_t i = 0; i < B14_MODULE_COUNT; i++) B14_MODULES[i].set_fn(agent2);
    for (size_t i = 0; i < B14_MODULE_COUNT; i++) B14_MODULES[i].set_fn(nullptr);
    nimcp_health_agent_destroy(agent2);
}

TEST_F(HeartbeatB14IntegrationTest, ProgressiveModuleConnection) {
    nimcp_health_agent_start(agent_);
    for (size_t i = 0; i < B14_MODULE_COUNT; i++) {
        B14_MODULES[i].set_fn(agent_);
        nimcp_health_agent_heartbeat_ex(agent_, B14_MODULES[i].name, 0);
