/**
 * @file test_heartbeat_B19_remaining_modules_integration.cpp
 * @brief Integration tests for B19 heartbeat (all remaining uncovered modules)
 */
...
class HeartbeatB19IntegrationTest : public ::testing::Test {
protected:
...
TEST_F(HeartbeatB19IntegrationTest, ConnectAllModulesAndVerifyHeartbeatFlow) {
    ...
    for (size_t i = 0; i < B19_MODULE_COUNT; i++)
        nimcp_health_agent_heartbeat_ex(agent_, B19_MODULES[i].name, 0);
    nimcp_health_agent_get_stats(agent_, &after);
    EXPECT_GE(after.heartbeats_received, before.heartbeats_received + B19_MODULE_COUNT);
    nimcp_health_agent_stop(agent_);
}

TEST_F(HeartbeatB19IntegrationTest, MultipleModulesShareSingleAgent) {
    for (size_t i = 0; i < B19_MODULE_COUNT; i++) B19_MODULES[i].set_fn(agent_);
...
TEST_F(HeartbeatB19IntegrationTest, DisconnectModulesWhileAgentRunning) {
    nimcp_health_agent_start(agent_);
    for (size_t i = 0; i < B19_MODULE_COUNT; i++) B19_MODULES[i].set_fn(agent_);
...
TEST_F(HeartbeatB19IntegrationTest, DisconnectAllModulesWhileAgentRunning) {
    nimcp_health_agent_start(agent_);
    for (size_t i = 0; i < B19_MODULE_COUNT; i++) B19_MODULES[i].set_fn(agent_);
    for (size_t i = 0; i < B19_MODULE_COUNT; i++) B19_MODULES[i].set_fn(nullptr);
    nimcp_health_agent_stop(agent_);
...
TEST_F(HeartbeatB19IntegrationTest, AgentRestartWithModulesConnected) {
    for (size_t i = 0; i < B19_MODULE_COUNT; i++) B19_MODULES[i].set_fn(agent_);
    nimcp_health_agent_start(agent_);
    nimcp_health_agent_stop(agent_);
    nimcp_health_agent_start(agent_);
...
TEST_F(HeartbeatB19IntegrationTest, ConcurrentConnectionDuringHeartbeats) {
    nimcp_health_agent_start(agent_);
...
TEST_F(HeartbeatB19IntegrationTest, ReplaceAgentOnAllModulesAtomically) {
    ...
    for (size_t i = 0; i < B19_MODULE_COUNT; i++) B19_MODULES[i].set_fn(agent_);
    for (size_t i = 0; i < B19_MODULE_COUNT; i++) B19_MODULES[i].set_fn(agent2);
    for (size_t i = 0; i < B19_MODULE_COUNT; i++) B19_MODULES[i].set_fn(nullptr);
    nimcp_health_agent_destroy(agent2);
}

TEST_F(HeartbeatB19IntegrationTest, ProgressiveModuleConnection) {
    nimcp_health_agent_start(agent_);
    for (size_t i = 0; i < B19_MODULE_COUNT; i++) {
        B19_MODULES[i].set_fn(agent_);
        nimcp_health_agent_heartbeat_ex(agent_, B19_MODULES[i].name, 0);
