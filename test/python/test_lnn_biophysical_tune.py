#!/usr/bin/env python3
"""Smoke test for LNN Phase 2 tunables exposed to Python (substrate + thalamic).

Verifies that each of the 6 new `lnn_tune` names round-trips through the
Python binding: set a known value, then read it back via `lnn_tune_get` and
...
Mirrors the pattern in test_snn_biophysical_tune.py. Runs in-process (no
...
TUNABLES = [
    # (name,                             good,   bad)
    # Substrate adapter (Phase 2 biological-substrate wiring for LNN)
    ("substrate_enabled",                1.0,    None),   # bool-like: no clamp rejection
    ("substrate_update_period",          25.0,   0.5),    # must be in [1, 10000]
    ("substrate_tau_compose_on",         1.0,    None),   # bool-like: no clamp rejection
    # Thalamic adapter (Phase 2 multi-network thalamic rollout)
    ("thalamic_enabled",                 1.0,    None),   # bool-like: no clamp rejection
    ("thalamic_input_gain_on",           1.0,    None),   # bool-like: no clamp rejection
    ("thalamic_burst_tau_clamp_on",      1.0,    None),   # bool-like: no clamp rejection
]
...
def _build_brain():
    ...
    import nimcp
    return nimcp.Brain("lnn_biophysical_tune_smoke", 128, 10)


def test_defaults_reasonable():
    """Sanity check default values match Phase 2 design: all 6 default to 1.0,
    with substrate_update_period defaulting to 10.0 (steps between recomputes).
    """
    b = _build_brain()
    params = b.lnn_tune_get()

    # Substrate + thalamic enables/bools all default ON (1.0).
    expect_on = [
        "substrate_enabled",
        "substrate_tau_compose_on",
        "thalamic_enabled",
        "thalamic_input_gain_on",
        "thalamic_burst_tau_clamp_on",
    ]
    ...
    print(f"  PASS: Phase 2 LNN defaults match design (5 enables=1.0, update_period=10.0)")


def test_all_new_tunables_present():
    """Every new name must appear in the lnn_tune_get dict on a fresh brain."""
    b = _build_brain()
    params = b.lnn_tune_get()
    missing = [name for name, _good, _bad in TUNABLES if name not in params]
    if missing:
        raise AssertionError(
            f"lnn_tune_get missing {len(missing)} Phase 2 keys: {missing}"
        )
    print(f"  PASS: all {len(TUNABLES)} LNN tunables exposed in lnn_tune_get()")


def test_round_trip_set_get():
    """Setting a valid value must be reflected in the next lnn_tune_get."""
    b = _build_brain()
    failed = []
    for name, good, _bad in TUNABLES:
        ok = b.lnn_tune(name, good)
        if not ok:
            failed.append(f"{name}: lnn_tune returned falsy")
            continue
        params = b.lnn_tune_get()
        got = params.get(name)
        if got is None:
    ...
    print(f"  PASS: {len(TUNABLES)} tunables round-trip through lnn_tune -> lnn_tune_get")
...
def test_invalid_values_are_clamped():
    ...
    for name, good, bad in TUNABLES:
        ...
        b.lnn_tune(name, good)
        before = float(b.lnn_tune_get()[name])
        ...
        after = float(b.lnn_tune_get()[name])
...
def main():
    ...
    for name, fn in tests:
        print(f"[lnn_biophysical_tune] {name}")
        try:
            fn()
    ...
    if failures:
        ...
        sys.exit(1)
    print(f"\nAll {len(tests)} LNN biophysical tunable smoke tests passed.")
