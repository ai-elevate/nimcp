/**
 * @file test_heartbeat_B17_emotion_cluster_regression.cpp
 * @brief Regression tests for B17 heartbeat
...
class HeartbeatB17RegressionTest : public ::testing::Test {
protected:
...
TEST_F(HeartbeatB17RegressionTest, NullAlwaysAcceptedOnFirstCall) {
    for (size_t i = 0; i < B17_MODULE_COUNT; i++) { SCOPED_TRACE(B17_MODULES[i].name); B17_MODULES[i].set_fn(nullptr); }
}

TEST_F(HeartbeatB17RegressionTest, NullAlwaysAcceptedAfterValid) {
    for (size_t i = 0; i < B17_MODULE_COUNT; i++) { SCOPED_TRACE(B17_MODULES[i].name); B17_MODULES[i].set_fn(agent_); B17_MODULES[i].set_fn(nullptr); }
}

TEST_F(HeartbeatB17RegressionTest, NullAcceptedRepeatedlyOnSameModule) {
    for (size_t i = 0; i < B17_MODULE_COUNT; i++) { SCOPED_TRACE(B17_MODULES[i].name); for (int j = 0; j < 10; j++) B17_MODULES[i].set_fn(nullptr); }
}

TEST_F(HeartbeatB17RegressionTest, ValidAgentAlwaysAcceptedOnFirstCall) {
    for (size_t i = 0; i < B17_MODULE_COUNT; i++) { SCOPED_TRACE(B17_MODULES[i].name); B17_MODULES[i].set_fn(agent_); }
}

TEST_F(HeartbeatB17RegressionTest, ValidAgentAlwaysAcceptedAfterNull) {
    for (size_t i = 0; i < B17_MODULE_COUNT; i++) { SCOPED_TRACE(B17_MODULES[i].name); B17_MODULES[i].set_fn(nullptr); B17_MODULES[i].set_fn(agent_); }
}

TEST_F(HeartbeatB17RegressionTest, SetDuringActiveHeartbeatsNoCrash) {
    nimcp_health_agent_start(agent_);
    std::vector<std::thread> threads;
    for (size_t i = 0; i < B17_MODULE_COUNT; i++) {
        threads.emplace_back([this, i]() { B17_MODULES[i].set_fn(agent_); for (int j = 0; j < 20; j++) nimcp_health_agent_heartbeat_ex(agent_, B17_MODULES[i].name, 0); });
    }
    for (auto& t : threads) t.join();
    nimcp_health_agent_stop(agent_);
}

TEST_F(HeartbeatB17RegressionTest, ClearDuringActiveHeartbeatsNoCrash) {
    nimcp_health_agent_start(agent_);
    for (size_t i = 0; i < B17_MODULE_COUNT; i++) B17_MODULES[i].set_fn(agent_);
    std::thread heartbeat_thread([this]() { for (int j = 0; j < 100; j++) nimcp_health_agent_heartbeat_ex(agent_, "B17_regression", 0); });
    for (size_t i = 0; i < B17_MODULE_COUNT; i++) B17_MODULES[i].set_fn(nullptr);
    heartbeat_thread.join();
    nimcp_health_agent_stop(agent_);
}

TEST_F(HeartbeatB17RegressionTest, RapidSetClearCycleAllModules) {
...
TEST_F(HeartbeatB17RegressionTest, RapidSetClearCycleSingleModule) {
    for (size_t i = 0; i < B17_MODULE_COUNT; i++) { SCOPED_TRACE(B17_MODULES[i].name); for (int cycle = 0; cycle < 100; cycle++) { B17_MODULES[i].set_fn(agent_); B17_MODULES[i].set_fn(nullptr); } }
}

TEST_F(HeartbeatB17RegressionTest, MultipleAgentCreationDestructionCycle) {
    for (int cycle = 0; cycle < 5; cycle++) {
        ...
        for (size_t i = 0; i < B17_MODULE_COUNT; i++) B17_MODULES[i].set_fn(temp);
        for (size_t i = 0; i < B17_MODULE_COUNT; i++) B17_MODULES[i].set_fn(nullptr);
        nimcp_health_agent_destroy(temp);
    }
}

TEST_F(HeartbeatB17RegressionTest, AgentStartStopCycleWithModules) {
    for (size_t i = 0; i < B17_MODULE_COUNT; i++) B17_MODULES[i].set_fn(agent_);
...
TEST_F(HeartbeatB17RegressionTest, SetterSignatureIsVoidReturnSingleParam) {
    for (size_t i = 0; i < B17_MODULE_COUNT; i++) { SCOPED_TRACE(B17_MODULES[i].name); EXPECT_NE(B17_MODULES[i].set_fn, nullptr); B17_MODULES[i].set_fn(agent_); B17_MODULES[i].set_fn(nullptr); }
}

TEST_F(HeartbeatB17RegressionTest, StatsConsistentAfterModuleSetClear) {
    nimcp_health_agent_start(agent_);
    for (size_t i = 0; i < B17_MODULE_COUNT; i++) B17_MODULES[i].set_fn(agent_);
    health_agent_stats_t stats1; nimcp_health_agent_get_stats(agent_, &stats1);
    nimcp_health_agent_heartbeat_ex(agent_, "B17_consistency", 0);
