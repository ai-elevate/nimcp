/**
 * @file test_biological_regression.cpp
 * @brief Regression tests for Phase 3 biological connectivity enhancements
 *
 * Tests edge cases: null brain, boundary conditions, heavy training stability,
 * create-destroy cycles with bio features, consolidation boundary.
 */
...
static constexpr int      CONSOLIDATION_BOUNDARY = 500;
static constexpr int      POST_CONSOL_ITERS     = 100;

class BiologicalRegression : public ::testing::Test {
protected:
    static nimcp_brain_t brain;
    static void SetUpTestSuite() {
        brain = nimcp_brain_create_fast("bio_regression",
...
nimcp_brain_t BiologicalRegression::brain = nullptr;

TEST_F(BiologicalRegression, NullBrainActiveCount) {
    EXPECT_EQ(nimcp_brain_get_active_neuron_count(nullptr), 0u);
}

TEST_F(BiologicalRegression, NullBrainSparsity) {
    EXPECT_FLOAT_EQ(nimcp_brain_get_sparsity_ratio(nullptr), 0.0f);
}

TEST_F(BiologicalRegression, ZeroInputWithBioConnectivity) {
    if (!brain) GTEST_SKIP();
...
TEST_F(BiologicalRegression, LargeInputsWithBioConnectivity) {
    if (!brain) GTEST_SKIP();
...
TEST_F(BiologicalRegression, TinyInputsWithBioConnectivity) {
    if (!brain) GTEST_SKIP();
...
TEST_F(BiologicalRegression, ConsolidationBoundaryStability) {
    if (!brain) GTEST_SKIP();

    // Train exactly at the consolidation boundary (500 steps)
    for (int i = 0; i < CONSOLIDATION_BOUNDARY; i++) {
        ...
        EXPECT_TRUE(std::isfinite(result.loss))
            << "Loss not finite at boundary step " << i;
    }

    // Continue past boundary
    for (int i = 0; i < POST_CONSOL_ITERS; i++) {
        float input[INPUT_SIZE];
        for (uint32_t j = 0; j < INPUT_SIZE; j++)
            input[j] = cosf((float)(i * INPUT_SIZE + j) * 0.1f);
        float target[OUTPUT_SIZE] = {};
        target[(i + 50) % OUTPUT_SIZE] = 1.0f;
        nimcp_training_result_t result = {};
        nimcp_brain_train_step(brain, input, INPUT_SIZE,
                               target, OUTPUT_SIZE, &result);
        EXPECT_TRUE(std::isfinite(result.loss))
            << "Loss not finite post-consolidation at step " << i;
    }
}

TEST_F(BiologicalRegression, IdenticalInputsSameOutput) {
    if (!brain) GTEST_SKIP();
...
TEST_F(BiologicalRegression, RapidTrainInferCycles) {
    if (!brain) GTEST_SKIP();
...
TEST_F(BiologicalRegression, CreateDestroyWithBioFeatures) {
    for (int i = 0; i < CREATE_DESTROY_ITERS; i++) {
        nimcp_brain_t temp = nimcp_brain_create_fast("temp_bio",
                                    NIMCP_TASK_CLASSIFICATION,
...
TEST_F(BiologicalRegression, SparsityAfterConsolidation) {
    if (!brain) GTEST_SKIP();
...
TEST_F(BiologicalRegression, TrainWithZeroTarget) {
    if (!brain) GTEST_SKIP();
...
TEST_F(BiologicalRegression, TrainWithAllOnesTarget) {
    if (!brain) GTEST_SKIP();
