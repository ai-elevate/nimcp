/**
 * @file test_resource_regression.cpp
 * @brief Regression tests for Phase 3 resource optimizations
 *
 * Tests edge cases: null brain, zero/large/tiny inputs, rapid cycles,
 * create-destroy stability, determinism.
 */
...
class ResourceRegression : public ::testing::Test {
protected:
    static nimcp_brain_t brain;
    static void SetUpTestSuite() {
        brain = nimcp_brain_create_fast("resource_regression",
...
nimcp_brain_t ResourceRegression::brain = nullptr;

TEST_F(ResourceRegression, NullBrainActiveCount) {
    EXPECT_EQ(nimcp_brain_get_active_neuron_count(nullptr), 0u);
}

TEST_F(ResourceRegression, NullBrainSparsity) {
    EXPECT_FLOAT_EQ(nimcp_brain_get_sparsity_ratio(nullptr), 0.0f);
}

TEST_F(ResourceRegression, ZeroInputInference) {
    if (!brain) GTEST_SKIP();
...
TEST_F(ResourceRegression, LargeInputValues) {
    if (!brain) GTEST_SKIP();
...
TEST_F(ResourceRegression, TinyInputValues) {
    if (!brain) GTEST_SKIP();
...
TEST_F(ResourceRegression, TrainWithZeroTarget) {
    if (!brain) GTEST_SKIP();
...
TEST_F(ResourceRegression, TrainWithAllOnesTarget) {
    if (!brain) GTEST_SKIP();
...
TEST_F(ResourceRegression, IdenticalInputsProduceSameOutput) {
    if (!brain) GTEST_SKIP();
...
TEST_F(ResourceRegression, RapidTrainInferCycles) {
    if (!brain) GTEST_SKIP();
...
TEST_F(ResourceRegression, CreateDestroyMultipleBrains) {
    for (int i = 0; i < CREATE_DESTROY_ITERS; i++) {
        nimcp_brain_t temp = nimcp_brain_create_fast("temp_resource",
                                    NIMCP_TASK_CLASSIFICATION,
...
TEST_F(ResourceRegression, SparsityValidAfterHeavyTraining) {
    if (!brain) GTEST_SKIP();
