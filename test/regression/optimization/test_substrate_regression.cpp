/**
 * @file test_substrate_regression.cpp
 * @brief Regression tests for Phase 4 neural substrate optimizations
 *
 * Tests edge cases: null brain, zero/large/tiny inputs, rapid cycles,
 * create-destroy stability, determinism with substrate changes.
 */
...
static constexpr int      HEAVY_TRAIN_ITERS     = 300;

class SubstrateRegression : public ::testing::Test {
protected:
    static nimcp_brain_t brain;
    static void SetUpTestSuite() {
        brain = nimcp_brain_create_fast("substrate_regression",
...
nimcp_brain_t SubstrateRegression::brain = nullptr;

TEST_F(SubstrateRegression, NullBrainActiveCount) {
    EXPECT_EQ(nimcp_brain_get_active_neuron_count(nullptr), 0u);
}

TEST_F(SubstrateRegression, NullBrainSparsity) {
    EXPECT_FLOAT_EQ(nimcp_brain_get_sparsity_ratio(nullptr), 0.0f);
}

TEST_F(SubstrateRegression, ZeroInputInference) {
    if (!brain) GTEST_SKIP();
...
TEST_F(SubstrateRegression, LargeInputValues) {
    if (!brain) GTEST_SKIP();
...
TEST_F(SubstrateRegression, TinyInputValues) {
    if (!brain) GTEST_SKIP();
...
TEST_F(SubstrateRegression, TrainWithZeroTarget) {
    if (!brain) GTEST_SKIP();
...
TEST_F(SubstrateRegression, TrainWithAllOnesTarget) {
    if (!brain) GTEST_SKIP();
...
TEST_F(SubstrateRegression, IdenticalInputsSameOutput) {
    if (!brain) GTEST_SKIP();
...
TEST_F(SubstrateRegression, RapidTrainInferCycles) {
    if (!brain) GTEST_SKIP();
...
TEST_F(SubstrateRegression, CreateDestroyMultipleBrains) {
    for (int i = 0; i < CREATE_DESTROY_ITERS; i++) {
        nimcp_brain_t temp = nimcp_brain_create_fast("temp_substrate",
                                    NIMCP_TASK_CLASSIFICATION,
...
TEST_F(SubstrateRegression, SparsityValidAfterHeavyTraining) {
    if (!brain) GTEST_SKIP();
...
TEST_F(SubstrateRegression, InferenceAfterHeavyTraining) {
    if (!brain) GTEST_SKIP();
