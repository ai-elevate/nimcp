...
static void test_enum_count_is_14(void) {
    TEST("BRAIN_CYCLE_COUNT == 14 after Wave 4");
    ASSERT_EQ((int)BRAIN_CYCLE_COUNT, 14, "BRAIN_CYCLE_COUNT changed");
    PASS();
}

static void test_enum_predictive_immune_slot(void) {
    TEST("BRAIN_CYCLE_PREDICTIVE_IMMUNE is the last enum value before COUNT");
    ASSERT_EQ((int)BRAIN_CYCLE_PREDICTIVE_IMMUNE,
              (int)BRAIN_CYCLE_COUNT - 1,
              "PREDICTIVE_IMMUNE not last — enum order changed");
    PASS();
}
