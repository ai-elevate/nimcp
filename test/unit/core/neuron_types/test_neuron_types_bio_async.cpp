...
protected:
    void SetUp() override {
        // Initialize logging system with correct API
        ...
        // Initialize bio-async router with correct API
...
TEST_F(NeuronTypesBioAsyncTest, GetDefaultParams) {
    ...
    // Threshold is more positive (less negative) than rest potential: -55mV > -70mV
    EXPECT_GT(params.lif.threshold, params.lif.rest_potential);
...
TEST_F(NeuronTypesBioAsyncTest, ComprehensiveLoggingCoverage) {
    ...
    for (size_t i = 0; i < sizeof(types) / sizeof(types[0]); i++) {
        ...
        nimcp_error_t result = neuron_type_get_default_params(type, &params);
