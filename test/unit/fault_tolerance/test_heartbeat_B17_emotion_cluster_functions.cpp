/**
 * @file test_heartbeat_B17_emotion_cluster_functions.cpp
 * @brief Unit tests for B17 heartbeat setter functions
...
class HeartbeatB17UnitTest : public ::testing::Test {
protected:
...
TEST_F(HeartbeatB17UnitTest, AllModulesSetNull) {
    for (size_t i = 0; i < B17_MODULE_COUNT; i++) {
        SCOPED_TRACE(B17_MODULES[i].name);
        B17_MODULES[i].set_fn(nullptr);
    }
}

TEST_F(HeartbeatB17UnitTest, AllModulesSetValid) {
    for (size_t i = 0; i < B17_MODULE_COUNT; i++) {
        SCOPED_TRACE(B17_MODULES[i].name);
...
TEST_F(HeartbeatB17UnitTest, AllModulesReplaceAgent) {
    ...
    for (size_t i = 0; i < B17_MODULE_COUNT; i++) {
        SCOPED_TRACE(B17_MODULES[i].name);
        B17_MODULES[i].set_fn(agent_);
        B17_MODULES[i].set_fn(agent2);
    }
    for (size_t i = 0; i < B17_MODULE_COUNT; i++) B17_MODULES[i].set_fn(nullptr);
    nimcp_health_agent_destroy(agent2);
}

TEST_F(HeartbeatB17UnitTest, AllModulesShareSameAgent) {
...
TEST_F(HeartbeatB17UnitTest, SetNullAfterValid) {
    for (size_t i = 0; i < B17_MODULE_COUNT; i++) {
        SCOPED_TRACE(B17_MODULES[i].name);
        B17_MODULES[i].set_fn(agent_);
        B17_MODULES[i].set_fn(nullptr);
...
TEST_F(HeartbeatB17UnitTest, ConcurrentSetClear) {
...
TEST_F(HeartbeatB17UnitTest, HeartbeatCounterIncrements) {
    nimcp_health_agent_start(agent_);
    for (size_t i = 0; i < B17_MODULE_COUNT; i++) B17_MODULES[i].set_fn(agent_);
    health_agent_stats_t before, after;
    nimcp_health_agent_get_stats(agent_, &before);
    nimcp_health_agent_heartbeat_ex(agent_, "B17_test", 0);
    nimcp_health_agent_get_stats(agent_, &after);
    EXPECT_GT(after.heartbeats_received, before.heartbeats_received);
    nimcp_health_agent_stop(agent_);
}

TEST_F(HeartbeatB17UnitTest, ModuleCount) {
    EXPECT_EQ(B17_MODULE_COUNT, 32u);
}

TEST_F(HeartbeatB17UnitTest, DoubleSetSameAgentIdempotent) {
    for (size_t i = 0; i < B17_MODULE_COUNT; i++) {
        SCOPED_TRACE(B17_MODULES[i].name);
        B17_MODULES[i].set_fn(agent_);
        B17_MODULES[i].set_fn(agent_);
