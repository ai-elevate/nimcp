 ...
 * @brief Tests for the mathematical optimization engine
 */
...
TEST(gd_quadratic) {
    ...
    ASSERT_NEAR(res.x[0], 0.0, TOL);
    ASSERT_NEAR(res.fval, 0.0, TOL);
    ASSERT_NE(res.status, OPT_ERROR);
...
TEST(nelder_mead_rosenbrock) {
    ...
    ASSERT_NEAR(res.x[0], 1.0, 0.05);
    ASSERT_NEAR(res.x[1], 1.0, 0.05);
    ASSERT_LT(res.fval, 0.01);
...
TEST(bfgs_rosenbrock) {
    ...
    ASSERT_NEAR(res.x[0], 1.0, 0.01);
    ASSERT_NEAR(res.x[1], 1.0, 0.01);
    ASSERT_LT(res.fval, 1e-6);
...
TEST(conjugate_gradient_quadratic) {
...
TEST(numerical_gradient) {
    ...
    ASSERT_NEAR(grad_num[0], 6.0, 1e-4);
}
...
TEST(gradient_norm_computation) {
    ...
    ASSERT_NEAR(norm, 5.0, 1e-9);
}

/* ---------- dispatch via opt_minimize ---------- */

TEST(minimize_dispatch) {
    optimizer_t *opt = optim_create(OPT_GRADIENT_DESCENT, 1);
    ASSERT_NOT_NULL(opt);
    opt_set_objective(opt, f_quadratic, g_quadratic, NULL, NULL);
    opt_set_convergence(opt, 1e-10, 1e-12, 10000, 0.1);
...
TEST_MAIN_BEGIN()
    ...
    RUN_TEST_SAFE(gd_quadratic);
    RUN_TEST_SAFE(gd_armijo_quadratic);
    RUN_TEST_SAFE(newton_quartic);
    RUN_TEST_SAFE(nelder_mead_rosenbrock);
    RUN_TEST_SAFE(bfgs_rosenbrock);
    RUN_TEST_SAFE(conjugate_gradient_quadratic);
    RUN_TEST_SAFE(numerical_gradient);
    RUN_TEST_SAFE(gradient_norm_computation);
    RUN_TEST_SAFE(minimize_dispatch);
TEST_MAIN_END()
