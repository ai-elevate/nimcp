/**
 * @file test_neuron_substrate_optimizations.cpp
 * @brief Unit tests for Phase 4 neuron substrate optimizations
 *
 * Tests: hot/cold struct split, EMA activity tracking, prefetch correctness,
 *        inline type parameters, forward pass with optimized neurons.
 */
...
class NeuronSubstrate : public ::testing::Test {
protected:
    static nimcp_brain_t brain;

    static void SetUpTestSuite() {
        brain = nimcp_brain_create_fast("neuron_substrate_unit",
...
nimcp_brain_t NeuronSubstrate::brain = nullptr;

// =============================================================================
// Tests
// =============================================================================

TEST_F(NeuronSubstrate, BrainCreated) {
    ASSERT_NE(brain, nullptr);
}

// --- Forward pass correctness with optimized neurons ---

TEST_F(NeuronSubstrate, InferenceProducesValidOutput) {
    if (!brain) GTEST_SKIP();
...
TEST_F(NeuronSubstrate, DeterministicInference) {
    if (!brain) GTEST_SKIP();
...
// --- Training with optimized neuron substrate ---

TEST_F(NeuronSubstrate, TrainingProducesFiniteLoss) {
    if (!brain) GTEST_SKIP();
...
TEST_F(NeuronSubstrate, RepeatedTrainingStable) {
    if (!brain) GTEST_SKIP();
...
TEST_F(NeuronSubstrate, SparsityTrackingAfterInference) {
    if (!brain) GTEST_SKIP();
...
TEST_F(NeuronSubstrate, InferenceThroughputBenchmark) {
    if (!brain) GTEST_SKIP();
...
TEST_F(NeuronSubstrate, TrainingThroughputBenchmark) {
    if (!brain) GTEST_SKIP();
...
TEST_F(NeuronSubstrate, LanguagePipelineStillWorks) {
    if (!brain) GTEST_SKIP();

    nimcp_brain_learn_language(brain, "neurons fire through axons and dendrites", nullptr);

    float semantic[SEMANTIC_DIM] = {};
    float conf = 0.0f;
    nimcp_status_t s = nimcp_brain_comprehend(
        brain, "neural firing", semantic, SEMANTIC_DIM, &conf);
    EXPECT_EQ(s, NIMCP_OK);
}

// --- Interleaved operations ---

TEST_F(NeuronSubstrate, InterleavedTrainInferLanguage) {
    if (!brain) GTEST_SKIP();
    ...
    for (int i = 0; i < WARMUP_ITERS; i++) {
        ...
        switch (i % 3) {
            ...
            case 2: {
                nimcp_brain_learn_language(brain, "substrate optimization", nullptr);
                break;
            }
        }
    }
