...
TEST_F(ProprioceptionTest, UpdatePosition) {
    ...
    EXPECT_EQ(nimcp_swarm_proprio_update_position(system, &pos, nullptr), NIMCP_OK);
}
TEST_F(ProprioceptionTest, UpdateNeighbor) {
    nimcp_swarm_position_t pos = {6.0, 5.0, 0.0};
    EXPECT_EQ(nimcp_swarm_proprio_update_neighbor(system, 2, &pos, 0.9), NIMCP_OK);
}
TEST_F(ProprioceptionTest, GetNeighbors) {
    nimcp_swarm_neighbor_t neighbors[10];
    uint32_t count;
    EXPECT_EQ(nimcp_swarm_proprio_get_neighbors(system, neighbors, 10, &count), NIMCP_OK);
}
TEST_F(ProprioceptionTest, ClassifyShape) {
    nimcp_swarm_shape_descriptor_t desc;
    EXPECT_EQ(nimcp_swarm_proprio_classify_shape(system, &desc), NIMCP_OK);
}
...
TEST_F(ProprioceptionTest, BoundaryRole) {
    ...
    EXPECT_EQ(nimcp_swarm_proprio_boundary_role(system, &desc), NIMCP_OK);
}
TEST_F(ProprioceptionTest, CalculateDensity) {
    nimcp_swarm_density_info_t info;
    EXPECT_EQ(nimcp_swarm_proprio_density(system, &info), NIMCP_OK);
}
TEST_F(ProprioceptionTest, EstimateCOM) {
    nimcp_swarm_com_estimate_t est;
    EXPECT_EQ(nimcp_swarm_proprio_estimate_com(system, &est), NIMCP_OK);
}
TEST_F(ProprioceptionTest, FormationMetrics) {
    nimcp_swarm_formation_metrics_t metrics;
    EXPECT_EQ(nimcp_swarm_proprio_formation_metrics(system, &metrics), NIMCP_OK);
}
TEST_F(ProprioceptionTest, DetectVibration) {
    double signal[64] = {0};
    for (int i = 0; i < 64; i++) signal[i] = sin(i * 0.1);
    nimcp_swarm_vibration_data_t vib;
    EXPECT_EQ(nimcp_swarm_proprio_detect_vibration(system, signal, 64, &vib), NIMCP_OK);
}
