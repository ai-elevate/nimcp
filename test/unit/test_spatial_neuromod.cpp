...
protected:
    ...
    void SetUp() override {
        ...
        network = neural_network_create(NUM_NEURONS);
        ASSERT_NE(network, nullptr);

        // Add some connections (ring topology for simple tests)
        for (uint32_t i = 0; i < NUM_NEURONS; i++) {
            uint32_t next = (i + 1) % NUM_NEURONS;
            neural_network_add_synapse(network, i, next, 0.5f);
        }
...
TEST_F(SpatialNeuromodTest, InitialConcentrations) {
    ...
    float baseline = config.baseline;
    for (uint32_t i = 0; i < NUM_NEURONS; i++) {
...
TEST_F(SpatialNeuromodTest, DiffusionPropagation) {
    ...
    EXPECT_GT(spatial_neuromod_get_concentration(field, 0), config.baseline);
    ...
    EXPECT_GT(c1, config.baseline);
    EXPECT_GT(c99, config.baseline);
...
TEST_F(SpatialNeuromodTest, BatchRelease) {
    ...
    EXPECT_GT(spatial_neuromod_get_concentration(field, 0), config.baseline);
    EXPECT_GT(spatial_neuromod_get_concentration(field, 10), config.baseline);
    EXPECT_GT(spatial_neuromod_get_concentration(field, 20), config.baseline);
}
...
TEST_F(SpatialNeuromodTest, StabilityWithSubsteps) {
    ...
    spatial_neuromod_config_t substep_config = config;
...
TEST_F(SpatialNeuromodTest, ResetToBaseline) {
    ...
    for (uint32_t i = 0; i < NUM_NEURONS; i++) {
        float c = spatial_neuromod_get_concentration(field, i);
        EXPECT_FLOAT_EQ(c, config.baseline);
    }
}
