...
START_TEST(test_type_name_unknown)
{
    ...
    ck_assert_ptr_nonnull(name);
    /* Should return "UNKNOWN" or similar for invalid types */
...
START_TEST(test_is_supported_all_valid_types)
{
    ...
    for (uint8_t type = 0; type <= NIMCP_NETWORK_HYBRID; type++) {
        ...
        /* All defined types should be supported */
        if (type <= NIMCP_NETWORK_CNN) {
...
START_TEST(test_init_null_config)
{
    ...
    if (brain != NULL) {
        ...
        ck_assert_msg(brain != NULL, "Could not create test brain");
    }
}
END_TEST
...
START_TEST(test_step_null_result)
{
    ...
    if (brain != NULL) {
        ...
        /* May succeed or fail depending on initialization state,
           but should not crash with NULL result parameter */
        (void)ret;
...
START_TEST(test_set_reward_valid_positive)
{
    ...
    if (brain != NULL) {
        ...
        /* Should return 0 for SNN, -1 for non-SNN */
...
START_TEST(test_reset_valid_brain)
{
    ...
    if (brain != NULL) {
        ...
        /* May return 0 (success) or negative (if no training context) */
...
START_TEST(test_reset_uninitialized_brain)
{
    ...
    if (brain != NULL) {
        ...
        int result = training_dispatch_reset(brain);
        /* Should return negative if no training context exists */
        ck_assert(result == 0 || result < 0);
...
START_TEST(test_full_lifecycle_adaptive)
{
    ...
    if (brain != NULL) {
        ...
        ck_assert_int_eq(step_result, 0);
        ...
        ck_assert_int_eq(stats_result, 0);
        ck_assert_uint_ge(total_steps, 1);
