...
BRAIN_SOCKET = '/var/run/athena/brain.sock'
...
def fetch_step_cache():
    """Refresh STEP_CACHE from the local immersive_state.json on this pod.

    The trainer (immerse_athena.py) writes immersive_state.json on each
    snapshot. We mirror it into STEP_CACHE so collect() has a fast read
    even if the file is mid-write. 2026-04-26 fix: the previous
    implementation spawned an ssh subprocess back to a stale RunPod IP
    (74.2.96.55) — the script runs ON the pod, so a local read is correct.
    """
    src = '/workspace/nimcp/checkpoints/athena/immersive_state.json'
    try:
        with open(src) as f:
            payload = f.read()
        if payload.strip():
